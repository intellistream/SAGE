...
SAGE eliminates the artificial separation between data processing and control logic. Conditional branching, routing decisions, and error handling are expressed declaratively within the graph structure:

```python
# Complex routing logic expressed declaratively
pipeline = (env
    .from_source(QuerySource)
    .branch({
        "factual": DenseRetriever,
        "creative": CreativeGenerator,
        "default": HybridRetriever
    }, classifier=QueryClassifier)
    .merge()
    .map(AnswerSynthesizer)
    .sink(ResponseSink)
)
```

### Native Stateful Computing

Unlike stateless function-based approaches, SAGE treats memory as a first-class citizen. Session state, conversation history, and long-term knowledge are modeled as stateful operators within the graph:

```python
# Memory-aware reasoning pipeline
pipeline = (env
    .from_source(ConversationSource)
    .stateful_map(SessionMemory, config["session"])
    .map(ContextualRetriever, config["retriever"])
    .stateful_map(ConversationBuffer, config["buffer"])
    .map(ChatGenerator, config["generator"])
...
- **Debug Console**: Operator-level profiling and error tracking

```bash
# Start the dashboard
cd frontend/web_ui && python main.py --host 0.0.0.0 --port 8080
...
pipeline = (env
    ...
    .map(DenseRetriever, config["retriever"])
    .map(QAPromptor, config["promptor"])
    .map(VLLMGenerator, config["generator"])  # Use vLLM for distributed inference
    .sink(TerminalSink)
)
...
```python
# Express complex routing logic declaratively
pipeline = (env
    .from_source(QuerySource)
    .branch({
        "factual": DenseRetriever,
...
```python
# Memory-aware conversation pipeline
pipeline = (env
    .from_source(ConversationSource)
    .stateful_map(SessionMemory, config["session"])
    .map(ContextualRetriever, config["retriever"])
...
SAGE provides comprehensive command-line tools for cluster management and job control:

```bash
# Cluster management
sage cluster start                    # Start Ray cluster
sage cluster status                   # Check cluster health
sage cluster scale add worker:2       # Add worker nodes

# Job management
sage job list                         # List running jobs
sage job show <job_id>                # Show job details
sage job cancel <job_id>              # Cancel job

# System tools
sage-dev test                         # Run tests
sage-dev analyze                      # Dependency analysis
```
