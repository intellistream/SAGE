...
from ..models import VDBRetrievalResult, GraphMemoryResult, ClusteringResult
...
class TestDBSCANClusteringOperator(unittest.TestCase):
    ...
    def _create_sample_vdb_results(self, n: int = 10) -> list[VDBRetrievalResult]:
        ...
        for i in range(n):
            ...
            results.append(VDBRetrievalResult(
                doc_id=f"doc_{i}",
                content=content,
                ...
                stage=1,
                metadata={
                    "embedding": self._generate_similar_embedding(group, i)
                }
            ))
        return results
    ...
    def test_deduplication_rate(self):
        ...
        for i in range(30):
            ...
            results.append(VDBRetrievalResult(
                ...
                stage=1,
                metadata={
                    "embedding": self._generate_similar_embedding(group, i % 3)
                }
            ))
...
class TestSimilarityDeduplicator(unittest.TestCase):
    ...
    def setUp(self):
        """测试前准备"""
        self.operator = SimilarityDeduplicator(
            threshold=0.95,
            use_simhash=False
        )
    
    def _create_duplicate_docs(self, n_groups: int = 5, group_size: int = 3) -> list[VDBRetrievalResult]:
        """创建包含重复的文档"""
        results = []
...
class TestVisualizationAndAnalysis(unittest.TestCase):
    ...
    def _create_sample_clusters(self, n: int = 5) -> list[ClusteringResult]:
        ...
        for i in range(n):
            ...
            clusters.append(ClusteringResult(
                cluster_id=i,
