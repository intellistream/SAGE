...
def evaluate_predictions(predictions: List[str], ground_truths: List[List[str]], 
...
def calculate_detailed_scores(results_data: Dict[str, Any], metric: str = "all") -> Dict[str, Any]:
    """
    计算详细的评估分数
    Args:
        results_data: 推理结果数据
    ...
    for item in results:
        ...
        if metric in ["f1", "all"]:
            sample_scores["f1"] = max([compute_f1(pred, gt) for gt in truths]) if truths else 0.0
        
        if metric in ["exact_match", "all"]:
            sample_scores["exact_match"] = max([compute_exact_match(pred, gt) for gt in truths]) if truths else 0
...
def analyze_retrieval_quality(evaluation_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    retrieval_analysis = {
        ...
        "context_relevance_rate": np.mean(context_relevance_scores) if context_relevance_scores else 0.0
    }
...
def main():
    ...
    parser.add_argument("--results-file", type=str, required=True, help="推理结果文件路径")
    parser.add_argument("--metric", choices=["accuracy", "f1", "exact_match", "all"], 
                       default="all", help="评估指标")
    parser.add_argument("--output", type=str, help="输出评估结果文件路径")
    parser.add_argument("--show-details", action="store_true", help="显示每个样本的详细分数")
    ...
    if args.show_details:
        ...
        for i, item in enumerate(evaluation_result["detailed_results"][:10]):
            ...
            if 'retrieved_context' in item and item['retrieved_context']:
                ...
                print(f"   检索上下文预览: {item['retrieved_context'][0][:150]}..." if item['retrieved_context'] else "无")
