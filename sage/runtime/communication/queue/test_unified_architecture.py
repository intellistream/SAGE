...
class TestUnifiedQueueArchitecture:
    ...
    def test_legacy_wrapper_compatibility(self):
        """测试向后兼容包装器"""
        descriptor = QueueDescriptor.create_local_queue(queue_id="legacy_test")
        wrapper = LegacyQueueStubWrapper(descriptor)
        
        # 测试向后兼容接口
        wrapper.put("legacy_item")
        item = wrapper.get()
        assert item == "legacy_item"
        
        # 测试 to_descriptor 方法
        restored_descriptor = wrapper.to_descriptor()
        assert restored_descriptor is descriptor
        
        # 测试 from_descriptor 类方法
        wrapper2 = LegacyQueueStubWrapper.from_descriptor(descriptor)
        assert wrapper2._queue_descriptor is descriptor
...
if __name__ == "__main__":
    ...
    try:
        ...
        test.test_legacy_wrapper_compatibility()
        print("✓ Legacy wrapper tests passed")
