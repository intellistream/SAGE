#!/usr/bin/env python3
"""
测试统一多态队列描述符 (UnifiedQueueDescriptor) 的功能
...
def test_basic_functionality():
    ...
    # 创建本地队列描述符
    desc = UnifiedQueueDescriptor.create_local_queue("test_local", maxsize=10)
...
def test_serialization():
    ...
    # 创建可序列化的描述符
    desc1 = UnifiedQueueDescriptor.create_shm_queue("test_shm", "shared_queue_1", maxsize=100)
    print(f"原始描述符: {desc1}")
    ...
    # 反序列化
    desc2 = UnifiedQueueDescriptor.from_json(json_str)
...
def test_non_serializable_handling():
    ...
    desc = UnifiedQueueDescriptor.create_local_queue("test_with_queue", queue_instance=real_queue)
...
def test_lazy_loading():
    ...
    desc = UnifiedQueueDescriptor.create_ray_actor_queue("test_actor", "ray_queue_1")
...
def test_polymorphic_usage():
    ...
    # 创建不同类型的队列描述符
    queues = [
        UnifiedQueueDescriptor.create_local_queue("poly_local"),
        UnifiedQueueDescriptor.create_shm_queue("test_shm", "poly_shm"),
        UnifiedQueueDescriptor.create_rpc_queue("localhost", 8080, "poly_rpc")
    ]
