...
class Dispatcher():
    ...
    def _create_queue_descriptors(self):
        """预创建所有需要的队列描述符"""
        self.logger.info("Creating queue descriptors for all nodes and services")
        
        # 获取默认队列参数
        default_params = self.queue_creation_strategy.get_default_queue_params(self.remote)
        
        # 1. 为每个graph node创建input queue descriptor
        for node_name, graph_node in self.graph.nodes.items():
            ...
            self.logger.debug(f"Created input queue descriptor for node: {node_name}")
            
            # 2. 为每个graph node创建service response queue descriptor
            service_response_queue_desc = self.queue_creation_strategy.create_service_response_queue(
                service_name="response",  # 通用响应队列名称
                node_name=node_name,
                is_remote=self.remote,
                **default_params
            )
            self.queue_descriptors[f"service_response_{node_name}"] = service_response_queue_desc
            self.logger.debug(f"Created service response queue descriptor for node: {node_name}")
        
        # 3. 为每个service创建request queue descriptor  
        for service_node_name, service_node in self.graph.service_nodes.items():
            service_name = service_node.service_name
            request_queue_desc = self.queue_creation_strategy.create_service_request_queue(
                service_name=service_name,
                is_remote=self.remote,
                **default_params
            )
            self.queue_descriptors[f"service_request_{service_name}"] = request_queue_desc
            self.logger.debug(f"Created service request queue descriptor for service: {service_name}")
        
        self.logger.info(f"Created {len(self.queue_descriptors)} queue descriptors")
    
    def _distribute_queue_descriptors(self):
        """将队列描述符分发到相应的运行时上下文"""
        self.logger.info("Distributing queue descriptors to runtime contexts")
        ...
        for service_node_name, service_node in self.graph.service_nodes.items():
            service_name = service_node.service_name
            # service request队列
            if f"service_request_{service_name}" in self.queue_descriptors:
                service_request_descriptors[service_name] = self.queue_descriptors[f"service_request_{service_name}"]
            
        # 收集所有graph node的service response queue descriptors
        for node_name, graph_node in self.graph.nodes.items():
            if f"service_response_{node_name}" in self.queue_descriptors:
                service_response_descriptors[node_name] = self.queue_descriptors[f"service_response_{node_name}"]
        
        # 为每个graph node设置队列描述符
        for node_name, graph_node in self.graph.nodes.items():
            ctx = graph_node.ctx
            
            # 设置input queue descriptor
            if f"input_{node_name}" in self.queue_descriptors:
                ctx.set_input_queue_descriptor(self.queue_descriptors[f"input_{node_name}"])
                self.logger.debug(f"Set input queue descriptor for node: {node_name}")
            
            # 设置service response queue descriptor
            if f"service_response_{node_name}" in self.queue_descriptors:
                ctx.set_service_response_queue_descriptor(self.queue_descriptors[f"service_response_{node_name}"])
                self.logger.debug(f"Set service response queue descriptor for node: {node_name}")
            
            # 设置所有service的request queue descriptors（让graph node可以访问所有service）
            ctx.set_service_request_queue_descriptors(service_request_descriptors)
            self.logger.debug(f"Set service request queue descriptors for node: {node_name}")
        
        # 为每个service task设置队列描述符
        for service_node_name, service_node in self.graph.service_nodes.items():
            service_name = service_node.service_name
            ctx = service_node.ctx
            
            # 设置request queue descriptor（service自己的请求队列）
            if f"service_request_{service_name}" in self.queue_descriptors:
                ctx.set_request_queue_descriptor(self.queue_descriptors[f"service_request_{service_name}"])
                self.logger.debug(f"Set request queue descriptor for service: {service_name}")
            
            # 设置所有graph node的service response queue descriptors（让service可以访问所有response队列）
            ctx.set_service_response_queue_descriptors(service_response_descriptors)
