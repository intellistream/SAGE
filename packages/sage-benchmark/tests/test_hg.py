...
class TestHuggingFaceConnectivity:
    ...
    def test_model_file_accessibility(
        self, site_name, base_url, repo_id, filename
    ):
        """
    ...
    def test_connectivity_summary(self):
        ...
        official_ok = results.get("HuggingFace Official", {}).get("site_accessible", False) and \
                      results.get("HuggingFace Official", {}).get("file_accessible", False)
        mirror_ok = results.get("HF Mirror", {}).get("site_accessible", False) and \
                    results.get("HF Mirror", {}).get("file_accessible", False)
