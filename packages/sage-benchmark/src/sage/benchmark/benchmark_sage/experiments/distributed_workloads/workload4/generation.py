...
class Workload4MetricsSink(SinkFunction):
    ...
    def _write_header(self) -> None:
        ...
        if self.verbose:
            print(f"[Workload4MetricsSink] Initialized:")
    ...
    def execute(self, data: tuple[str, str, "Workload4Metrics"]) -> None:
        ...
        if self.verbose and self.count % 10 == 0:
            ...
            print(f"[Workload4MetricsSink] Processed {self.count} tasks, throughput: {throughput:.2f} QPS")
    ...
    def _write_metrics_to_file(self, metrics: "Workload4Metrics") -> None:
        ...
        with open(self.jsonl_file, "a") as f:
            record = {
                ...
                "vdb1_latency": metrics.vdb1_end_time - metrics.vdb1_start_time if metrics.vdb1_end_time > 0 else 0,
                "vdb2_latency": metrics.vdb2_end_time - metrics.vdb2_start_time if metrics.vdb2_end_time > 0 else 0,
                "graph_latency": metrics.graph_end_time - metrics.graph_start_time if metrics.graph_end_time > 0 else 0,
    ...
    def _generate_summary_report(self) -> None:
        ...
        join_latencies = [(m.join_time - m.query_arrival_time) * 1000 for m in self.metrics if m.join_time > 0]
        ...
        report = {
            ...
            "intermediate_results": {
                ...
                "dedup_rate": total_duplicates / (total_vdb1_results + total_vdb2_results) if (total_vdb1_results + total_vdb2_results) > 0 else 0,
            },
            "quality_metrics": {
                "avg_semantic_join_score": statistics.mean([m.semantic_join_score for m in self.metrics if m.semantic_join_score > 0]) if any(m.semantic_join_score > 0 for m in self.metrics) else 0.0,
                "avg_final_rerank_score": statistics.mean([m.final_rerank_score for m in self.metrics if m.final_rerank_score > 0]) if any(m.final_rerank_score > 0 for m in self.metrics) else 0.0,
                "avg_diversity_score": statistics.mean([m.diversity_score for m in self.metrics if m.diversity_score > 0]) if any(m.diversity_score > 0 for m in self.metrics) else 0.0,
            },
        }
        ...
        if self.verbose:
            ...
            print(f"  Avg Join Matched Docs: {report['intermediate_results']['avg_join_matched_docs']:.1f}")
