...
class Workload4QuerySource(SourceFunction):
    ...
    def _generate_query(self, query_id: str, timestamp: float) -> QueryEvent:
        ...
        query_type = self.rng.choices(
            list(self.query_type_dist.keys()),
            weights=list(self.query_type_dist.values()),
            k=1
        )[0]
        
        category = self.rng.choices(
            list(self.category_dist.keys()),
            weights=list(self.category_dist.values()),
            k=1
        )[0]
    ...
    def execute(self, data=None) -> QueryEvent | StopSignal:
        ...
        if not self._initialization_logged:
            self.logger.info(f"[INIT] Workload4QuerySource initialized with num_tasks={self.num_tasks}, qps={self.qps}")
            self._initialization_logged = True
        
        # DEBUG: Log EVERY execution
        self.logger.info(f"[EXEC] Workload4QuerySource.execute() called: task_count={self.task_count}, num_tasks={self.num_tasks}")
...
class Workload4DocumentSource(SourceFunction):
    ...
    def execute(self, data=None) -> DocumentEvent | StopSignal:
        ...
        # DEBUG: 第一次执行时记录初始化信息
        if not self._initialization_logged:
            self.logger.info(f"[INIT] Workload4DocumentSource initialized with num_docs={self.num_docs}, qps={self.qps}")
            self._initialization_logged = True
        
        # DEBUG: Log EVERY execution
        self.logger.info(f"[EXEC] Workload4DocumentSource.execute() called: doc_count={self.doc_count}, num_docs={self.num_docs}")
