...
当前实现使用 `self.call_service("embedding")` 和 `self.call_service("vector_db")` 进行向量检索，这种服务化方式在分布式环境下会带来额外复杂性。现在需要改为直接在算子内部使用简单的 FAISS VDB，类似于 `FiQAFAISSRetriever` 的实现方式。
...
2. **pipeline.py** - 可能需要移除服务注册（如果 Adaptive-RAG 不再依赖服务）
...
**目标实现**:
与 `SingleRetrievalStrategy` 类似，添加：
...
2. **远程 embedding 服务仍然需要** - 只是 VDB 改为本地 FAISS，embedding 仍用远程服务
3. **保持 LLM 调用方式不变** - `_generate_via_service` 和 `_llm_call_via_service` 保持原样
4. **考虑 Ray 分布式序列化** - FAISS 索引在每个 worker 上延迟加载，避免序列化问题
5. **保留 `__module__` 设置** - 确保 Ray 序列化一致性（operators.py 末尾的 `_ADAPTIVE_RAG_CLASSES` 列表）
