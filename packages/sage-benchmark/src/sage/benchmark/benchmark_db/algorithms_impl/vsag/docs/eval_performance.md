...
actually, the yaml config is just like how to config on command line. the optional config is same on
both runner methods

## 2. The Meaning Of The Output

After running the `eval_performance`. We will get the result on console. Different config has
different output.
