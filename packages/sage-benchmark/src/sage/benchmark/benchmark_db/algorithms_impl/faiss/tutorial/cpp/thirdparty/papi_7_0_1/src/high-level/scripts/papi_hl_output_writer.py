...
def merge_json_files(source):
    ...
    for item in file_list:
        ...
        try:
            ...
            print(f"Cannot open file {file_name} ({repr(ioe)})")
            return
    ...
    def get_json(self):
        ...
        for name in self.regions:
                        ...
                        if event_key == "region_count":
                            ...
                            if region_count > 1:
                                ...
                                if event_key == "cycles" or event_key == "real_time_nsec":
                                    ...
                                    if (
                                        event_definitions[event_key]["type"] == "delta"
                                        and event_definitions[event_key]["component"]
                                        == "perf_event"
                                    ):
                                        events[event_key]["total"] = event_value.get_sum()
                                ...
                                if event_key == "cycles" or event_key == "real_time_nsec":
                                    ...
                                    if (
                                        event_definitions[event_key] == "instant"
                                        and region_count > 1
                                    ):
                                        events[event_key] = OrderedDict()
                                        events[event_key]["min"] = event_value.get_min()
            ...
            if self.regions[name]["rank_num"] > 1 or self.regions[name]["thread_num"] > 1:
                ...
                events["Number of threads per rank"] = int(
                    self.regions[name]["thread_num"] / self.regions[name]["rank_num"]
                )
...
def derive_sum_json_object(data):
    ...
    for region_key, region_value in data.items():
        ...
        if "perf::TASK-CLOCK" in events:
            ...
            if region_cnt > 1:
                derive_events[event_name] = convert_value(
                    events["perf::TASK-CLOCK"]["total"], "CPUtime"
                )
            else:
            ...
            try:
                if region_cnt > 1:
                    ipc = float(
                        format(
                            float(
                                int(events["PAPI_TOT_INS"][metric])
                                / int(events["PAPI_TOT_CYC"][metric])
                            ),
                            ".2f",
                        )
                    )
                    ...
                    ipc = float(
                        format(
                            float(int(events["PAPI_TOT_INS"]) / int(events["PAPI_TOT_CYC"])), ".2f"
                        )
                    )
            except:
                ipc = "n/a"
        ...
        for rate_event in event_rate_names:
            if rate_event in events:
                ...
                try:
                    if region_cnt > 1:
                        rate = float(
                            format(float(events[rate_event][metric]) / 1000000 / rt[metric], ".2f")
                        )
                    else:
                        rate = float(
                            format(float(events[rate_event]) / 1000000 / rt[metric], ".2f")
                        )
                except:
                    rate = "n/a"
...
def derive_events(events):
    ...
    if "real_time_nsec" in events:
        if isinstance(events["real_time_nsec"], dict):
            ...
            derive_events[derived_metric_names["real_time_nsec"]] = derive_read_events(
                events["real_time_nsec"], "Runtime"
            )
        else:
            rt = convert_value(events["real_time_nsec"], "Runtime")
            derive_events[derived_metric_names["real_time_nsec"]] = convert_value(
                events["real_time_nsec"], "Runtime"
            )
        del events["real_time_nsec"]
    ...
    if "perf::TASK-CLOCK" in events:
        if isinstance(events["perf::TASK-CLOCK"], dict):
            derive_events[derived_metric_names["perf::TASK-CLOCK"]] = derive_read_events(
                events["perf::TASK-CLOCK"], "CPUtime"
            )
        else:
            derive_events[derived_metric_names["perf::TASK-CLOCK"]] = convert_value(
                events["perf::TASK-CLOCK"], "CPUtime"
            )
        del events["perf::TASK-CLOCK"]
    ...
    for rank, rank_value in data["ranks"].items():
        for thread, thread_value in rank_value["threads"].items():
            for region, region_value in thread_value["regions"].items():
                data["ranks"][rank]["threads"][thread]["regions"][region] = derive_events(
                    region_value
                )
    return data
...
def write_json_file(data, file_name):
    with open(file_name, "w", encoding="utf8") as outfile:
        str_ = json.dumps(
            data, indent=4, sort_keys=False, separators=(",", ": "), ensure_ascii=False
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="papi_hl_output",
        help="Measurement directory of raw data.",
    )
    parser.add_argument(
        "--format", type=str, required=False, default="json", help="Output format, e.g. json."
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    if os.path.isdir(source) == False:
        print(f"Measurement directory '{source}' does not exist!\n")
        parser.print_help()
        parser.exit()
    ...
    if output_format != "json":
        print(f"Output format '{output_format}' is not supported!\n")
        parser.print_help()
        parser.exit()
    ...
    if output_type != "detail" and output_type != "summary":
        print(f"Output type '{output_type}' is not supported!\n")
        parser.print_help()
        parser.exit()
    ...
    if output_notation != "raw" and output_notation != "derived":
        print(f"Output notation '{output_notation}' is not supported!\n")
        parser.print_help()
        parser.exit()
...
if __name__ == "__main__":
    args = parse_args()
    main(format=args.format, source=args.source, type=args.type, notation=args.notation)
