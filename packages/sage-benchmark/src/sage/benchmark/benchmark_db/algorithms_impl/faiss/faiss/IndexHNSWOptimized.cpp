...
#include <faiss/IndexFlatCodes.h>  //add by ghr
#include <faiss/IndexIVFPQ.h>
#include <faiss/impl/AuxIndexStructures.h>
#include <faiss/impl/HNSWGorder.h>  //add by ghr
...
IndexHNSWOptimized::IndexHNSWOptimized(int d, int M, MetricType metric)
        : Index(d, metric), hnsw(M) {}

IndexHNSWOptimized::IndexHNSWOptimized(Index* storage, int M)
        : Index(storage->d, storage->metric_type), hnsw(M), storage(storage) {}

IndexHNSWOptimized::~IndexHNSWOptimized() {
    if (own_fields) {
        delete storage;
    }
}

void IndexHNSWOptimized::train(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexHNSWFlatOptimized (or variants) instead of IndexHNSWOptimized directly");
    // hnsw structure does not require training
    storage->train(n, x);
    is_trained = true;
}
std::vector<idx_t> IndexHNSWOptimized::search_arrays(idx_t n, const std::vector<float> x, idx_t k, int param){
...
void IndexHNSWOptimized::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
...
void IndexHNSWOptimized::add(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexHNSWFlatOptimized (or variants) instead of IndexHNSWOptimized directly");
...
void IndexHNSWOptimized::reset() {
    hnsw.reset();
    storage->reset();
    ntotal = 0;
}

void IndexHNSWOptimized::reconstruct(idx_t key, float* recons) const {
    storage->reconstruct(key, recons);
}

void IndexHNSWOptimized::shrink_level_0_neighbors(int new_size) {
#pragma omp parallel
    {
        std::unique_ptr<DistanceComputer> dis(
                storage_distance_computer(storage));
...
void IndexHNSWOptimized::search_level_0(
        idx_t n,
        const float* x,
        idx_t k,
...
void IndexHNSWOptimized::init_level_0_from_knngraph(
        int k,
        const float* D,
        const idx_t* I) {
...
void IndexHNSWOptimized::init_level_0_from_entry_points(
        int n,
...
void IndexHNSWOptimized::reorder_links() {
...
void IndexHNSWOptimized::link_singletons() {
...
//modified by ghr
void IndexHNSWOptimized::permute_entries(const idx_t* perm) {
    FAISS_THROW_IF_NOT(storage);
    FAISS_THROW_IF_NOT(ntotal == hnsw.levels.size());

    auto flat_storage = dynamic_cast<IndexFlatCodes*>(storage);
    FAISS_THROW_IF_NOT_MSG(
            flat_storage,
            "IndexHNSWOptimized::permute_entries currently supports Flat storage only");

    flat_storage->permute_entries(perm);
    hnsw.permute_entries(perm);

    if (reconstruct_from_neighbors && reconstruct_from_neighbors->code_size) {
        auto& recon = *reconstruct_from_neighbors;
        FAISS_THROW_IF_NOT(recon.code_size * ntotal == recon.codes.size());
        std::vector<uint8_t> new_codes(recon.codes.size());
        for (idx_t i = 0; i < ntotal; i++) {
            memcpy(new_codes.data() + i * recon.code_size,
                   recon.codes.data() + perm[i] * recon.code_size,
                   recon.code_size);
        }
        recon.codes.swap(new_codes);
    }
}

//add by ghr
void IndexHNSWOptimized::reorder_gorder(int window) {
    if (ntotal == 0) {
        return;
    }
    if (window < 0) {
    ...
    std::vector<idx_t> old_to_new =
            compute_hnsw_gorder_old_to_new(hnsw, window);
    if (old_to_new.empty()) {
        return;
    }
    FAISS_THROW_IF_NOT(old_to_new.size() == ntotal);

    std::vector<idx_t> perm(old_to_new.size());
    for (idx_t old_id = 0; old_id < (idx_t)old_to_new.size(); old_id++) {
        idx_t new_id = old_to_new[old_id];
        FAISS_THROW_IF_NOT(new_id >= 0 && new_id < (idx_t)old_to_new.size());
        perm[new_id] = old_id;
    }
...
/**************************************************************
 * ReconstructFromNeighborsOptimized implementation
 **************************************************************/

ReconstructFromNeighborsOptimized::ReconstructFromNeighborsOptimized(
        const IndexHNSWOptimized& index,
        size_t k,
        size_t nsq)
...
void ReconstructFromNeighborsOptimized::reconstruct(
        storage_idx_t i,
        float* x,
        float* tmp) const {
...
void ReconstructFromNeighborsOptimized::reconstruct_n(
        storage_idx_t n0,
        storage_idx_t ni,
        float* x) const {
#pragma omp parallel
...
size_t ReconstructFromNeighborsOptimized::compute_distances(
        size_t n,
...
void ReconstructFromNeighborsOptimized::get_neighbor_table(storage_idx_t i, float* tmp1)
        const {
    const HNSW& hnsw = index.hnsw;
    size_t begin, end;
    hnsw.neighbor_range(i, 0, &begin, &end);
...
/// called by add_codes
void ReconstructFromNeighborsOptimized::estimate_code(
        const float* x,
        storage_idx_t i,
        uint8_t* code) const {
...
void ReconstructFromNeighborsOptimized::add_codes(size_t n, const float* x) {
...
IndexHNSWFlatOptimized::IndexHNSWFlatOptimized() {
    is_trained = true;
}

IndexHNSWFlatOptimized::IndexHNSWFlatOptimized(int d, int M, MetricType metric)
        : IndexHNSWOptimized(
...
IndexHNSWPQOptimized::IndexHNSWPQOptimized() = default;

IndexHNSWPQOptimized::IndexHNSWPQOptimized(int d, int pq_m, int M, int pq_nbits)
        : IndexHNSWOptimized(new IndexPQ(d, pq_m, pq_nbits), M) {
    own_fields = true;
    is_trained = false;
}

void IndexHNSWPQOptimized::train(idx_t n, const float* x) {
    IndexHNSWOptimized::train(n, x);
...
IndexHNSWSQOptimized::IndexHNSWSQOptimized(
        int d,
        ScalarQuantizer::QuantizerType qtype,
        int M,
        MetricType metric)
        : IndexHNSWOptimized(new IndexScalarQuantizer(d, qtype, metric), M) {
    is_trained = this->storage->is_trained;
    own_fields = true;
}

IndexHNSWSQOptimized::IndexHNSWSQOptimized() = default;

/**************************************************************
 * IndexHNSW2Level implementation
 **************************************************************/

IndexHNSW2LevelOptimized::IndexHNSW2LevelOptimized(
        Index* quantizer,
        size_t nlist,
        int m_pq,
        int M)
        : IndexHNSWOptimized(new Index2Layer(quantizer, nlist, m_pq), M) {
    own_fields = true;
    is_trained = false;
}

IndexHNSW2LevelOptimized::IndexHNSW2LevelOptimized() = default;
...
void IndexHNSW2LevelOptimized::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
    ...
    if (dynamic_cast<const Index2Layer*>(storage)) {
        IndexHNSWOptimized::search(n, x, k, distances, labels);
...
void IndexHNSW2LevelOptimized::flip_to_ivf() {
