...
def readResultVector(singleValueVec, resultPath):
    ...
    return np.array(thrVec), np.array(lat95Vec), np.array(froErrorVec), np.array(errorBoundRatioVec)
...
def main():
    ...
    commonBase = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/" + scanTag + "2S" + "/"
    )
    figPath = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/" + scanTag + "2s" + "CPP"
    )
    ...
    if len(sys.argv) < 2:
        ...
        reRun = 1
    # skech
    thrAll, lat95All, periodAll, fro, eb = compareMethod(
