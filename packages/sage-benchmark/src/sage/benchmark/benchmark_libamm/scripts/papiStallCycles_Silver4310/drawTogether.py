...
def main():
    ...
    commonBase = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/" + scanTag + "_stalls/"
    )
    figPath = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/" + scanTag + "_stalls"
    )
    methodTags = ["CRS", "MM"]
    resultPaths = ["crs", "mm"]
    ...
    for valueChose in range(len(valueVec)):
        ...
        accuBar.DrawFigure(
            ...
            "",
            "cycles",
            figPath + "/" + scanTag + "_stall_accubar" + str(valueVecDisp[valueChose]),
            allowLegend,
