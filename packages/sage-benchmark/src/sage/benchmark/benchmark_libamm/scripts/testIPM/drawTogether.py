...
from OoOCommon import OPT_FONT_NAME, editConfig, readConfig
...
def readResultVector(singleValueVec, resultPath):
    ...
    instructionVec = []
    for i in singleValueVec:
        elapsedTime, cacheMiss, cacheRefs, froError, errorBoundRatio, ins = readResultSingle(
            i, resultPath
        )
    ...
    return (
        ...
        np.array(cacheMissVec),
        np.array(cacheRefVec),
        np.array(froErrorVec),
        np.array(errorBoundRatioVec),
        np.array(instructionVec),
    )
...
def main():
    ...
    commonBase = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/" + scanTag + "IPM/"
    )
    ...
    elapsedTimeAll, cacheMissAll, periodAll, fro, eb, ins, mem = compareMethod(
    ...
    groupLine.DrawFigureYnormal(
        ...
        0,
        100,
        figPath + "/" + scanTag + "sampling_cpp_ins",
        True,
    )
    groupLine.DrawFigureYnormal(
        periodAll,
        ...
        0,
        100,
        figPath + "/" + scanTag + "sampling_cpp_mem",
        True,
    )
    groupLine.DrawFigureYnormal(
        periodAll,
        ...
        0,
        100,
        figPath + "/" + scanTag + "sampling_cpp_ipm",
        True,
    )
    ...
    groupLine.DrawFigureYnormal(
        ...
        0,
        100,
        figPath + "/" + scanTag + "nocofd_cpp_ins",
        True,
    )
    groupLine.DrawFigureYnormal(
        periodAll,
        ...
        0,
        100,
        figPath + "/" + scanTag + "nocdfd_cpp_mem",
        True,
    )
    groupLine.DrawFigureYnormal(
        periodAll,
        ...
        0,
        100,
        figPath + "/" + scanTag + "nocofd_cpp_ipm",
        True,
    )
