...
def runPeriod(exePath, algoTag, resultPath, configTemplate="config.csv", prefixTag="null"):
    ...
    if algoTag == "vq":
        ...
        pqvqCodewordLookUpTablePath = glob.glob(
            f"{pqvqCodewordLookUpTableDir}/zeroMasked_{prefixTag}_m1*"
        )[0]
    elif algoTag == "pq":
        pqvqCodewordLookUpTablePath = glob.glob(
            f"{pqvqCodewordLookUpTableDir}/zeroMasked_{prefixTag}_m10*"
        )[0]
    editConfig(
        exePath + "temp1.csv",
        exePath + configFname,
...
def main():
    ...
    commonBasePath = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/scanZeroMaskACycles/"
    )
    ...
    groupLine.DrawFigureYLog(
        ...
        elapsedTimeAll,
        methodTags,
        "Propotion of left-top NNZ rows and columns (%)",
        "95% latency (ms)",
        0,
        1,
        figPath + "/" + "nnz_lat",
