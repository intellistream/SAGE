...
def DrawLegend(legend_labels, filename):
    ...
    for i in range(len(FIGURE_LABEL)):
        bars[i] = ax1.bar(
            x_values, data, width, hatch=PATTERNS[i], color=LINE_COLORS[i], linewidth=0.2
        )
...
def ReadFile(id):
    ...
    for i in range(id, bound, 1):
        cnt = 0
        print(i)
        f = open(exp_dir + f"/results/breakdown/PMJ_JBCR_NP_{i}.txt")
        read = f.readlines()
        others = 0
        for x in read:
...
def DrawPercentageFigure(
    x_values, y_values, legend_labels, x_label, y_label, filename, allow_legend, title
):
    # you may change the figure size on your own.
