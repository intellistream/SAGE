...
def runPeriod(
    exePath, srcA, srcB, algoTag, resultPath, configTemplate="config.csv", prefixTag="null"
):
    ...
    editConfig(
        ...
        exePath + "temp1.csv",
        "sketchDimension",
        int(dataset_acols_mapping[prefixTag] * 0.1),
    )
    ...
    if algoTag == "vq":
        ...
        pqvqCodewordLookUpTablePath = glob.glob(f"{pqvqCodewordLookUpTableDir}/{prefixTag}_m10_*")[
            0
        ]
    editConfig(
        exePath + "temp1.csv",
        exePath + configFname,
        "pqvqCodewordLookUpTablePath",
        pqvqCodewordLookUpTablePath,
    )
    ...
    # run
    os.system(
        "export OMP_NUM_THREADS=1 &&" + "cd " + exePath + "&& sudo ./benchmark " + configFname
    )
    # copy result
...
def runPeriodVector(
    exePath, periodVec, pS, algoTag, resultPath, prefixTag, configTemplate="config.csv", reRun=1
):
...
def readResultVector(singleValueVec, resultPath):
    ...
    for i in singleValueVec:
        elapsedTime, memLoad, memStore, instructions, fpVector, fpScalar, branchIns = (
            readResultSingle(i, resultPath)
        )
    ...
    return (
        np.array(elapseTimeVec),
        np.array(memLoadVec),
        np.array(memStoreVec),
        np.array(instructionsVec),
        np.array(fpVectorVec),
        np.array(fpScalarVec),
        np.array(branchVec),
    )
...
def compareMethod(
    ...
    for i in range(len(algos)):
        ...
        if reRun == 1:
            ...
            runPeriodVector(
                exeSpace, srcAVec, srcBVec, algoTag, resultPath, dataSetName, csvTemplate
            )
        else:
            if reRun == 2:
        ...
        if resultIsComplete:
            elapsedTime, memLoad, memStore, instructions, fpVector, fpScalar, branchIns = (
                readResultVector(dataSetName, resultPath)
            )
    ...
    return (
        np.array(elapsedTimeAll),
        np.array(memLoadAll),
        np.array(periodAll),
        np.array(instructionsAll),
        np.array(memStoreAll),
        np.array(fpVectorAll),
        np.array(fpScalarAll),
        np.array(branchAll),
    )
...
def main():
    ...
    commonBasePath = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/instructions_breakdown_1/"
    )

    figPath = (
        os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/instructions_breakdown_1/"
    )
    ...
    srcAVec = [
        "datasets/ECO/wm2.mtx",
        "datasets/DWAVE/dwa512.mtx",
        "datasets/AST/mcfe.mtx",
        "datasets/UTM/utm1700a.mtx",
        "datasets/RDB/rdb2048.mtx",
        "datasets/ZENIOS/zenios.mtx",
        "datasets/QCD/qcda_small.mtx",
        "datasets/BUS/gemat1.mtx",
    ]
    srcBVec = [
        "datasets/ECO/wm3.mtx",
        "datasets/DWAVE/dwb512.mtx",
        "datasets/AST/mcfe.mtx",
        "datasets/UTM/utm1700b.mtx",
        "datasets/RDB/rdb2048l.mtx",
        "datasets/ZENIOS/zenios.mtx",
        "datasets/QCD/qcdb_small.mtx",
        "datasets/BUS/gemat1.mtx",
    ]
    dataSetNames = ["ECO", "DWAVE", "AST", "UTM", "RDB", "ZENIOS", "QCD", "BUS"]
    ...
    for valueChose in range(len(valueVec)):
        ...
        accuBar.DrawFigure(
            ...
            ["Mem-load", "Mem-store", "Math-V", "Math-S", "Flow-B", "Flow-O"],
            "",
            "Propotion  (%)",
            figPath + "/" + "insbreakDown" + "_ins_accubar" + str(valueVec[valueChose]),
            True,
        ...
        if str(valueVec[valueChose]) == "BUS":
            ...
            #
            flowTemp = getCyclesPerMethod(branchAll, valueChose) + getCyclesPerMethod(
                otherIns, valueChose
            )
            prop5 = flowTemp / flowTemp[-1]
            groupBar2.DrawFigureYLog2(
                methodTags,
                [prop1, prop3, prop2, prop4, prop5],
                ["Total", "Mem-load", "Math", "Mem-store", "Flow"],
                "",
                r"Instructions ($\times LTMM$)",
                5,
                15,
                figPath + "/" + "bus_ins_times_ltmm",
                True,
            )
    ...
    groupBar2.DrawFigureYLog(
        ...
        methodTags,
        "Datasets",
        "FP Ins (times of LTMM)",
        5,
        15,
        figPath + "/" + "FP_instructions",
        True,
    )
    groupBar2.DrawFigureYLog(
        dataSetNames,
        memInsAll / memInsAll[-1],
        methodTags,
        "Datasets",
        "Mem Ins (times of LTMM)",
        5,
        15,
