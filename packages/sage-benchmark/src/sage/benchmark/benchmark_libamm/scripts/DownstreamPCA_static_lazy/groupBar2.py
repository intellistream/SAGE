...
def DrawFigure(
    ...
    filename,
    allow_legend,
    drawMaxDashedGreyLine=False,
    markCrossOn0andNegativeInf=False,
):
    ...
    for i in range(len(y_values)):
        ...
        if markCrossOn0andNegativeInf:
            for j, y in enumerate(y_values[i]):
                if y == 0 or y == -np.inf:
                    plt.plot(
                        index[j] + i * width + width / 2, 4, marker="x", color="red", markersize=10
                    )
...
def ReadFile():
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/NPJ_{id}.txt"
        f = open(file)
        read = f.readlines()
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/PRJ_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get the 99th timestamp
        col2.append(x)
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/MWAY_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get the 99th timestamp
        col3.append(x)
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/MPASS_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get the 99th timestamp
        col4.append(x)
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/SHJ_JM_NP_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get last timestamp
        col5.append(x)
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/SHJ_JBCR_NP_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get last timestamp
        col6.append(x)
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/PMJ_JM_NP_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get last timestamp
        col7.append(x)
    ...
    for id in it.chain(range(38, 42)):
        file = exp_dir + f"/results/latency/PMJ_JBCR_NP_{id}.txt"
        f = open(file)
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).strip("\n"))  # get last timestamp
        col8.append(x)
