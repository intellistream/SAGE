#!/bin/bash
# 运行 TiM Locomo PreRetrieval 实验 - keyword_extract
# 使用方法: bash script/query_formulation_strategy/run_tim_locomo_keyword_extract.sh

set -e  # 遇到错误立即退出

# 获取脚本所在目录的绝对路径
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 获取项目根目录 (从 script/query_formulation_strategy/ 向上 8 层到 SAGE/)
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../../../../.." && pwd)"
# Python 脚本的相对路径
PYTHON_SCRIPT="$SCRIPT_DIR/../../memory_test_pipeline.py"
# 配置文件路径
CONFIG_FILE="$SCRIPT_DIR/../../config/query_formulation_strategy/TiM_locomo_keyword_extract_pre_retrieval_pipeline.yaml"
...
echo "========================================================================"
echo "🎉 所有任务执行完毕 - TiM-keyword_extract"
