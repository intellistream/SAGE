...
> 目标：在不破坏架构清晰性的前提下，对 PostRetrieval → PreRetrieval → PreInsert → PostInsert → MemoryService
...
- 分层渐进（从易到难）：PostRetrieval（只读）→ PreRetrieval（只读）→ PreInsert（间接改状态）→ PostInsert（直接改状态）→
  MemoryService（状态语义）。
- 明确职责边界：算子不跨权；服务负责最终状态；仿真策略仅在必要时用于解耦。
- 控制组合爆炸：分阶段筛选 Top-K 或采用小型正交阵（L9/L12）。
...
### 2.1 PostRetrieval（不破坏记忆状态，可多次调用）
...
### 2.2 PreRetrieval（只改查询，不改状态）

- 目的：塑形查询形式与 hint。
- 候选水平：
  - `none`：直接问题。
  - `optimize.rewrite`：LLM 改写。
  - `optimize.expand_keywords`：关键词/同义扩展（利于 KV/KG/Hybrid）。
  - `optimize.embed_only`：仅向量检索路径。
  - `hybrid_hints`：输出 `retrieve_mode/retrieve_params`（由服务/适配器决定是否使用）。
- 核心配置键：
  - `operators.pre_retrieval.action`
  - `operators.pre_retrieval.optimize_type`
  - 产出：`question`, `query_embedding`, `retrieve_mode`, `retrieve_params`

### 2.3 PreInsert（间接改状态：改变插入内容/结构/元数据）
...
### 2.4 PostInsert（直接改状态：CRUD/蒸馏/遗忘/迁移/建边）
...
### 2.5 MemoryService（数据结构，决定状态与检索语义）
...
- 候选水平：
  ...
  - `hybrid_memory | vector_hash_memory`：多索引/哈希桶（如可用）。
- 核心配置键：
...
- Phase A（PostRetrieval 扫描，其他固定：`pre_ret=none, pre_ins=none, post_ins=none, service=STM`）

  - 取：`none, rerank.time_weighted, rerank.recency, filter.threshold, merge.simple, rerank.llm(抽样)`
  - 目的：隔离观察“结果加工”的收益/成本（准确率、时延、调用费用）。

- Phase B（在 A 中 Top-2 的 PostRetrieval 上，引入 PreRetrieval）

  - 取：`none, optimize.rewrite, optimize.expand_keywords, optimize.embed_only`（或 `hybrid_hints`）
  - 目的：查询塑形对固定存储语义的影响。

- Phase C（在 A+B 优胜组合上，引入 PreInsert）

  - 取：`none, transform.summarize, transform.chunking, transform.segment, extract.triple, score.importance`
  - 目的：插入内容形态对检索与 QA 的影响。

- Phase D（在 A+B+C 优胜组合上，引入 PostInsert，直接改状态）

  - 取：`none, forgetting, distillation, crud, migrate, link_evolution（非图用仿真）`
  - 目的：服务侧维护/增强策略对质量与时延的影响。

- Phase E（替换 MemoryService 并复跑“优胜子集”）

  - 路径：`STM → Hierarchical → Graph → Hybrid`（若可用）；对耦合点使用仿真以保持可比。

建议每阶段采用 Top-K 保留或小型正交阵（如 L9/L12）以控制组合规模。

______________________________________________________________________

## 5. 命名、配置映射与运行

- 命名约定（用于 `runtime.memory_name` 与落盘目录）：

  - `MS-{service}__PI-{pre_insert}__POI-{post_insert}__PR-{pre_retrieval}__POR-{post_retrieval}`
  - 例：`MS-stm__PI-summarize__POI-forgetting__PR-rewrite__POR-timew`

- 关键配置键（YAML）：

  - 服务：`services.register_memory_service` + `{service_name}.*`
  - PreInsert：`operators.pre_insert.action` + `transform_type | extract_type`
  - PostInsert：`operators.post_insert.action` + 动作专属参数
  - PreRetrieval：`operators.pre_retrieval.action` + `optimize_type`（可输出 `retrieve_*`）
  - PostRetrieval：`operators.post_retrieval.action` +
    `rerank_type | threshold | conversation_format_prompt | top_k | time_decay_rate`

- 参考 YAML（primitive memory models）：

  - STM 基线：
    [packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_short_term_memory_pipeline.yaml](packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_short_term_memory_pipeline.yaml)
  - MemoryBank（三层+遗忘）：[packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_memorybank_pipeline.yaml](packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_memorybank_pipeline.yaml)
  - HippoRAG（图+建边）：[packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_hipporag_pipeline.yaml](packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_hipporag_pipeline.yaml)
  - MemGPT（Agent/工具）：[packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_memgpt_pipeline.yaml](packages/sage-benchmark/src/sage/benchmark/benchmark_memory/experiment/config/primitive_memory_model/locomo_memgpt_pipeline.yaml)
