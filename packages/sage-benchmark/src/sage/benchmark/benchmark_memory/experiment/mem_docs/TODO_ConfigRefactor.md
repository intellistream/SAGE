...
2. **配置即文档**: YAML 配置文件本身成为功能说明文档
3. **快速失败**: 程序启动时检测配置完整性，缺少必需配置立即报错退出
...
2. **删除所有默认值**: `config.get("key", default)` → `config.get("key")` + 校验
3. **添加缺失配置报错**: 配置缺失时抛出明确错误，说明需要的配置键
4. **统一配置路径**: 规范化配置键的命名，使用层级结构
...
2. **快速失败**: 缺少任何必需配置时，程序启动即报错，错误信息包含缺失的配置键
3. **配置完整**: 提供至少 4 个完整可运行的预设配置文件
4. **文档完善**: 每个配置项都有明确的说明和示例

---

## 🔗 相关文件

- 需修改: `libs/pre_insert.py`, `libs/post_insert.py`, `libs/pre_retrieval.py`, `libs/post_retrieval.py`
