...
3. **打印当前任务信息**：输出统计信息（会话数、对话数、数据包数等）
4. **初始化任务指针**：设置遍历所需的指针变量
...
1. 添加 10ms 延迟（背压控制）
2. 检查是否遍历完所有数据，是则返回 `None` 触发停止
3. 获取当前会话信息
4. 检查当前会话是否遍历完，若是则移动到下一个会话
5. 从数据加载器读取当前对话
6. 构造输出数据包（统一格式）
7. 移动指针到下一组对话
8. 返回数据包
...
2. **统一的执行流程**：所有数据集使用相同的 `execute()` 方法，确保行为一致
3. **统一的输出格式**：所有数据集返回相同结构的数据包，便于下游处理
4. **简单的扩展方式**：添加新数据集只需创建 DataLoader 并注册，无需修改核心逻辑

## 注意事项

1. **修改代码时请同步更新本文档**
2. `BatchFunction` 的 `execute()` 会被循环调用，返回 `None` 时表示数据源耗尽
3. `dialog_ptr` 每次递增 2，对应一组问答对
4. 数据包序号 `packet_idx` 从 0 开始，用于进度跟踪
5. 背压延迟时间可根据实际性能需求调整
