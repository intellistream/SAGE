...
本维度负责**记忆检索后的结果整合**，是用户最终看到的记忆上下文的关键处理环节。你需要实现 6 种新的 action：重排序、筛选、多源融合、结果增强、内容压缩、格式化输出。其中 `rerank` 和 `merge` 是核心功能，直接影响最终生成质量。
...
> **配置键**: `operators.post_retrieval.action`
> **职责**: 记忆检索后的结果整合（重排序、筛选、融合、摘要）
