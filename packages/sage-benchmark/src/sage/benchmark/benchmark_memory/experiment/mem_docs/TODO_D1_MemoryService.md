...
本维度负责**记忆数据的底层存储结构**，是整个记忆系统的基础。你需要实现 4 种新的存储后端：图存储、分层存储、混合存储、键值存储。每种存储都需要提供统一的 `insert(entry, vector, metadata)` 和 `retrieve(query, vector, metadata)` 接口，以便与上层算子无缝对接。
...
> **代码位置**: `services/` 目录
> **配置键**: `services.register_memory_service`
> **职责**: 记忆数据的底层存储与检索
