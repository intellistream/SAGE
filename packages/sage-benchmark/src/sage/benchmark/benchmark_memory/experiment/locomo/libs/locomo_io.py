...
class LocomoSource(BatchFunction):
    ...
    def __init__(self, sample_id):
        ...
        for idx, (session_id, max_dialog_idx) in enumerate(self.turns):
            ...
            print(f"   - 会话 {idx+1} (session_id={session_id}): {dialog_count} 个对话 (max_dialog_idx={max_dialog_idx})")
    ...
    def execute(self):
        ...
        if self.dialog_ptr > max_dialog_idx:
            # 移动到下一个session
            print(f"➡️  会话 {session_id} 已完成 (dialog_ptr={self.dialog_ptr} > max={max_dialog_idx})，移动到下一个会话")
            self.session_idx += 1
            self.dialog_ptr = 0
        ...
        # 打印当前执行信息
        print(f"📊 LocomoSource.execute()：session_idx={self.session_idx}/{len(self.turns)}, session={session_id}, dialog={self.dialog_ptr}, max={max_dialog_idx}")
