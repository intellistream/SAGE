...
class F1Score(BaseMetric):
    ...
    def __init__(self):
        super().__init__(name="F1", description="F1 Score - 精确率和召回率的调和平均数")
    ...
    def compute_single_question(
        ...
        pred_str = (
            str(predicted_answer) if not isinstance(predicted_answer, str) else predicted_answer
        )
        ref_str = (
            str(reference_answer) if not isinstance(reference_answer, str) else reference_answer
        )
