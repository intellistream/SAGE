...
export class JobInformationComponent implements OnInit, OnDestroy {
  ...
  runtimeDuration = 0;
  listenIntervalId!: ReturnType<typeof setInterval>;     // interval id for listening to the performance data
...
private startQAPolling() {
 ...
 this.startQAPolling();
  ...
  drawTpgGraph() {
    ...
    this.nodesSelections.on('mouseover', (event: any, d: any) => {
      tooltip.transition()
          .duration(200)
          .style('opacity', 0.9);
  ...
  tpgZoomed({transform}: {transform: any}) {
