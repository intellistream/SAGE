...
class ProjectStatusChecker:
    ...
    def _check_dependencies(self) -> Dict[str, Any]:
        ...
        # 检查关键依赖包
        critical_deps = [
            "typer", "rich", "click", "pydantic", "pathlib",
            "tomli",  # TOML解析库
            "numpy", "pandas"  # 数据处理库（可选）
        ]
    ...
    def _check_configuration(self) -> Dict[str, Any]:
        ...
        if sage_home:
            ...
            config_info["sage_home_status"] = {
                ...
                "logs_dir_exists": (sage_home_path / "logs").exists() if sage_home_path.exists() else False
            }
        else:
    ...
    def _format_result_for_display(self, result: Dict[str, Any]) -> str:
        ...
        if isinstance(result, dict):
            ...
            # 特殊处理服务信息
            if "ray" in result:
                lines.append(f"⚡ Ray: {'✅ 运行中' if result['ray'].get('running') else '❌ 未运行'}")
                lines.append(f"🔧 JobManager: {'✅ 可用' if result['jobmanager'].get('available') else '❌ 不可用'}")
                return "\n".join(lines)
            ...
            if "python_version" in result:
                ...
                lines.append(f"🏠 SAGE_HOME: {'✅ 已设置' if sage_home != 'Not set' else '❌ 未设置'}")
                return "\n".join(lines)
            ...
            if "config_files" in result:
                ...
                if sage_home_status.get("configured", True):
                    lines.append(f"🏠 SAGE_HOME: {'✅ 配置正确' if sage_home_status.get('exists') else '❌ 路径不存在'}")
                else:
    ...
    def generate_status_summary(self, status_data: Dict[str, Any]) -> str:
        ...
        successful_checks = sum(1 for check in status_data["checks"].values() if check["status"] == "success")
