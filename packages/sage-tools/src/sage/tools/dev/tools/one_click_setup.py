...
class OneClickSetupTester:
        ...
        try:
            ...
            if not kwargs.get("test_only", False):
                ...
                if setup_result["status"] != "success":
                    return self._build_failure_result(
                        phases, start_time, "Setup failed"
                    )
            ...
            if not kwargs.get("test_only", False):
                ...
                if install_result["status"] != "success":
                    return self._build_failure_result(
                        phases, start_time, "Installation failed"
                    )
    ...
    def _install_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if requirements_dev.exists():
                ...
                console_print(
                    "⚠️  requirements-dev.txt not found, using legacy package installation..."
                )
                ...
                if packages_dir.exists():
                    for package_path in packages_dir.iterdir():
                        if (
                            package_path.is_dir()
                            and (package_path / "pyproject.toml").exists()
                        ):
                            result = subprocess.run(
                                [
                                    sys.executable,
            ...
            return {
                ...
                "method": (
                    "requirements-dev.txt" if requirements_dev.exists() else "legacy"
                ),
    ...
    def _run_tests(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if kwargs.get("quick_test", False):
                cmd.extend(
                    ["-x", "--tb=short"]
                )  # Stop at first failure, short traceback
    ...
    def _build_summary(self, phases: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "phases_successful": sum(
                1 for p in phases.values() if p.get("status") == "success"
            ),
        }
    ...
    def _build_failure_result(
        self, phases: Dict[str, Any], start_time: float, error: str
    ) -> Dict[str, Any]:
