...
class SAGEHomeManager:
    ...
    def _create_sage_config(self) -> None:
        ...
        if config_file.exists():
            try:
                ...
                # Update project info
                existing_config["projects"][self.project_root.name] = config[
                    "projects"
                ][self.project_root.name]
    ...
    def cleanup_old_files(self, days_old: int = 30) -> Dict[str, int]:
        ...
        try:
            cleanup_stats = {"files_removed": 0, "dirs_removed": 0, "space_freed_mb": 0}
    ...
    def repair_symlinks(self) -> Dict[str, str]:
        ...
        try:
            ...
            return {"repaired_symlinks": repair_results["created"], "status": "success"}
        except Exception as e:
    ...
    def remove_sage_integration(self) -> Dict[str, List[str]]:
        ...
        try:
            results = {"removed_symlinks": [], "moved_back_dirs": [], "warnings": []}
            ...
            for name in symlinks_to_remove:
                repo_path = self.project_root / name
                sage_path = self.project_dirs.get(
                    name.replace(".sage_", ""),
                    self.project_dirs.get(
                        name.replace("_reports", "")
                        .replace("_logs", "logs")
                        .replace("_results", "test_results")
                    ),
                )
