...
def start(
    ...
    reload: bool = typer.Option(
        False, "--reload", "-r", help="开启自动重载 (开发模式)"
    ),
):
    """启动 Web UI 服务"""
    ...
    try:
        # 动态导入以避免硬依赖
        from sage.tools.frontend.web_ui.app import start_server

        start_server(host=host, port=port, reload=reload)
    except ImportError as e:
        console.print(f"[red]❌ 无法启动 Web UI: {e}[/red]")
        console.print(
            "[yellow]💡 请确保已安装 frontend 依赖: pip install isage[frontend][/yellow]"
        )
        raise typer.Exit(1)
...
def status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            console.print(
                f"[yellow]⚠️ 服务响应异常: HTTP {response.status_code}[/yellow]"
            )
