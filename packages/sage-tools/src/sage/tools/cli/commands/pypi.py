...
def check(
    ...
    for wheel_file in wheel_files:
        ...
        console.print(f"  📦 {wheel_file.name} ({file_size:.1f}MB, {file_time})")
        
        if verbose:
            # 显示wheel内容概览
            try:
                result = subprocess.run(
                    [sys.executable, "-m", "wheel", "unpack", "--dest", "/tmp", str(wheel_file)],
                    capture_output=True,
                    text=True
                )
        ...
        if result.returncode == 0:
            ...
            if dist_dir.exists():
                ...
                if wheel_files:
                    ...
                    for wheel_file in wheel_files:
                        file_size = wheel_file.stat().st_size / 1024 / 1024  # MB
                        console.print(f"  • {wheel_file.name} ({file_size:.1f}MB)")
...
def publish(
    ...
    packages: Optional[List[str]] = typer.Option(None, "--package", help="指定要发布的包"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="显示详细输出"),
):
    """发布SAGE包到PyPI
...
def _check_publish_dependencies() -> bool:
    ...
    # 检查build
    try:
        result = subprocess.run([sys.executable, "-m", "build", "--help"], capture_output=True, text=True)
        if result.returncode != 0:
...
class PyPIPublisher:
    ...
    def publish_packages(self, specified_packages: Optional[List[str]] = None, skip_build: bool = False) -> bool:
        """发布包"""
        packages_dir = self.project_root / "packages"
        ...
        # 记录摘要到日志
        self.log_to_file(f"发布摘要: 成功={success_count}, 跳过={skipped_count}, 失败={failed_count}")
