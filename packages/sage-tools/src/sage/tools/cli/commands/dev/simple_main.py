...
try:
    ...
    console.self.logger.info(f"[yellow]警告: Issues管理功能不可用: {e}[/yellow]")
...
try:
    ...
    console.self.logger.info(f"[yellow]警告: PyPI发布管理功能不可用: {e}[/yellow]")
...
try:
    ...
    console.self.logger.info(f"[yellow]警告: 版本管理功能不可用: {e}[/yellow]")
...
def quality(
    ...
    if not project_path.exists():
        console.self.logger.info(f"[red]❌ 项目根目录不存在: {project_path}[/red]")
        raise typer.Exit(1)

    console.self.logger.info(f"📁 项目根目录: {project_path}")
    ...
    console.self.logger.info(f"🎯 检查目录: {', '.join(target_paths)}")
    ...
    if format_code:
        ...
        if should_fix:
            ...
            if result.returncode == 0:
                console.self.logger.info("[green]✅ 代码格式化完成[/green]")
                if result.stdout.strip():
                    console.self.logger.info(result.stdout)
            else:
                console.self.logger.info(f"[red]❌ 代码格式化失败: {result.stderr}[/red]")
                quality_issues = True
        else:
            # 检查模式
            cmd = (
            ...
            if result.returncode != 0:
                console.self.logger.info("[yellow]⚠️ 发现代码格式问题[/yellow]")
                if check_only and result.stdout.strip():
                    console.self.logger.info(result.stdout)
                quality_issues = True
            else:
                console.self.logger.info("[green]✅ 代码格式检查通过[/green]")
    ...
    if sort_imports:
        ...
        if should_fix:
            ...
            if result.returncode == 0:
                console.self.logger.info("[green]✅ 导入排序完成[/green]")
                if result.stdout.strip():
                    console.self.logger.info(result.stdout)
            else:
                console.self.logger.info(f"[red]❌ 导入排序失败: {result.stderr}[/red]")
                quality_issues = True
        else:
            # 检查模式
            cmd = (
            ...
            if result.returncode != 0:
                console.self.logger.info("[yellow]⚠️ 发现导入排序问题[/yellow]")
                if check_only and result.stdout.strip():
                    console.self.logger.info(result.stdout)
                quality_issues = True
            else:
                console.self.logger.info("[green]✅ 导入排序检查通过[/green]")
    ...
    if lint_code:
        ...
        try:
            ...
            if result.returncode != 0:
                console.self.logger.info("[yellow]⚠️ 发现代码质量问题[/yellow]")
                console.self.logger.info(result.stdout)
                quality_issues = True
            else:
                console.self.logger.info("[green]✅ 代码质量检查通过[/green]")
        except FileNotFoundError:
            console.self.logger.info("[yellow]⚠️ flake8 未安装，跳过代码质量检查[/yellow]")
            console.self.logger.info("[yellow]💡 建议安装: pip install flake8[/yellow]")
        except Exception as e:
            console.self.logger.info(f"[yellow]⚠️ flake8 检查失败: {e}[/yellow]")
    ...
    if quality_issues:
        ...
        console.self.logger.info("[green]✅ 所有代码质量检查通过[/green]")
...
def _run_quality_check(
    ...
    if not quiet:
        console.self.logger.info(f"🎯 检查目录: {', '.join(str(p) for p in target_paths)}")
    ...
    if format_code:
        if not quiet:
            console.self.logger.info("🎨 运行代码格式化检查 (black)...")
    ...
    if sort_imports:
        if not quiet:
            console.self.logger.info("📦 运行导入排序检查 (isort)...")
    ...
    if lint_code:
        if not quiet:
            console.self.logger.info("🔍 运行代码检查 (flake8)...")
    ...
    if quality_issues:
        if not quiet:
            if fix:
                ...
                console.self.logger.info("[yellow]💡 建议运行: sage-dev quality --fix[/yellow]")
            else:
                console.self.logger.info(
...
def analyze(
    ...
    try:
        ...
        if analysis_type == "all":
            ...
            console.self.logger.info(f"[red]不支持的分析类型: {analysis_type}[/red]")
            console.self.logger.info("支持的类型: all, health, report")
            raise typer.Exit(1)
        ...
        if output_format == "json":
            ...
            console.self.logger.info(json.dumps(serializable_result, indent=2, ensure_ascii=False))
        elif output_format == "markdown":
            # Markdown格式输出
            markdown_output = _generate_markdown_output(result, analysis_type)
            console.self.logger.info(markdown_output)
        else:
            # 简要输出
            if isinstance(result, dict):
                console.self.logger.info("📊 分析结果:")
                if "summary" in result:
                    summary = result["summary"]
                    console.self.logger.info(f"  📦 总包数: {summary.get('total_packages', 0)}")
                    console.self.logger.info(
                    ...
                    console.self.logger.info(f"  💯 健康评分: {result.get('health_score', 'N/A')}")
                    console.self.logger.info(f"  📊 等级: {result.get('grade', 'N/A')}")
                else:
                    console.self.logger.info("  📋 分析完成")
            console.self.logger.info("[green]✅ 分析完成[/green]")

    except Exception as e:
        console.self.logger.info(f"[red]分析失败: {e}[/red]")
        import traceback

        console.self.logger.info(f"[red]详细错误:\n{traceback.format_exc()}[/red]")
        raise typer.Exit(1)
...
def clean(
    ...
    try:
        ...
        if dry_run:
            console.self.logger.info("[yellow]预览模式 - 不会实际删除文件[/yellow]")
        ...
        if target == "all":
            ...
            console.self.logger.info(f"[red]不支持的清理目标: {target}[/red]")
            console.self.logger.info("支持的目标: all, cache, build, logs")
            raise typer.Exit(1)
        ...
        if cleaned_items:
            ...
            for item in cleaned_items[:10]:  # 限制显示数量
                console.self.logger.info(f"  📁 {item}")
            if len(cleaned_items) > 10:
                console.self.logger.info(f"  ... 还有 {len(cleaned_items) - 10} 个项目")
        else:
            console.self.logger.info("[blue]没有找到需要清理的项目[/blue]")

        console.self.logger.info("[green]✅ 清理完成[/green]")

    except Exception as e:
        console.self.logger.info(f"[red]清理失败: {e}[/red]")
        import traceback
...
def status(
    ...
    try:
        ...
        if output_format == "json":
            ...
            console.self.logger.info(json.dumps(status_data, indent=2, ensure_ascii=False))
        elif output_format == "full":
            # 完整详细输出
            status_data = checker.check_all(verbose=True)
            console.self.logger.info("\n" + "=" * 60)
            console.self.logger.info(checker.generate_status_summary(status_data))
            ...
            # 简要摘要输出 (默认)
            console.self.logger.info("🔍 检查项目状态...")
            status_data = checker.check_all(verbose=False)
            ...
            if issues:
                ...
                for issue in issues[:5]:  # 限制显示数量
                    console.self.logger.info(f"  {issue}")

            if failed_checks:
                console.self.logger.info(f"\n❌ 失败的检查项目: {', '.join(failed_checks)}")
                console.self.logger.info("💡 使用 --output-format full 查看详细信息")
            elif not issues:
        ...
        console.self.logger.info(f"[red]状态检查失败: {e}[/red]")
        if verbose:
            import traceback

            console.self.logger.info(f"[red]详细错误信息:\n{traceback.format_exc()}[/red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        if not (project_path / "packages").exists():
            ...
            if not found_root:
                ...
                if not found_root:
                    console.self.logger.info("[red]❌ 无法找到 SAGE 项目根目录[/red]")
                    console.self.logger.info(f"当前目录: {Path.cwd()}")
                    console.self.logger.info(f"指定目录: {project_root}")
                    console.self.logger.info(
        ...
        if not skip_quality_check:
            ...
            if has_quality_issues and not quiet:
                console.self.logger.info("[yellow]⚠️ 发现代码质量问题，但继续运行测试[/yellow]")
            elif not quiet:
                console.self.logger.info("[green]✅ 代码质量检查通过，继续运行测试[/green]")
        elif not quiet:
            console.self.logger.info("[yellow]⚠️ 跳过代码质量检查[/yellow]")

        # 诊断模式
        if diagnose:
            console.self.logger.info("🔍 运行诊断模式...")
            _run_diagnose_mode(str(project_path))
            return

        # Issues Manager 测试
        if issues_manager:
            console.self.logger.info("🔧 运行 Issues Manager 测试...")
        ...
        if packages:
            ...
            console.self.logger.info(f"🎯 指定测试包: {target_packages}")
        ...
        if not quiet:
            console.self.logger.info(f"🧪 运行 {test_type} 测试...")
            console.self.logger.info(
        ...
        if test_type == "quick":
            ...
            console.self.logger.info(f"[red]不支持的测试类型: {test_type}[/red]")
            console.self.logger.info("支持的类型: all, unit, integration, quick")
            raise typer.Exit(1)
        ...
        if result and result.get("status") == "success":
            ...
            if not quiet:
                console.self.logger.info("[red]❌ 测试失败[/red]")
            raise typer.Exit(1)
...
def home(
    ...
    try:
        ...
        if action == "init":
            ...
            console.self.logger.info("[green]✅ SAGE目录初始化完成[/green]")
            console.self.logger.info(f"  📁 SAGE目录: {sage_paths.sage_dir}")
            console.self.logger.info(f"  📊 项目根目录: {sage_paths.project_root}")
            console.self.logger.info(
            ...
            if not logs_dir.exists():
                console.self.logger.info("[yellow]⚠️ 日志目录不存在[/yellow]")
                return
            ...
            console.self.logger.info("🏠 SAGE目录状态:")
            console.self.logger.info(f"  📁 SAGE目录: {sage_paths.sage_dir}")
            console.self.logger.info(
            ...
            for name, path in subdirs:
                ...
                if path.exists():
                    ...
                    console.self.logger.info(f"  � {name}: {status}")

        else:
            console.self.logger.info(f"[red]不支持的操作: {action}[/red]")
            console.self.logger.info("支持的操作: init, clean, status")
            raise typer.Exit(1)

    except Exception as e:
        console.self.logger.info(f"[red]SAGE目录操作失败: {e}[/red]")
        import traceback
...
def _run_diagnose_mode(project_root: str):
    ...
    try:
        ...
        console.self.logger.info("🔍 SAGE 完整安装诊断")
        console.self.logger.info("=" * 50)

        # 1. 基础导入测试
        console.self.logger.info("📦 基础导入测试...")
        imports_to_test = [
            "sage",
            "sage.common",
            "sage.kernel",
        ...
        for module in imports_to_test:
            try:
                ...
                console.self.logger.info(f"  ✅ {module} (版本: {version})")
            except ImportError as e:
                import_results[module] = {"status": "failed", "error": str(e)}
                console.self.logger.info(f"  ❌ {module}: {str(e)}")
            except Exception as e:
        ...
        try:
            ...
            if hasattr(sage, "__path__"):
                console.self.logger.info(f"  ✅ sage 命名空间路径: {sage.__path__}")
                ...
                for finder, name, ispkg in pkgutil.iter_modules(
                    ...
                    if name.split(".")[-1] in [
                        ...
                        console.self.logger.info(f"    📦 发现子包: {name}")
            else:
                console.self.logger.info("  ⚠️  sage 不是命名空间包")
        except Exception as e:
            console.self.logger.info(f"  ❌ 命名空间检查失败: {e}")
        ...
        if packages_dir.exists():
            ...
            for package_dir in packages_dir.iterdir():
                if package_dir.is_dir() and package_dir.name.startswith("sage-"):
                    ...
                    console.self.logger.info(f"  📦 {package_name}")
                    console.self.logger.info(
                        "    ✅ pyproject.toml"
                    ...
                    if structure_info["src"]:
                        console.self.logger.info("    ✅ src/ 目录")
                    if structure_info["tests"]:
                        console.self.logger.info("    ✅ tests/ 目录")
                    else:
                        console.self.logger.info("    ⚠️  tests/ 目录缺失")
        else:
            console.self.logger.info("  ❌ packages 目录不存在")
        ...
        for var in env_vars:
            ...
            if value:
                ...
                console.self.logger.info(f"  ⚠️  {var}: 未设置")
        ...
        for cmd in cli_commands:
            try:
                ...
                if result.returncode == 0:
                    console.self.logger.info(f"  ✅ {cmd} 可用")
                else:
                    console.self.logger.info(f"  ❌ {cmd} 返回错误码: {result.returncode}")
            except subprocess.TimeoutExpired:
                console.self.logger.info(f"  ⚠️  {cmd} 超时")
            except FileNotFoundError:
                console.self.logger.info(f"  ❌ {cmd} 未找到")
            except Exception as e:
                console.self.logger.info(f"  ❌ {cmd} 检查失败: {e}")
        ...
        for dep in key_dependencies:
            try:
                ...
                console.self.logger.info(f"  ✅ {dep} (版本: {version})")
            except ImportError:
                console.self.logger.info(f"  ⚠️  {dep} 未安装")
            except Exception as e:
                console.self.logger.info(f"  ❌ {dep} 检查失败: {e}")
        ...
        console.self.logger.info(f"  📊 导入成功率: {successful_imports}/{total_imports}")

        if successful_imports == total_imports:
            console.self.logger.info("  🎉 SAGE 安装完整，所有模块可正常导入")
        elif successful_imports > 0:
            console.self.logger.info("  ⚠️  SAGE 部分安装，部分模块存在问题")
        else:
            console.self.logger.info("  ❌ SAGE 安装存在严重问题，无法导入核心模块")

        console.self.logger.info("\n✅ 完整诊断完成")

    except Exception as e:
        console.self.logger.info(f"[red]诊断失败: {e}[/red]")
        import traceback
...
def _run_issues_manager_test(project_root: str, verbose: bool):
    ...
    try:
        ...
        if success:
            console.self.logger.info("✅ Issues Manager 测试通过")
        else:
            console.self.logger.info("❌ Issues Manager 测试失败")

    except Exception as e:
        console.self.logger.info(f"[red]Issues Manager 测试失败: {e}[/red]")
...
def _run_quick_tests(runner, config: dict, quiet: bool):
    ...
    if not quiet:
        console.self.logger.info(f"🚀 快速测试模式 - 测试包: {quick_packages}")
...
def _run_all_tests(runner, config: dict, quiet: bool):
    """运行全部测试"""
    if not quiet:
        console.self.logger.info("🧪 全面测试模式")
...
def _run_unit_tests(runner, config: dict, quiet: bool):
    """运行单元测试"""
    if not quiet:
        console.self.logger.info("🔬 单元测试模式")
...
def _run_integration_tests(runner, config: dict, quiet: bool):
    """运行集成测试"""
    if not quiet:
        console.self.logger.info("🔗 集成测试模式")
...
def _generate_test_report(
    ...
    try:
        ...
        console.self.logger.info(f"📊 测试报告已保存到: {report_path}")

    except Exception as e:
        console.self.logger.info(f"[red]生成测试报告失败: {e}[/red]")
...
def _display_test_results(
    ...
    if result:
        status = result.get("status", "unknown")
        if status == "success":
            console.self.logger.info("✅ 状态: 成功")
        else:
            console.self.logger.info("❌ 状态: 失败")

        console.self.logger.info(f"⏱️ 执行时间: {execution_time:.2f}秒")

        # Get summary data from either top level or summary sub-dict
        summary = result.get("summary", result)
        console.self.logger.info(f"📊 总测试数: {summary.get('total', 0)}")
        console.self.logger.info(f"✅ 通过: {summary.get('passed', 0)}")
        console.self.logger.info(f"❌ 失败: {summary.get('failed', 0)}")
        console.self.logger.info(f"💥 错误: {summary.get('errors', 0)}")

        if not summary_only and result.get("failed_tests"):
            console.self.logger.info("\n❌ 失败的测试:")
            for test in result["failed_tests"]:
                console.self.logger.info(f"  - {test}")
    else:
        console.self.logger.info("❓ 无法获取测试结果")
                ...
                try:
                    ...
                    result = subprocess.run(
                        [
                            "python",
                            "-c",
                            f"import {package_name.replace('-', '.')}; self.logger.info(getattr({package_name.replace('-', '.')}, '__version__', 'unknown'))",
                        ],
                        capture_output=True,
                        text=True,
                        timeout=5,
...
def _show_packages_status_summary(project_path):
    ...
    if "error" in data:
        console.self.logger.info(f"[red]❌ {data['error']}[/red]")
        return
    ...
    console.self.logger.info(f"  📊 总包数: {total}")
    console.self.logger.info(f"  ✅ 可导入: {importable}/{total}")
    console.self.logger.info(f"  🧪 有测试: {has_tests}/{total}")
...
def _show_packages_status(
    ...
    """显示详细包状态"""
    console.self.logger.info("📦 SAGE Framework 包状态详情")
    console.self.logger.info("=" * 50)
    ...
    for package_name, info in packages.items():
        ...
        if info.get("has_pyproject"):
            console.self.logger.info("  ✅ pyproject.toml")
        else:
            console.self.logger.info("  ❌ pyproject.toml 缺失")
        ...
        if info.get("import_status") == "success":
            version = info.get("version", "unknown")
            console.self.logger.info(f"  ✅ 导入成功 (版本: {version})")
        else:
            console.self.logger.info("  ❌ 导入失败")
            if verbose and info.get("import_error"):
                console.self.logger.info(f"     错误: {info['import_error']}")

        # 详细版本信息
        if check_versions and verbose:
            console.self.logger.info(f"  📍 路径: {info.get('path', 'unknown')}")
...
def _check_package_dependencies(package_name: str, verbose: bool):
    ...
    try:
        ...
        console.self.logger.info(f"    🔗 检查 {package_name} 依赖...")

        # 这里可以添加更详细的依赖检查逻辑
        # 暂时简化处理
        console.self.logger.info("    ℹ️  依赖检查功能待完善")

    except Exception as e:
        if verbose:
            console.self.logger.info(f"    ❌ 依赖检查失败: {e}")
...
def tools(
    ...
    if command == "help":
        ...
        console.self.logger.info(f"[red]未知命令: {command}[/red]")
        _show_tools_help()


def _show_tools_help():
    """显示工具帮助信息"""
    console.self.logger.info("🧪 [bold blue]SAGE Framework 开发工具[/bold blue]")
    console.self.logger.info()
    console.self.logger.info("[bold]使用方法:[/bold]")
    console.self.logger.info("  sage dev tools <command> [options]")
    console.self.logger.info()
    console.self.logger.info("[bold]命令:[/bold]")
    console.self.logger.info("  [green]test[/green]      运行测试 (quick|all|diagnose)")
    console.self.logger.info("  [green]diagnose[/green]  运行诊断和状态检查")
    console.self.logger.info("  [green]status[/green]    显示包状态")
    console.self.logger.info("  [green]help[/green]      显示此帮助")
    console.self.logger.info()
    console.self.logger.info("[bold]快速命令:[/bold]")
    console.self.logger.info("  sage dev tools test --type quick              # 快速测试")
    console.self.logger.info("  sage dev tools test --type all --verbose      # 测试所有包")
    console.self.logger.info("  sage dev tools diagnose                       # 运行诊断")
    console.self.logger.info("  sage dev tools test --packages sage-libs      # 测试指定包")
    console.self.logger.info()
    console.self.logger.info("[bold]详细用法:[/bold]")
    console.self.logger.info("  sage dev test --help                   # 查看完整测试选项")
    console.self.logger.info("  sage dev status --help                 # 查看状态选项")
    ...
    if test_type == "diagnose":
        ...
        try:
            ...
            if test_type == "quick":
                ...
                console.self.logger.info(f"[red]未知测试类型: {test_type}[/red]")
                return
