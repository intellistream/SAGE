...
from .data import prepare_dataset, load_training_data, format_alpaca_sample
...
class LoRATrainer:
    ...
    def setup_trainer(self):
        ...
        print(f"   • 预计步数: ~{len(self.dataset) // self.config.effective_batch_size * self.config.num_train_epochs}")
        print()
    ...
    def train(self):
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                print("\n❌ 显存不足 (OOM) 错误!")
                print("\n💡 建议:")
                print(f"  1. 减小 max_length (当前 {self.config.max_length} -> {self.config.max_length // 2})")
