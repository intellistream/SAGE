...
class FastPipValidator:
    ...
    def show_progress(self, message: str, duration: float = 0):
        ...
        while time.time() - start_time < duration:
            logging.info(f"\r  {chars[i % len(chars)]} {message}", end="", flush=True)
            time.sleep(0.1)
            i += 1
    ...
    def build_or_find_wheel(self) -> Optional[Path]:
        ...
        try:
            ...
            if not sage_package_dir.exists():
                logging.info(f"  ❌ sage包目录不存在: {sage_package_dir}")
                return None
            ...
            if not self.skip_wheel:
                ...
                if returncode != 0:
                    logging.info(f"  ❌ 构建wheel包失败: {stderr}")
                    return None

            # 查找wheel文件
            if not dist_dir.exists():
                logging.info(f"  ❌ dist目录不存在: {dist_dir}")
                return None

            wheel_files = list(dist_dir.glob("*.whl"))
            if not wheel_files:
                logging.info(f"  ❌ 未找到wheel包文件")
                return None

            wheel_file = wheel_files[0]
            logging.info(f"  ✅ 找到wheel包: {wheel_file.name}")
            self.results["wheel_build"] = True
            return wheel_file

        except Exception as e:
            logging.info(f"  ❌ 处理wheel包失败: {e}")
            return None
    ...
    def install_package(self, wheel_file: Path) -> bool:
        ...
        try:
            logging.info(f"  📦 安装: {wheel_file.name}")
            ...
            install_thread.join()
            logging.info("\r" + " " * 20 + "\r", end="", flush=True)
            ...
            # 快速验证安装
            logging.info(f"  🔍 使用Python路径: {self.python_exe}")
            test_cmd = [
                str(self.python_exe),
                "-c",
                "import sage; logging.info(f'SAGE {sage.__version__} 安装成功')",
            ]
            logging.info(f"  🔍 执行命令: {' '.join(test_cmd)}")
            ...
            logging.info(f"  🔍 返回码: {returncode}")
            logging.info(f"  🔍 标准输出: {stdout}")
            logging.info(f"  🔍 标准错误: {stderr}")
            ...
            if returncode != 0:
                ...
                diag_returncode, diag_stdout, diag_stderr = self.run_command(
                    [
                        str(self.python_exe),
                        "-c",
                        "import sys, os; logging.info(f'工作目录: {os.getcwd()}'); logging.info('Python路径:'); [logging.info(f'  {p}') for p in sys.path]; import sage; logging.info(f'sage文件: {sage.__file__}'); logging.info(f'sage属性: {dir(sage)}')",
                    ],
                    env=clean_env,
                )
                logging.info(f"  🔍 诊断输出: {diag_stdout}")
                if diag_stderr.strip():
                    logging.info(f"  🔍 诊断错误: {diag_stderr}")

                return False

            logging.info(f"  ✅ {stdout.strip()}")
...
"""
        ...
        try:
            ...
            if returncode == 0:
                ...
                logging.info(f"  {stderr.strip()}")
                return False

        except Exception as e:
            logging.info(f"  ❌ 导入测试异常: {e}")
            return False
...
if len(sink.received) == 2:
    logging.info("✅ 数据流测试成功")
    logging.info(f"处理数据: {sink.received}")
else:
    logging.info(f"❌ 数据流测试失败: 预期2条，实际{len(sink.received)}条")
    exit(1)
"""
        ...
        try:
            ...
            if returncode == 0:
                ...
                logging.info(f"  ❌ 功能测试失败: {stderr}")
                return False

        except Exception as e:
            logging.info(f"  ❌ 功能测试异常: {e}")
            return False
        ...
        try:
            ...
            returncode, stdout, stderr = self.run_command(
                [
                    str(self.python_exe),
                    "-c",
                    "import sage; logging.info('✅ SAGE模块CLI支持正常')",
                ],
                timeout=10,
            )
            ...
            if returncode == 0:
                ...
                logging.info(f"  ❌ CLI测试失败: {stderr}")
                return False

        except Exception as e:
            logging.info(f"  ❌ CLI测试异常: {e}")
            return False
    ...
    def run_fast_validation(self) -> bool:
        """运行快速发布准备验证"""
        logging.info("🚀 SAGE PyPI发布准备快速验证")
        logging.info("=" * 50)
        ...
        logging.info(f"⏱️  验证时间: {duration:.1f}秒")
        logging.info(f"📈 完成步骤: {completed_steps}/{len(steps)}")

        if all_passed:
            logging.info("\n🎉 快速发布准备验证通过！")
            logging.info("📦 SAGE核心功能可以正常工作")
            logging.info("🚀 建议运行完整验证确认发布准备")
            return True
        else:
            logging.info("\n⚠️  快速发布准备验证失败")
            logging.info("🔧 建议运行完整验证以获取详细信息")
            return False
    ...
    def cleanup(self):
        ...
        if self.test_dir.exists():
            ...
            try:
                ...
                logging.info(f"⚠️  清理失败: {e}")
                logging.info("💡 请手动删除测试目录")
