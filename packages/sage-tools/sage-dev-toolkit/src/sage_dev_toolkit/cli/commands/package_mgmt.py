...
@app.command("compile")
def compile_command(
    package_path: Optional[str] = typer.Argument(None, help="Path to the package to compile (single package or comma-separated list). If not provided, all packages from project_config.toml will be compiled."),
    ...
    try:
        ...
        # 检查是否提供了package_path，如果没有则使用项目配置中的所有包
        if not package_path or package_path.strip() == "":
            console.print("📦 未指定包路径，将编译项目配置中的所有包...", style="blue")
            package_paths = _get_all_package_paths(project_root_path)
            if not package_paths:
                console.print("❌ 项目配置中没有找到有效的包路径", style="red")
                console.print("💡 使用 --info 可以只显示SAGE home信息", style="yellow")
                raise typer.Exit(1)
            batch_mode = True
            console.print(f"🎯 将编译 {len(package_paths)} 个包", style="bold cyan")
        else:
            # 解析用户提供的包路径
            if "," in package_path:
                # 多个包路径
        ...
        for path in package_paths:
            if not path.is_absolute():
                # 如果是相对路径，尝试相对于项目根目录
                abs_path = project_root_path / path
            else:
                abs_path = path
