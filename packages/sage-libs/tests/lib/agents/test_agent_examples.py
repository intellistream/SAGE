    ...
    def test_main_function_config_not_found(self):
        ...
        with patch('sys.exit') as mock_exit:
            with patch('builtins.print') as mock_print:
                ...
                mock_print.assert_called_with("❌ Configuration file not found: " + 
                    os.path.join(os.path.dirname(agent.__file__), "..", "config", "config_agent_min.yaml"))
                mock_exit.assert_called_once_with(1)    @patch('examples.agents.agent.iter_queries')
    @patch('examples.agents.agent.importlib.import_module')
    @patch('examples.agents.agent.load_config')
    @patch('os.path.exists')
    def test_main_function_successful_execution(self, mock_exists, mock_load_config, mock_import, mock_iter_queries):
            ...
            with patch('examples.agents.agent.OpenAIGenerator') as mock_generator:
                with patch('examples.agents.agent.LLMPlanner') as mock_planner:
                    with patch('examples.agents.agent.MCPRegistry') as mock_registry:
                        with patch('examples.agents.agent.AgentRuntime') as mock_runtime:
                            ...
                            with patch('builtins.print') as mock_print:
                                ...
                                mock_registry_instance.register.assert_called_once_with(mock_tool_instance)
                                ...
                                assert any("🧑‍💻 User: Test query 1" in call for call in print_calls)
                                assert any("🧑‍💻 User: Test query 2" in call for call in print_calls)
...
if __name__ == "__main__":
    if os.getenv("SAGE_EXAMPLES_MODE") == "test" or os.getenv("SAGE_TEST_MODE") == "true":
        ...
        main()
""", {'__name__': '__main__', 'os': os, 'main': agent.main, 'print': print, 'sys': __import__('sys')})
