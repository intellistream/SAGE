...
class MilvusDenseRetriever(MapFunction):
    ...
    def _load_knowledge_from_file_dense(self, file_path: str):
        ...
        try:
            # 使用Milvus后端加载
            success = self.milvus_backend.load_knowledge_from_file_dense(file_path, self.embedding_model)
            if not success:
                self.logger.error(f"Failed to load knowledge from file: {file_path}")
        except Exception as e:
    ...
    def execute(self, data: str) -> Dict[str, Any]:
        ...
        if not isinstance(input_query, str):
            ...
            if is_dict_input:
                ...
                return {"query": str(input_query), "retrieved_documents": [], "input": data}
        ...
        try:
            ...
            if is_dict_input:
                ...
                return {"query": input_query, "retrieved_documents": retrieved_docs, "input": data}
