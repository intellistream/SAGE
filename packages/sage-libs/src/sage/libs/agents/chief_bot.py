...
class ChiefBot(FlatMapFunction):
    ...
    def __init__(self, config: Dict[str, Any] = None, **kwargs):
        ...
        self.logger.info(
            f"ChiefBot initialized with {len(self.available_tools)} available tools"
        )
...
User Query: {{query}}"""
    ...
    def _parse_tool_selection(self, response: str) -> Dict[str, Any]:
        ...
        # 所有解析方法都失败
        self.logger.error(
            f"All parsing methods failed for response: {repr(cleaned_response[:200])}"
        )
        return self._get_fallback_response()
    ...
    def _parse_with_regex(self, response: str) -> Dict[str, Any]:
        ...
        return {"analysis": analysis, "selected_tools": tools}
    ...
    def _create_enhanced_template(
        self, original_template: ModelContext, tool_info: Dict[str, Any], analysis: str
    ) -> ModelContext:
        """为特定工具创建增强的template"""
...
Execute your task and provide a clear, focused result."""
        ...
        enhanced_template.prompts = [
            ...
            {
                "role": "user",
                "content": tool_info.get("sub_query", original_template.raw_question),
            },
        ]
    ...
    def execute(self, template: ModelContext) -> List[ModelContext]:
        ...
        try:
            self.query_count += 1
            self.logger.debug(
                f"ChiefBot processing query #{self.query_count}: {template.raw_question}"
            )
            ...
            try:
                ...
                self.logger.debug(
                    f"LLM response received, length: {len(response)} chars"
                )
            ...
            self.logger.info(
                f"ChiefBot selected {len(filtered_tools)} tools after filtering"
            )
            ...
            for tool_info in filtered_tools:
                ...
                if tool_name not in self.available_tools:
                    self.logger.warning(
                        f"Unknown tool selected: {tool_name}, skipping..."
                    )
                    continue
            ...
            # 6. 如果没有工具，提供直接响应
            if not result_templates and self.tool_selection_config.get(
                "fallback_to_direct_response", True
            ):
    ...
    def get_tool_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_used_tool": (
                max(self.tool_usage_stats.items(), key=lambda x: x[1])[0]
