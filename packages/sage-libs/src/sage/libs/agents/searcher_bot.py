...
class SearcherBot(MapFunction):
    ...
    def __init__(self, config: dict, **kwargs):
        ...
        self.logger.info(
            f"SearcherBot initialized with max_queries: {self.max_queries}"
        )

    def _analyze_information_completeness(
        self, template: ModelContext
    ) -> Dict[str, any]:
        """
        分析信息完整性，判断是否需要额外搜索
        ...
        analysis = {
            "has_question": bool(
                template.raw_question and template.raw_question.strip()
            ),
    ...
    def _update_template_with_search_results(
        self,
        ...
        optimization_metadata = {
            "searcher_bot_run_count": template.get_tool_config(
                "optimization_metadata", {}
            ).get("searcher_bot_run_count", 0)
            + 1,
        ...
        self.logger.debug(
            f"Updated template with {len(search_queries)} new queries, total: {len(all_queries)}"
        )
    ...
    def execute(self, template: ModelContext) -> ModelContext:
        ...
        try:
            ...
            self.logger.debug(
                f"Built optimization prompts with {len(optimization_prompts)} messages"
            )
            ...
            self.logger.info(
                f"SearcherBot generated {len(search_queries)} optimized queries (#{self.query_count})"
            )
...
class EnhancedSearchBot(SearcherBot):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"EnhancedSearchBot initialized with fallback: {fallback_to_original}"
        )
    ...
    def _should_skip_search(self, template: ModelContext) -> bool:
        ...
        # 这里可以设置更复杂的判断逻辑
        return (
            total_content_length > 2000 and len(template.retriver_chunks) >= 3
        ) or run_count >= 3
    ...
    def execute(self, template: ModelContext) -> ModelContext:
        ...
        try:
            ...
            if self._should_skip_search(template):
                self.logger.info(
                    "Sufficient information available or max runs reached, skipping search optimization"
                )
                return template
            ...
            if not enhanced_template.has_search_queries() and self.fallback_to_original:
                self.logger.info(
                    "No optimized queries generated, falling back to original question"
                )
