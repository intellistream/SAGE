...
class AnswerBot(MapFunction):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.logger.info(
            "AnswerBot initialized - ready to process comprehensive context"
        )

    def _build_comprehensive_prompts(
        ...
        prompts = [
            ...
            {"role": "user", "content": f"Question: {template.raw_question}"},
        ]
    ...
    def execute(self, template: ModelContext) -> ModelContext:
        ...
        try:
            ...
            if not self._validate_template(template):
                template.response = (
                    "Error: Invalid input template - missing required raw_question"
                )
                return template
            ...
            self.logger.debug(
                f"Built comprehensive prompts with {len(comprehensive_prompts)} messages"
            )
            ...
            # 设置错误响应
            template.response = (
                f"Error occurred during comprehensive answer generation: {str(e)}"
            )
...
Please ensure your response is clear, accurate, and helpful.
    ...
    def _build_enhanced_prompts(self, template: ModelContext) -> List[Dict[str, str]]:
        ...
        return [
            ...
            {
                "role": "user",
                "content": f"Please provide your comprehensive analysis and answer.",
            },
        ]
