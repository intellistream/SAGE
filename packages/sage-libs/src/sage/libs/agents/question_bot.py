...
class QuestionBot(MapFunction):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"QuestionBot initialized - generating {questions_per_batch} questions per batch"
        )
    ...
    def _build_generation_prompt(self, num_questions: int) -> List[Dict[str, str]]:
        ...
        """
        # 准备模板数据
        template_data = {"num_questions": num_questions}
    ...
    def _parse_generated_questions(self, response: str) -> List[str]:
        ...
        for line in lines:
            ...
            clean_line = re.sub(
                r"^Question\s+\d+[\.\)\:]?\s*", "", clean_line, flags=re.IGNORECASE
            )
    ...
    def _ensure_question_diversity(self, questions: List[str]) -> List[str]:
        ...
        for i, question in enumerate(questions):
            ...
            if enhancement_chance < 0.3:  # 30%的机会添加背景信息
                ...
                # 简化：移除额外的词汇，保持核心
                simplified = question.replace("Can you please", "").replace(
                    "Could you", ""
                )
    ...
    def _create_ModelContext_from_question(self, question: str) -> ModelContext:
        ...
        """
        template = ModelContext(sequence=self.generation_count, raw_question=question)
    ...
    def execute(self, trigger_data: Any = None) -> ModelContext:
        ...
        try:
            self.logger.debug(
                f"QuestionBot generating question #{self.generation_count + 1}"
            )
            ...
            self.logger.info(
                f"Generated question #{self.generation_count}: '{final_question[:50]}...'"
            )
...
class EnhancedQuestionBot(QuestionBot):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"EnhancedQuestionBot initialized with domain focus: {domain_focus}"
        )
