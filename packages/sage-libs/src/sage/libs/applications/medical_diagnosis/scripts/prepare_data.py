...
DISEASE_MAPPING = {
    ...
    1: {"name": "轻度退行性变", "severity": "轻度", "description": "L4/L5椎间盘轻度退行性变"},
    2: {"name": "椎间盘突出", "severity": "中度", "description": "L4/L5椎间盘向后突出,压迫硬膜囊"},
    3: {"name": "多节段退行性变", "severity": "中度", "description": "L3/L4, L4/L5, L5/S1多节段退行性变"},
    4: {"name": "椎管狭窄", "severity": "重度", "description": "L4/L5椎管狭窄,硬膜囊受压"},
    5: {"name": "椎间盘脱出", "severity": "重度", "description": "L5/S1椎间盘脱出,游离到椎管内"},
...
def generate_mock_report(label: int, patient_info: Dict) -> str:
    ...
    if label == 0:
        ...
        conclusion = f"{disease_info['name']}，程度{disease_info['severity']}。"
        recommendations = "建议尽早手术治疗(椎间盘摘除术或椎管减压术)，以解除神经压迫。术后康复训练。"
...
def prepare_dataset():
    ...
    print(f"   ✓ train_index.json")
    print(f"   ✓ test_index.json")
    print(f"   ✓ all_cases.json")
