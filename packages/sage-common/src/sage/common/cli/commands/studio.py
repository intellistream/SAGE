...
def run_studio_script(command: str) -> int:
    ...
    try:
        ...
        result = subprocess.run(
            [str(script_path), command], capture_output=False, text=True
        )
...
def info():
    ...
    path_table.add_row(
        "Studio目录", str(studio_dir), "✅ 存在" if studio_dir.exists() else "❌ 不存在"
    )
    path_table.add_row(
        "管理脚本", str(script_path), "✅ 存在" if script_path.exists() else "❌ 不存在"
    )
