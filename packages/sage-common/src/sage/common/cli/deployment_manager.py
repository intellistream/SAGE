...
class DeploymentManager:
    ...
    def _find_project_root(self) -> Path:
        ...
        common_project_names = ["SAGE", "sage", "workspace/SAGE", "workspace/sage", "projects/SAGE", "code/SAGE"]
        home_dir = Path.home()
        ...
        try:
            with tarfile.open(package_path, "w:gz") as tar:
                ...
                if scripts_dir.exists():
                    ...
                    essential_scripts = [
                        "requirements/",  # 依赖文件
                        "common_utils.sh", "logging.sh", "config.sh"  # 工具脚本
                    ]
                ...
                # 4. 添加必需的配置文件
                required_files = ["project_config.toml", "pyproject.toml", "quickstart.sh"]
    ...
    def execute_ssh_command_with_progress(self, host: str, port: int, command: str, timeout: int = 60, step_name: str = "") -> bool:
        """执行SSH命令并显示进度"""
        ...
        ssh_cmd = [
            ...
            '-o', f'ConnectTimeout={ssh_config.get("connect_timeout", 30)}',
        ...
        try:
            ...
            def show_progress():
                ...
                while progress_active.is_set():
                    ...
                    progress_str = "." * (dots % 4)
                    typer.echo(f"\r⏳ 执行中{progress_str:<3} (已用时: {elapsed}s/{timeout}s)", nl=False)
                    dots += 1
                    time.sleep(1)
            ...
            if result.returncode == 0:
                ...
                typer.echo(f"❌ {step_name}失败 (返回码: {result.returncode})" if step_name else f"❌ 命令执行失败 (返回码: {result.returncode})")
                return False
                
        except subprocess.TimeoutExpired:
            progress_active.clear()
            typer.echo()
            typer.echo(f"❌ {step_name}超时 ({timeout}s)" if step_name else f"❌ SSH命令超时 ({timeout}s)")
            return False
        except Exception as e:
            progress_active.clear()
            typer.echo()
    ...
    def deploy_to_worker(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            if not self.execute_ssh_command_with_progress(host, port, simple_test, 15, "基本连接测试"):
            ...
            if not self.execute_ssh_command_with_progress(host, port, extract_cmd, 120, "文件解压"):  # 2分钟
                return False
            ...
            if not self.execute_ssh_command_with_progress(host, port, conda_init_commands, 30, "conda环境初始化"):
                return False
            ...
            # 安装步骤使用更长的超时时间
            if not self.execute_ssh_command_with_progress(host, port, install_command, 600, "SAGE安装"):  # 10分钟
                return False
