...
from .common import PROJECT_ROOT_OPTION, console, format_size
...
def home_command(
    ...
    days: int = typer.Option(
        7, "--days", "-d", help="Clean logs older than specified days"
    ),
    log_type: Optional[str] = typer.Option(
        None, "--type", "-t", help="Log type filter: test, jobmanager, toolkit, all"
    ),
):
    ...
    try:
        ...
        if action == "setup":
            ...
            with console.status("📊 Checking SAGE home status..."):
                ...
                link_valid = (
                    link_exists
                    and project_sage_link.is_symlink()
                    and project_sage_link.resolve() == real_sage_home
                )
            ...
            if real_exists:
                ...
                console.print(
                    f"✅ Real home exists with {len(existing_subdirs)}/{len(subdirs)} subdirectories"
                )
            else:
...
def _clean_logs(
    sage_home: Path, force: bool, days: int, log_type: Optional[str] = None
):
    ...
    console.print(
        f"📅 Cleaning logs older than {days} days (before {cutoff_date.strftime('%Y-%m-%d %H:%M:%S')})"
    )
        ...
        if item.is_file():
            ...
            if file_mtime < cutoff_time:
                ...
                if log_type and log_type != "all":
                    if not any(
                        pattern in item.name for pattern in _get_log_patterns(log_type)
                    ):
                        continue
    ...
    # Show summary
    console.print(
        f"🧹 Found {len(files_to_clean)} old log files ({format_size(total_size)}) and {len(dirs_to_clean)} empty directories"
    )
    ...
    if len(files_to_clean) <= 10:
        for file_path, size in files_to_clean:
            ...
            console.print(
                f"  📄 {rel_path} ({format_size(size)}, {age_days:.1f} days old)"
            )
    else:
    ...
    if not force:
        confirm = typer.confirm(
            f"🗑️ Delete {len(files_to_clean)} files and {len(dirs_to_clean)} directories?"
        )
        if not confirm:
    ...
    # Report results
    console.print(
        f"✅ Cleaned {cleaned_files} log files ({format_size(cleaned_size)})",
        style="green",
    )
...
def _show_logs_info(sage_home: Path):
    ...
    if oldest_file and newest_file:
        ...
        console.print(
            f"  📅 Date range: {oldest_date.strftime('%Y-%m-%d')} to {newest_date.strftime('%Y-%m-%d')} ({age_days:.1f} days)"
        )
    ...
    for category, data in categories.items():
        if data["files"]:
            icon = {"test": "🧪", "jobmanager": "📋", "toolkit": "🔧", "other": "📄"}[
                category
            ]
            console.print(
                f"  {icon} {category.title()}: {len(data['files'])} files ({format_size(data['size'])})"
            )
    ...
    if recent_files:
        ...
        for item in recent_files:
            ...
            console.print(
                f"  📄 {rel_path} ({file_size}, {file_time.strftime('%Y-%m-%d %H:%M')})"
            )
