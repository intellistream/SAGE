...
class PublishCommand(BaseCommand):
    ...
    def opensource(
        ...
        package_path: str = typer.Argument(
            ..., help="包路径（包含 pyproject.toml 的目录）"
        ),
        dry_run: bool = typer.Option(
            True, "--dry-run/--no-dry-run", help="是否为预演模式"
        ),
        ...
        try:
            ...
            if not force and not dry_run:
                if not Confirm.ask(
                    f"确认发布开源包 [bold]{package_name}[/bold] 到 PyPI?"
                ):
    ...
    def proprietary(
        ...
        try:
            ...
            if not force and not dry_run:
                if not Confirm.ask(
                    f"确认发布闭源包 [bold]{package_name}[/bold] 到 PyPI?"
                ):
                    console.print("取消发布", style="yellow")
                    raise typer.Exit(0)
            ...
            result = self._build_proprietary_package(
                package_path, package_name, output_path
            )
    ...
    def _validate_package_path(self, package_path: Path) -> bool:
        ...
        if not pyproject_file.exists() and not setup_file.exists():
            console.print(
                f"❌ 未找到 pyproject.toml 或 setup.py: {package_path}", style="red"
            )
            return False
    ...
    def _build_proprietary_package(
        self, package_path: Path, package_name: str, output_dir: Optional[Path] = None
    ) -> dict:
        """构建闭源包（编译为字节码）"""
                ...
                if "compiled_path" in locals():
                    ...
                    if compiled_pyproject.exists():
                        ...
                        if (
                            ...
                            or "declare" in error_msg.lower()
                        ):
                            console.print(
                                "    - 💡 可能是pyproject.toml配置重复或语法错误",
                                style="yellow",
                            )
                    else:
                        console.print(
                            "    - ⚠️ 编译后的pyproject.toml不存在", style="yellow"
                        )
                else:
    ...
    def _upload_to_pypi(self, build_path: Path):
        ...
        try:
            ...
            for wheel_file in wheel_files:
                result = subprocess.run(
                    ["twine", "upload", str(wheel_file)], capture_output=True, text=True
                )
                ...
                if result.returncode == 0:
                    ...
                    console.print(
                        f"  ❌ {wheel_file.name} 上传失败: {error_msg}", style="red"
                    )
    ...
    def _upload_wheel_file(self, wheel_path: Path):
        ...
        try:
            result = subprocess.run(
                ["twine", "upload", str(wheel_path)], capture_output=True, text=True
            )
            ...
            if result.returncode == 0:
                ...
                error_msg = result.stderr or result.stdout or "未知错误"
                console.print(
                    f"  ❌ {wheel_path.name} 上传失败: {error_msg}", style="red"
                )
                raise RuntimeError(f"上传失败: {error_msg}")
