...
class TestCommand(BaseCommand):
    ...
    def _discover_test_files(
        self, test_dir: Path, pattern: str = "test_*.py"
    ) -> List[Path]:
        """发现测试文件"""
        test_files = []
    ...
    def _run_tests_parallel(
        ...
        console.print(
            f"🚀 Running {len(test_files)} test files with {max_workers} parallel workers...",
            style="blue",
        )
                ...
                try:
                    ...
                    if result["passed"]:
                        ...
                        if verbose and result.get("error"):
                            console.print(
                                f"   Error: {result['error']}", style="dim red"
                            )
    ...
    def _run_tests(
        ...
        if not test_dir:
            ...
            console.print(
                "   Expected directories: tests, test, Tests, Test", style="dim"
            )
            return {
                "total": 0,
                "passed": 0,
        ...
        if not test_files:
            console.print(
                f"❌ No test files found matching pattern: {pattern}", style="red"
            )
            return {
                "total": 0,
                "passed": 0,
        ...
        while project_root.parent != project_root:
            if (project_root / ".sage").exists() or (
    ...
    def _register_commands(self):
        ...
        def test_main(
            ...
            failed: bool = typer.Option(
                False, "--failed", help="Run only previously failed tests"
            ),
            ...
            if not project_path.is_dir():
                console.print(
                    f"❌ Path is not a directory: {project_path}", style="red"
                )
                raise typer.Exit(1)
            ...
            try:
                ...
                if results["total"] > 0:
                    if results["failed"] == 0 and results["errors"] == 0:
                        console.print(
                            f"✅ All tests passed! ({results['passed']}/{results['total']})",
                            style="green",
                        )
                    else:
                        console.print(
                            f"❌ Tests failed: {results['failed']} failed, {results['errors']} errors, {results['passed']} passed",
                            style="red",
                        )

                        if results["failed_tests"]:
                            console.print(
                                f"📝 Failed tests logged to .sage/logs/ directory",
                                style="dim",
                            )
                        raise typer.Exit(1)
                else:
        ...
        def test_cache(
            ...
            if action == "clear":
                ...
                if failed_tests:
                    console.print(
                        f"📝 Found {len(failed_tests)} previously failed tests:",
                        style="blue",
                    )
        ...
        def list_tests(
            ...
            if not test_dir:
                console.print(
                    f"❌ No tests directory found in {project_path}", style="red"
                )
                raise typer.Exit(1)
            ...
            console.print(
                f"📁 Found {len(test_files)} test file(s) in {test_dir}:", style="blue"
            )
            for test_file in test_files:
                try:
                    rel_path = test_file.relative_to(project_path)
