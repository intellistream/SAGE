...
def dependencies(
    analysis_type: str = typer.Option(
        "all", help="Analysis type: all, circular, missing, conflicts"
    ),
    output_format: str = typer.Option(
        "summary", help="Output format: summary, json, markdown"
    ),
    ...
    try:
        ...
        result = toolkit.analyze_dependencies(
            analysis_type=analysis_type, output_format=output_format
        )
        ...
        if output_format == "summary":
            ...
            if result.get("issues"):
                console.print(
                    f"⚠️  Found {len(result['issues'])} issues", style="yellow"
                )
