...
class CheckDependencyCommand(BaseCommand):
    ...
    def _create_command(self):
        ...
        def check_dependency_main(
            ...
            max_workers: int = typer.Option(
                ...
                "--max-workers",
                "-w",
                help="Maximum number of concurrent import checks",
            ),
            ...
            try:
                ...
                console.print(
                    f"⚠️  A module called sys.exit() during analysis (exit code: {e.code})",
                    style="yellow",
                )
                console.print(
                    "💡 This usually indicates missing dependencies in the analyzed package",
                    style="dim yellow",
                )
    ...
    def _check_imports_batch(
        self, imports: Set[str], max_workers: int = 10
        ...
        if max_workers == 1:
            for import_name in imports:
                try:
                    ...
                    console.print(
                        f"❌ Unexpected error testing {import_name}: {e}", style="red"
                    )
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in as_completed(future_to_import):
                        try:
                            ...
                            console.print(
                                f"❌ Unexpected error in thread for {import_name}: {e}",
                                style="red",
                            )
                            results.append((import_name, False, str(e)))
            except Exception as e:
                console.print(
                    f"❌ Error in concurrent import testing: {e}", style="red"
                )
                # 回退到非并发模式
    ...
    def _write_import_check_log(
        ...
        with open(log_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"  Success rate: {(successful_imports/total_imports*100):.1f}%\n\n"
            )
    ...
    def _run_dependency_check(self, path: str, max_workers: int, verbose: bool):
        ...
        if not source_files:
            console.print(
                f"⚠️  No Python source files found in {src_dir}", style="yellow"
            )
            return
        ...
        if failed_count > 0:
            console.print(
                "💡 Install missing dependencies or check import paths", style="yellow"
            )
