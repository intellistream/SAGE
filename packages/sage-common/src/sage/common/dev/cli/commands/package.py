...
def manage_package(
    action: str = typer.Argument(
        help="Action: list, install, uninstall, status, build"
    ),
    ...
    try:
        ...
        result = toolkit.manage_packages(
            action=action, package_name=package_name, dev=dev, force=force
        )
...
def dependencies(
    ...
    output_format: str = typer.Option(
        "json", help="Output format: json, markdown, summary"
    ),
    project_root: str = PROJECT_ROOT_OPTION,
    verbose: bool = VERBOSE_OPTION,
):
    ...
    try:
        ...
        result = toolkit.analyze_dependencies(
            action=action, output_format=output_format
        )
...
def fix_imports(
    ...
    try:
        ...
        if verbose:
            console.print(
                f"🔧 Fixing imports for: {package_name or 'all packages'}", style="blue"
            )

        result = toolkit.fix_imports(
            package_name=package_name, dry_run=dry_run, verbose=verbose
        )

        if dry_run:
            console.print(
                f"🔍 Would fix {result.get('changes_count', 0)} import issues",
                style="yellow",
            )
            console.print("💡 Use --no-dry-run to apply changes", style="blue")
        else:
            console.print(
                f"✅ Fixed {result.get('changes_count', 0)} import issues",
                style="green",
            )
