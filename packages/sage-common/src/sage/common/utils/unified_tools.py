        ...
        if current >= min_version:
            if not quiet:
                Logger.print_success(
                    f"✅ Python {'.'.join(map(str, current))} (满足要求)"
                )
            return True
        else:
            if not quiet:
                Logger.print_error(
                    f"❌ Python {'.'.join(map(str, min_version))}+ 需要, 当前: {'.'.join(map(str, current))}"
                )
            return False
    ...
    def check_disk_space(min_gb: float = 1.0, quiet: bool = False) -> bool:
        try:
            ...
            if free_gb >= min_gb:
                ...
                if not quiet:
                    Logger.print_error(
                        f"❌ 磁盘空间不足: {free_gb:.1f}GB (需要 {min_gb}GB)"
                    )
                return False
        except:
            if not quiet:
...
class ProcessRunner:
    ...
    def run_conda_command(
        args: List[str], env_name: str = None, description: str = ""
    ) -> bool:
        """运行conda命令"""
        cmd = ["conda"] + args
        if env_name:
