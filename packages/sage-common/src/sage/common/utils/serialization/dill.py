...
def trim_object_for_remote(
    ...
    只做清理工作，不进行实际序列化，调用方自行选择序列化方式（cloudpickle / dill）。
    适用于 Flownet / gRPC / TCP 远程调用前清理对象，避免序列化错误。
...
class ObjectTrimmer:
    """用于远程调用的对象预处理器（运行时无关，支持 Flownet / gRPC / TCP）"""
    ...
    def trim_for_remote_call(
        ...
        if not deep_clean:
            ...
            # 深度清理：使用完整的预处理流程
            return trim_object_for_remote(obj, include, exclude)
    ...
    def trim_transformation(transformation_obj: Any) -> Any:
        ...
        return ObjectTrimmer.trim_for_remote_call(transformation_obj, exclude=exclude_attrs)
    ...
    def trim_operator(operator_obj: Any) -> Any:
        ...
        return ObjectTrimmer.trim_for_remote_call(operator_obj, exclude=exclude_attrs)
