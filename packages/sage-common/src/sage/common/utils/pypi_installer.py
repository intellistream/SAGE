...
# 导入统一工具模块
try:
    from .unified_tools import (checker, create_progress_bar, interact, logger,
                                runner)
except ImportError:
    ...
    try:
        ...
        try:
            ...
            logger.print_warning(
                "If you just installed it, try restarting your terminal/Python session"
            )
            return False
        ...
        if missing_packages:
            logger.print_warning(
                f"Some packages missing: {', '.join(missing_packages)}"
            )
        ...
        try:
            ...
            logger.print_warning(
                "Try: pip install isage[cli] or pip install isage[full]"
            )
                ...
                try:
                    if interact.confirm("Start JobManager now?", default=True):
                        ...
                        if start_success:
                            ...
                            logger.print_warning(
                                "Failed to start JobManager automatically"
                            )
