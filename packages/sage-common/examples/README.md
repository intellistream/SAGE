...
├── L5-cli/                # 接口层：CLI 工具（见 sage-examples 仓库）
│
├── config/                # 配置文件示例
└── docs/                  # 文档（快速参考、故障排除）
```
...
**说明**：L5 接口层（sage-cli, sage-tools）为 CLI 工具，相关示例请查看各工具的帮助命令：

```bash
sage --help          # CLI 工具帮助
sage-dev --help      # 开发工具帮助
```

**独立仓库**（不在 SAGE 核心）：

- **sage-benchmark**: 性能评估框架 (`pip install isage-benchmark`)
- **sage-examples**: 教程和应用示例 (GitHub 仓库)
- **sage-studio**: 可视化 Web UI (独立仓库)
...
| L5       | sage-cli, sage-tools | CLI         | 接口层       |
| **总计** | -                    | **70+**     | **5 层架构** |
...
- **2025-01**: 简化为 5 层架构（L1-L5）
  - sage-benchmark/examples/studio 独立为单独仓库
  - sage-cli 和 sage-tools 合并为 L5 接口层
- **2025-01**: 简化为 5 层架构（L1-L5）
  - sage-benchmark/examples/studio 独立为单独仓库
  - sage-cli 和 sage-tools 合并为 L5 接口层
