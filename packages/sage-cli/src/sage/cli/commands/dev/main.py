...
# 添加Issues管理子命令
try:
    from sage.cli.dev.issues.cli import app as issues_app
...
# 添加PyPI管理子命令
try:
    from sage.cli.commands.pypi import app as pypi_app
...
def quality(
    ...
    if architecture and not submodules_only:
        ...
        console.print("=" * 60)
        try:
            from sage.cli.dev.tools.architecture_checker import ArchitectureChecker
    ...
    if devnotes and not submodules_only:
        ...
        console.print("=" * 60)
        try:
            from sage.cli.dev.tools.devnotes_checker import DevNotesChecker
    ...
    if readme and not submodules_only:
        ...
        console.print("=" * 60)
        try:
            from sage.cli.dev.tools.package_readme_checker import PackageREADMEChecker
...
def analyze(
    ...
    """分析项目依赖和结构"""
    try:
        from sage.cli.dev.tools.dependency_analyzer import DependencyAnalyzer
...
def status(
    ...
    try:
        ...
        from sage.cli.dev.tools.project_status_checker import ProjectStatusChecker
...
def test(
    ...
    try:
        ...
        from sage.cli.dev.tools.enhanced_test_runner import EnhancedTestRunner
...
def _run_issues_manager_test(project_root: str, verbose: bool):
    ...
    try:
        ...
        # 导入并运行新的Python测试模块
        from sage.cli.dev.issues.tests import IssuesTestSuite
...
def architecture(
    ...
    """
    from sage.cli.dev.tools.architecture_checker import (
