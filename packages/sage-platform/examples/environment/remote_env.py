...
class SimpleProcessor(MapFunction):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        import socket as _socket  # 在类内部导入，确保远程 Actor 可访问
...
def example_default_scheduler():
    ...
    print("📦 [1/6] 创建 FlownetEnvironment (使用 load_aware 调度器)...")
    ...
    env = FlownetEnvironment(name="distributed_scheduler_demo", scheduler=scheduler)
    ...
    # 步骤4: 提交任务
    print("🚀 [4/6] 提交任务到 JobManager...")
    step_start = time.time()
    try:
        env.submit(autostop=True)  # autostop=True: 阻塞直到批处理完成
        step_duration = time.time() - step_start
    ...
    step_start = time.time()
    try:
        nodes = env.health_check()
        step_duration = time.time() - step_start
        print(f"   ✅ 运行时健康检查完成 (耗时: {step_duration:.3f}秒)")
        print(f"   📋 节点状态条目: {len(nodes)}")
        if nodes:
            print(f"   📋 首条节点状态: {nodes[0]}")
        print()
    except Exception as e:
        step_duration = time.time() - step_start
        print(f"   ⚠️  运行时健康检查异常 (耗时: {step_duration:.3f}秒)")
        print(f"   错误: {e}\n")

    # 查看调度器节点放置统计（来自 scheduler，而非 environment API）
    print("📊 获取调度器节点放置统计...")
    if hasattr(scheduler, "node_selector"):
        node_task_count = scheduler.node_selector.node_task_count
        if node_task_count:
            print("\n   📍 节点放置统计:")
            for node_id, count in sorted(node_task_count.items()):
        ...
        print("   ℹ️  当前调度器不暴露 node_selector 统计")
    print()
