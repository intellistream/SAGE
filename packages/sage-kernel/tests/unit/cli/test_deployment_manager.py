...
class TestDeploymentManager:
    ...
    def test_init(self, mock_get_config_manager):
        ...
        # 验证项目根目录是一个有效的Path对象，而不是硬编码特定名称
        assert isinstance(manager.project_root, Path)
        assert manager.project_root.exists()
        # 验证项目根目录是绝对路径
        assert manager.project_root.is_absolute()
        # 验证项目根目录包含预期的目录结构特征
        # 这里使用更灵活的验证方式，检查是否为开发环境或安装环境
        is_dev_env = 'site-packages' not in str(manager.project_root)
        if is_dev_env:
            # 开发环境，应该能找到源码结构
            assert manager.project_root.name in ['src', 'sage-cli'] or 'sage' in str(manager.project_root)
    ...
    def test_project_root_detection_flexibility(self, mock_get_config_manager):
        ...
        # 验证不依赖于硬编码的目录名称
        # 项目根目录可能是多种情况之一，但不应该硬编码期望特定名称
        possible_indicators = [
            # 可能的项目标识符
            'sage' in str(project_root).lower(),
            'src' in str(project_root),
            project_root.name in ['sage-cli', 'SAGE'],
            # 或者至少是一个合理的目录路径
            len(project_root.parts) >= 2
        ]
