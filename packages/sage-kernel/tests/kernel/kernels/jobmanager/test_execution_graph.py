...
from sage.kernel.kernels.jobmanager.execution_graph.graph_edge import GraphEdge
...
class TestExecutionGraphInitialization:
    ...
    def test_logging_system_setup(self, mock_environment):
        ...
        with patch.object(ExecutionGraph, '_build_graph_from_pipeline'), \
             patch.object(ExecutionGraph, '_build_service_nodes'), \
             patch.object(ExecutionGraph, 'generate_runtime_contexts'), \
             patch('sage.kernel.utils.logging.custom_logger.CustomLogger') as mock_logger:
...
class TestExecutionGraphRuntimeContext:
    ...
    def test_generate_runtime_contexts_for_nodes(self, execution_graph):
        ...
        with patch('sage.kernel.kernels.runtime.task_context.TaskContext') as mock_task_context:
    ...
    def test_generate_runtime_contexts_for_service_nodes(self, execution_graph):
        ...
        with patch('sage.kernel.kernels.runtime.service_context.ServiceContext') as mock_service_context:
            mock_context_instance = Mock()
...
class TestExecutionGraphIntegration:
    ...
    def test_complete_graph_construction(self):
        ...
        with patch('sage.kernel.utils.logging.custom_logger.CustomLogger'), \
             patch('sage.kernel.kernels.runtime.task_context.TaskContext'), \
             patch('sage.kernel.kernels.runtime.service_context.ServiceContext'):
