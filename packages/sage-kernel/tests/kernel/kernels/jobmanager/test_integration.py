...
class TestJobManagerFullIntegration:
    ...
    def test_complete_job_submission_workflow(self, reset_jobmanager_singleton, mock_environment):
        """测试完整的作业提交工作流程"""
        with patch('sage.kernel.jobmanager.job_manager.JobManagerServer'), \
             patch('sage.kernel.utils.serialization.dill.deserialize_object'), \
             patch('sage.kernel.runtime.dispatcher.Dispatcher'):
    ...
    def test_resource_cleanup_on_shutdown(self, reset_jobmanager_singleton):
        """测试关闭时的资源清理"""
        with patch('sage.kernel.jobmanager.job_manager.JobManagerServer') as mock_server_class:
            mock_server = Mock()
...
class TestJobManagerExecutionGraphIntegration:
    ...
    def test_runtime_context_generation_integration(self, mock_environment_with_pipeline):
        ...
        with patch.object(ExecutionGraph, '_build_graph_from_pipeline'), \
            ...
            with patch('sage.kernel.runtime.task_context.TaskContext') as mock_task_context, \
                 patch('sage.kernel.runtime.service_context.ServiceContext') as mock_service_context:
