...
class ServiceDict:
    ...
    def __init__(self, service_manager: "ServiceManager", logger=None):
        ...
        self._service_proxies: Dict[str, "ServiceCallProxy"] = (
            {}
        )  # 缓存ServiceCallProxy对象
...
class AsyncServiceCallProxy:
    ...
    def __init__(
        ...
        self.logger = (
            ...
            else logging.getLogger(f"{__name__}.async.{service_name}")
        )

        self.logger.debug(
            f"[ASYNC_PROXY] Created AsyncServiceCallProxy for service: {service_name}"
        )

    def __getattr__(self, method_name: str):
        """获取服务方法的异步调用代理，返回 Future 对象"""
        self.logger.debug(
            f"[ASYNC_PROXY] Creating async method proxy for {self._service_name}.{method_name}"
        )

        def async_method_call(*args, timeout: float = 30.0, **kwargs):
            """异步方法调用，返回 Future 对象"""
            self.logger.info(
                f"[ASYNC_PROXY] Starting async call: {self._service_name}.{method_name} with timeout={timeout}s"
            )
            ...
            self.logger.debug(
                f"[ASYNC_PROXY] Future created for {self._service_name}.{method_name}"
            )
            return future
...
class AsyncServiceDict:
    ...
    def __init__(self, service_manager: "ServiceManager", logger=None):
        self._service_manager = service_manager
        self._async_service_proxies: Dict[str, AsyncServiceCallProxy] = (
            {}
        )  # 缓存AsyncServiceCallProxy对象
        self.logger = logger if logger is not None else logging.getLogger(__name__)
...
class BaseRuntimeContext:
    ...
    def call_service_async(self):
        ...
        if not hasattr(self, "_async_service_dict") or self._async_service_dict is None:
            self._async_service_dict = AsyncServiceDict(
                self.service_manager, logger=self.logger
            )
