...
class JobManagerController:
            ...
            if result["success"]:
                ...
                if result.get("method") == "access_denied" and needs_sudo_for_proc:
                    ...
                    if sudo_result["success"]:
                        ...
                        typer.echo(f"❌ Failed to kill process {proc.pid} even with sudo: {sudo_result['error']}")
    ...
    def is_port_occupied(self) -> bool:
        """检查端口是否被占用"""
        from sage.kernel.utils.system.network import is_port_occupied as check_port_occupied
