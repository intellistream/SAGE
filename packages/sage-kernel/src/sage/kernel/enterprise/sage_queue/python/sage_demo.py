...
class SAGEDataProcessor:
    ...
    def process_batch(
        self, input_queue_name: str, output_queue_name: str, batch_size: int
    ):
        """处理一批数据"""
...
def demonstration_simple_pipeline():
    ...
    try:
        ...
        generator_process = multiprocessing.Process(
            target=sage_data_generator, args=(input_queue_name, data_count)
        )
        ...
        collector_process = multiprocessing.Process(
            target=sage_result_collector, args=(output_queue_name, data_count)
        )
...
def demonstration_queue_reference():
    ...
    for i in range(2):
        process = multiprocessing.Process(
            target=worker_process, args=(queue_ref_state, i)
        )
        process.start()
