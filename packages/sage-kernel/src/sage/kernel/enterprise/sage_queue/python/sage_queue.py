...
import threading
import time
from ctypes import (POINTER, Structure, c_bool, c_char, c_int, c_uint32,
                    c_uint64, c_void_p)
...
3. 检查系统权限和资源限制"""
                ...
                logger.error(
                    f"Failed to create or open ring buffer: {name}", exc_info=True
                )
                raise
            # 对于打开的现有缓冲区，需要增加引用计数
            try:
                ...
                if ref_count < 0:
                    raise RuntimeError(
                        f"Failed to increment reference count for ring buffer: {name}"
                    )
            except Exception as e:
                logger.error(
                    f"Failed to increment reference count for ring buffer: {name}",
    ...
    def _load_library(self) -> ctypes.CDLL:
        ...
        if os.environ.get("CI") or os.path.exists(
            os.path.join(current_dir, "ring_buffer.cpp")
        ):
...
4. Compile manually: g++ -std=c++11 -fPIC -shared -O2 -I. -o libring_buffer.so ring_buffer.cpp
    ...
    def _setup_function_signatures(self):
        ...
        # 读写操作
        self._lib.ring_buffer_write.argtypes = [
            POINTER(RingBufferStruct),
            c_void_p,
            c_uint32,
        ]
        self._lib.ring_buffer_write.restype = c_int

        self._lib.ring_buffer_read.argtypes = [
            POINTER(RingBufferStruct),
            c_void_p,
            c_uint32,
        ]
        ...
        self._lib.ring_buffer_peek.argtypes = [
            POINTER(RingBufferStruct),
            c_void_p,
            c_uint32,
        ]
        ...
        # 统计信息
        self._lib.ring_buffer_get_stats.argtypes = [
            POINTER(RingBufferStruct),
            POINTER(RingBufferStats),
        ]
            ...
            with self._lock:
                ...
                if available >= len(data_with_len):
                    result = self._lib.ring_buffer_write(
                        self._rb, ctypes.c_char_p(data_with_len), len(data_with_len)
                    )
                    if result > 0:
    ...
    def close(self):
        ...
        if self._rb:
            try:
                ...
                logger.debug(
                    f"Decreased ref count for {self.name}, new count: {ref_count}"
                )
...
def destroy_queue(name: str):
    ...
    # 清理可能的共享内存文件
    shm_paths = [f"/dev/shm/sage_ringbuf_{name}", f"/tmp/sage_ringbuf_{name}"]
...
def cleanup_invalid_queues():
    ...
    try:
        # 获���当前用户拥有的共享内存文件
        result = subprocess.run(
            ["ls", "-la", "/dev/shm/"], capture_output=True, text=True, check=False
        )
        if result.returncode != 0:
            return
...
def list_all_sage_queues():
    ...
    try:
        ...
        if queues:
            print(
                f"{'原始名称':<20} {'命名空间':<15} {'PID':<8} {'所有者':<10} {'大小':<10} {'进程状态':<10} {'文件名'}"
            )
            print("-" * 100)
            for q in queues:
                print(
                    f"{q['original_name']:<20} {q['namespace'] or 'N/A':<15} {q['pid'] or 'N/A':<8} {q['owner']:<10} {q['size']:<10} {q['process_status']:<10} {q['filename']}"
                )
        else:
            print("没有找到SAGE队列")
