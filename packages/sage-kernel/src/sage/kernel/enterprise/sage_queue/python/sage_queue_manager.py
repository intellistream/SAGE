...
from sage.extensions.sage_queue.python.sage_queue import (
    SageQueue, cleanup_invalid_queues, cleanup_user_queues,
...
示例用法:
  ...
  %(prog)s test TestQueue          # 创建并测试队列
    ...
    cleanup_parser.add_argument(
        "--invalid", action="store_true", help="只清理无效队列（进程已死）"
    )
    cleanup_parser.add_argument(
        "--all", action="store_true", help="清理所有当前用户的队列"
    )
    cleanup_parser.add_argument(
        "--quiet", action="store_true", help="静默模式，不显示详细信息"
    )

    # cleanup-user命令
    cleanup_user_parser = subparsers.add_parser(
        "cleanup-user", help="清理指定用户的队列"
    )
    cleanup_user_parser.add_argument("user", nargs="?", help="用户名（默认为当前用户）")
    cleanup_user_parser.add_argument(
        "--force", action="store_true", help="强制清理（包括活跃进程的队列）"
    )
    cleanup_user_parser.add_argument(
        "--quiet", action="store_true", help="静默模式，不显示详细信息"
    )
    ...
    test_parser.add_argument(
        "--no-multi-tenant", action="store_true", help="禁用多租户模式"
    )
    test_parser.add_argument("--auto-cleanup", action="store_true", help="启用自动清理")
