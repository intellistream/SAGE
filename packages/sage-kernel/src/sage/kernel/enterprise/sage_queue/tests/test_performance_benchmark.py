...
def benchmark_throughput() -> BenchmarkResult:
    ...
    try:
        ...
        for msg_size in message_sizes:
            ...
            write_bandwidth = (
                (written * msg_size) / (1024 * 1024) / write_time
                if write_time > 0
                else 0
            )
            read_bandwidth = (
                (read * msg_size) / (1024 * 1024) / read_time if read_time > 0 else 0
            )
            ...
            print(
                f"    写入: {written} 消息, {write_throughput:.0f} msg/s, {write_bandwidth:.1f} MB/s"
            )
            print(
                f"    读取: {read} 消息, {read_throughput:.0f} msg/s, {read_bandwidth:.1f} MB/s"
            )
...
def benchmark_latency() -> BenchmarkResult:
    ...
    try:
        ...
        print(
            f"  写入延迟 (ms): 平均={write_stats['mean']:.3f}, P95={write_stats['p95']:.3f}"
        )
        print(
            f"  读取延迟 (ms): 平均={read_stats['mean']:.3f}, P95={read_stats['p95']:.3f}"
        )
        print(
            f"  往返延迟 (ms): 平均={roundtrip_stats['mean']:.3f}, P95={roundtrip_stats['p99']:.3f}"
        )
        ...
        for num_threads in thread_counts:
            ...
            def worker(thread_id: int):
                try:
                    ...
                    with results_lock:
                        results_data["completed_ops"] += (
                            messages_per_thread * 2
                        )  # put + get
                        results_data["total_time"] = max(
                            results_data["total_time"], end_time - start_time
                        )
            ...
            print(
                f"    操作数: {total_ops}, 用时: {wall_time:.3f}s, 性能: {ops_per_second:.0f} ops/s"
            )
...
def benchmark_memory_efficiency() -> BenchmarkResult:
    ...
    try:
        ...
        for buffer_size in buffer_sizes:
            ...
            efficiency = (
                messages_written / theoretical_max if theoretical_max > 0 else 0
            )
            ...
            memory_results[buffer_size] = {
                ...
                "bytes_per_message": (
                    buffer_size / messages_written if messages_written > 0 else 0
                ),
            }

            print(
                f"    存储消息: {messages_written}, 利用率: {utilization:.1%}, 效率: {efficiency:.1%}"
            )
...
def benchmark_multiprocess_worker(
    queue_name: str, worker_id: int, num_operations: int
) -> Dict[str, Any]:
    """多进程基准测试工作进程"""
    try:
        queue = SageQueue(queue_name)
        ...
        return {
            ...
            "ops_per_second": (
                completed / (end_time - start_time) if end_time > start_time else 0
            ),
            "success": True,
        }
...
def benchmark_multiprocess() -> BenchmarkResult:
    ...
    try:
        ...
        print(
            f"  启动 {num_processes} 个进程，每个执行 {operations_per_process} 对操作..."
        )
        ...
        avg_ops_per_sec = (
            statistics.mean([r["ops_per_second"] for r in successful_workers])
    ...
    for benchmark_func in benchmarks:
        ...
        try:
            ...
            if benchmark_result.success:
                print(
                    f"✓ {benchmark_result.name} 完成 ({benchmark_result.duration:.2f}s)"
                )
            else:
                print(
                    f"✗ {benchmark_result.name} 失败: {benchmark_result.metrics.get('error', 'Unknown error')}"
                )
