...
def test_edge_cases() -> TestResult:
    ...
    try:
        ...
        for expected in special_values:
            ...
            assert (
                actual == expected
            ), f"特殊值不匹配: expected={expected}, actual={actual}"
...
def test_concurrent_access() -> TestResult:
    ...
    try:
        ...
        assert (
            total_messages_read == expected_messages
        ...
        stats = {
            ...
            "avg_get_time": sum(time_taken for time_taken, _ in results["get"])
            / num_threads,
        }
...
def test_persistence() -> TestResult:
    ...
    try:
        ...
        ), f"数据不一致: {retrieved_data} != {test_data}"
...
def test_stress_performance() -> TestResult:
    ...
    try:
        ...
        for msg_size in message_sizes:
            ...
            # 生成测试数据
            test_message = {"id": 0, "data": "x" * msg_size, "timestamp": time.time()}
            ...
            throughput_mbps = (
                (write_count * msg_size) / (1024 * 1024) / max(write_time, 0.001)
            )
...
def test_multiprocess_robustness() -> TestResult:
    ...
    def worker_process(
        queue_name: str, worker_id: int, operation_count: int, seed: int
    ):
        """工作进程"""
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for worker_id in range(num_workers):
                future = executor.submit(
                    worker_process, queue_name, worker_id, operations_per_worker, seed
                )
