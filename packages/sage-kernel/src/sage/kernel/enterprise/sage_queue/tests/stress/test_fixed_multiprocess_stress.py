...
class FixedMultiprocessStressTester:
                    ...
                    if queue_name in queues:
                        ...
                        if stats["messages_sent"] % 100 == 0:
                            result_file = os.path.join(
                                test_dir, f"producer_{worker_id}_progress.json"
                            )
    ...
    def fixed_consumer_worker(
        ...
        try:
            ...
            while (
                received_count < expected_messages
                and (time.time() - start_time) < total_timeout
            ):
                try:
                    queue_name = random.choice(queue_names)
                    if queue_name in queues:
                        message = queues[queue_name].receive(
                        ...
                        if message is not None:
                            ...
                            if received_count % 100 == 0:
                                result_file = os.path.join(
                                    test_dir, f"consumer_{worker_id}_progress.json"
                                )
                                with open(result_file, "w") as f:
    ...
    @staticmethod
    def fixed_lifecycle_worker(
        worker_id: int, cycles: int, test_dir: str
    ) -> Dict[str, Any]:
        """修复版的生命周期工作进程"""
        try:
            from sage.extensions.sage_queue.python.sage_queue import SageQueue
            ...
            for cycle in range(cycles):
                try:
                    ...
                    if stats["cycles_completed"] % 10 == 0:
                        result_file = os.path.join(
                            test_dir, f"lifecycle_{worker_id}_progress.json"
                        )
                        with open(result_file, "w") as f:
                            json.dump(
                                {"cycles_completed": stats["cycles_completed"]}, f
                            )
...
class TestFixedMultiprocessStress:
    ...
    def test_fixed_concurrent_read_write_stress(self):
        ...
        config = StressTestConfig(
            num_processes=4, num_queues=2, test_duration=10, message_size=100
        )
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=config.num_processes) as executor:
                ...
                producer_futures = [
                    executor.submit(
                        FixedMultiprocessStressTester.fixed_producer_worker, *args
                    )
                ...
                consumer_futures = [
                    executor.submit(
                        FixedMultiprocessStressTester.fixed_consumer_worker, *args
                    )
    ...
    def test_fixed_lifecycle_management_stress(self):
        ...
        config = StressTestConfig(num_processes=6, lifecycle_cycles=20)
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=config.num_processes) as executor:
                futures = [
                    executor.submit(
                        FixedMultiprocessStressTester.fixed_lifecycle_worker, *args
                    )
    ...
    def _analyze_fixed_results(
        self, producer_results: List[Dict], consumer_results: List[Dict]
    ):
        """分析修复版的测试结果"""
        print("\n=== 修复版多进程并发读写压力测试结果 ===")
        ...
        print(
            f"✓ 修复版压力测试通过：发送 {total_sent} 条消息，接收 {total_received} 条消息"
        )
