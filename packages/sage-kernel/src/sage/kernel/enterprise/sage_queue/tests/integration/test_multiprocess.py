...
from ..utils import ConcurrencyTester, DataGenerator, ProducerConsumerScenario
...
class TestMultiprocessBasic:
    ...
    def test_single_producer_single_consumer(self, queue_name, process_helper):
        ...
        producer_proc = process_helper.start_process(
            producer_worker, args=(queue_name, num_messages, message_size)
        )

        # Start consumer
        consumer_proc = process_helper.start_process(
            consumer_worker, args=(queue_name, num_messages, 10.0)
        )
    ...
    def test_multiple_producers_single_consumer(self, queue_name, process_helper):
        ...
        for i in range(num_producers):
            process_helper.start_process(
                producer_worker, args=(f"{queue_name}_p{i}", messages_per_producer, 30)
            )

        # Start consumer
        process_helper.start_process(
            consumer_worker, args=(queue_name, total_messages, 20.0)
        )
    ...
    def test_single_producer_multiple_consumers(self, queue_name, process_helper):
        ...
        # Start producer
        process_helper.start_process(
            producer_worker, args=(queue_name, total_messages, 40)
        )

        # Start consumers
        for i in range(num_consumers):
            expected = messages_per_consumer + (
                1 if i < (total_messages % num_consumers) else 0
            )
            process_helper.start_process(
                consumer_worker, args=(queue_name, expected, 20.0)
            )
...
class TestMultiprocessStress:
    ...
    def test_many_producers_many_consumers(self, queue_name, process_helper):
        ...
        for i in range(num_producers):
            process_helper.start_process(
                producer_worker, args=(queue_name, messages_per_producer, 20)
            )
        ...
        for i in range(num_consumers):
            ...
            process_helper.start_process(
                consumer_worker, args=(queue_name, expected, 30.0)
            )
        ...
        successful_processes = sum(
            1 for proc in process_helper.processes if proc.exitcode == 0
        )
    ...
    def test_long_running_multiprocess(self, queue_name, process_helper):
        ...
        for i in range(num_consumers):
            ...
            process_helper.start_process(
                consumer_worker, args=(queue_name, expected, 60.0)
            )
...
class TestMultiprocessErrorHandling:
    ...
    def test_consumer_timeout_handling(self, queue_name, process_helper):
        ...
        process_helper.start_process(
            consumer_worker, args=(queue_name, 10, 2.0)  # Short timeout
        )
    ...
    def test_process_interruption_handling(self, queue_name, process_helper):
        ...
        producer_proc = process_helper.start_process(
            producer_worker, args=(queue_name, 1000, 50)
        )
        ...
        assert (
            consumer_proc.exitcode == 0
        ), "Consumer should handle interrupted producer"
...
def test_producer_consumer_scenario_integration(queue_name):
    ...
    scenario = ProducerConsumerScenario(
        queue_factory=queue_factory, num_producers=2, num_consumers=2
    )

    result = scenario.run(messages_per_producer=50, timeout=20.0)
