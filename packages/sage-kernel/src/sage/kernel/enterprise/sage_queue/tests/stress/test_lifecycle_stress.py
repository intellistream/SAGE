...
def lifecycle_creator_worker(
    worker_id: int, config: LifecycleStressConfig, monitor: QueueLifecycleMonitor
) -> Dict[str, Any]:
    """生命周期创建者工作进程"""
    try:
        # 尝试导入真实的 SageQueue
        ...
        for batch in range(config.queues_per_creator // config.creation_burst_size):
            ...
            for i in range(config.creation_burst_size):
                queue_id = (
                    f"lifecycle_{worker_id}_{batch}_{i}_{int(time.time()*1000000)}"
                )
                ...
                try:
                    ...
                    # 记录创建
                    monitor.record_creation(
                        queue_id, f"creator_{worker_id}", time.time()
                    )
                    ...
                    monitor.record_error(
                        "creation_failed", str(e), f"creator_{worker_id}"
                    )
...
def lifecycle_destructor_worker(
    worker_id: int,
        ...
        while time.time() - destruction_start < config.test_duration * 0.5:
            ...
            for queue_id in targets:
                try:
                    ...
                    monitor.record_destruction(
                        queue_id, f"destructor_{worker_id}", time.time()
                    )
                    ...
                    monitor.record_error(
                        "destruction_failed", str(e), f"destructor_{worker_id}"
                    )
...
def resource_leak_detector_worker(
    config: LifecycleStressConfig, monitor: QueueLifecycleMonitor
) -> Dict[str, Any]:
    """资源泄漏检测工作进程"""
    try:
        # 尝试导入真实的 SageQueue
            ...
            try:
                ...
                if len(monitor.memory_samples) > 0:
                    ...
                    if cycle > 0 and len(monitor.memory_samples) > 10:
                        baseline_memory = monitor.memory_samples[
                            max(0, len(monitor.memory_samples) - 10)
                        ]["rss_mb"]
        ...
        return {"leak_detections": 0, "resource_warnings": 1, "fatal_error": str(e)}
...
class TestLifecycleStress:
    ...
    def test_concurrent_creation_destruction(self):
        ...
        try:
            with ProcessPoolExecutor(
                max_workers=config.concurrent_creators + config.concurrent_destructors
            ) as executor:
                futures = []
                ...
                for i in range(config.concurrent_creators):
                    future = executor.submit(
                        lifecycle_creator_worker, i, config, monitor
                    )
                ...
                for i in range(config.concurrent_destructors):
                    future = executor.submit(
                        lifecycle_destructor_worker,
                        i + config.concurrent_creators,
                        config,
                        monitor,
                        delay=5.0 + i * 2.0,
                    )  # 错开启动时间
    ...
    def test_resource_leak_detection(self):
        ...
        try:
            with ProcessPoolExecutor(
                max_workers=config.concurrent_creators + 2
            ) as executor:
                futures = []
    ...
    def test_rapid_lifecycle_cycles(self):
        ...
        def rapid_cycle_worker(worker_id: int) -> Dict[str, Any]:
            ...
            try:
                ...
                while time.time() - stats["start_time"] < config.test_duration:
                    queue_id = (
                        f"rapid_{worker_id}_{cycle_count}_{int(time.time()*1000000)}"
                    )
                    ...
                    try:
                        ...
                        monitor.record_creation(
                            queue_id, f"rapid_{worker_id}", time.time()
                        )
                        ...
                        monitor.record_error(
                            "rapid_cycle_failed", str(e), f"rapid_{worker_id}"
                        )
        ...
        try:
            with ProcessPoolExecutor(
                max_workers=config.concurrent_creators
            ) as executor:
                futures = []
            ...
            try:
                ...
                while time.time() - stats["start_time"] < config.test_duration:
                    try:
                        ...
                        monitor.record_creation(
                            queue_id, f"pressure_{worker_id}", time.time()
                        )
                        ...
                        if len(active_queues) > 10:
                            ...
                            try:
                                ...
                                old_queue.destroy()
                                monitor.record_destruction(
                                    old_queue_id, f"pressure_{worker_id}", time.time()
                                )
                            except:
                                pass
                        ...
                        monitor.record_error(
                            "memory_pressure_failed", str(e), f"pressure_{worker_id}"
                        )
    ...
    def _analyze_lifecycle_stress_results(
        self,
        ...
        destruction_errors = sum(
            stat["destruction_errors"] for stat in destructor_stats
        )
        ...
        print(
            f"  - 成功率: {(total_destroyed/(total_destroyed+destruction_errors))*100:.1f}%"
        )
        ...
        assert (
            creation_errors < total_created * 0.1
        ), f"创建错误率过高: {creation_errors}/{total_created}"
        assert (
            destruction_errors < total_destroyed * 0.1
        ), f"销毁错误率过高: {destruction_errors}/{total_destroyed}"

    def _analyze_leak_detection_results(
        self,
        all_stats: List[Dict],
        monitor: QueueLifecycleMonitor,
        config: LifecycleStressConfig,
        ...
        leak_detector_stats = [
            s for s in all_stats if s.get("worker_type") == "leak_detector"
        ]
        ...
        if len(memory_samples) > 10:
            ...
            # 检查内存增长是否超过阈值
            assert (
                abs(memory_growth) < config.max_memory_growth_mb
            ), f"内存增长超过阈值: {memory_growth:.2f} > {config.max_memory_growth_mb} MB"

    def _analyze_rapid_cycle_results(
        self, rapid_stats: List[Dict], monitor: QueueLifecycleMonitor
    ):
        """分析快速循环结果"""
        print("\n=== 快速生命周期循环测试结果 ===")
        ...
        if rapid_stats:
            avg_duration = sum(stat.get("duration", 0) for stat in rapid_stats) / len(
                rapid_stats
            )
        ...
        assert (
            total_errors < total_cycles * 0.05
        ), f"快速循环错误率过高: {total_errors}/{total_cycles}"

    def _analyze_memory_pressure_lifecycle_results(
        self,
        pressure_stats: List[Dict],
        monitor: QueueLifecycleMonitor,
        config: LifecycleStressConfig,
    ):
        """分析内存压力生命周期结果"""
        ...
        if memory_samples:
            ...
            # 检查内存使用是否合理
            assert (
                final_memory - initial_memory < config.max_memory_growth_mb
            ), f"内存增长超过限制: {final_memory - initial_memory:.2f} > {config.max_memory_growth_mb} MB"

        # 断言验证
        assert total_cycles > 0, "没有完成任何内存压力循环"
        assert (
            total_errors < total_cycles * 0.1
        ), f"内存压力错误率过高: {total_errors}/{total_cycles}"
