...
def lifecycle_stress_worker(worker_id: int, cycles: int, results_file: str) -> None:
    ...
    try:
        ...
        for cycle in range(cycles):
            queue_name = (
                f"lifecycle_test_{worker_id}_{cycle}_{int(time.time()*1000000)}"
            )
            queue = None
...
class TestSimpleMultiprocessStress:
    ...
    def test_simple_concurrent_read_write_stress(self):
        ...
        try:
            queue_names = [
                f"simple_stress_queue_{i}_{int(time.time())}"
    ...
    def test_simple_lifecycle_management_stress(self):
        """测试简化生命周期管控压力"""
        config = SimpleStressConfig(num_processes=4, test_duration=30)
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=config.num_processes) as executor:
                ...
                for i in range(config.num_processes):
                    future = executor.submit(
                        lifecycle_stress_worker, i, 30, results_file  # cycles
                    )
                    futures.append(future)
    ...
    def test_mixed_workload_stress(self):
        """测试混合工作负载压力"""
        config = SimpleStressConfig(
            num_processes=6, num_queues=3, messages_per_process=150, test_duration=35
        )
        ...
        try:
            queue_names = [
                f"mixed_stress_queue_{i}_{int(time.time())}"
                for i in range(config.num_queues)
            ]
    ...
    def _analyze_simple_stress_results(self, all_results: List[Dict[str, Any]]):
        ...
        producer_results = [
            r for r in all_results if r.get("worker_type") == "producer"
        ]
        consumer_results = [
            r for r in all_results if r.get("worker_type") == "consumer"
        ]
        ...
        if total_sent > 0:
            assert (
                producer_errors < total_sent * 0.2
    ...
    def _analyze_simple_lifecycle_results(self, all_results: List[Dict[str, Any]]):
        ...
        lifecycle_results = [
            r for r in all_results if r.get("worker_type") == "lifecycle"
        ]
        ...
        destruction_errors = sum(
            r.get("destruction_errors", 0) for r in lifecycle_results
        )
    ...
    def _analyze_mixed_workload_results(self, all_results: List[Dict[str, Any]]):
        ...
        successful_processes = len(
            [r for r in all_results if not r.get("fatal_error", False)]
        )
        success_rate = (
