...
def stress_producer_worker_func(
    worker_id: int, queue_names: List[str], message_count: int, message_size: int
) -> Dict[str, Any]:
    """压力测试生产者工作进程（独立函数）"""
    try:
        # 尝试导入真实的 SageQueue
...
def stress_consumer_worker_func(
    worker_id: int, queue_names: List[str], expected_messages: int, timeout: float
) -> Dict[str, Any]:
    """压力测试消费者工作进程（独立函数）"""
    try:
        # 尝试导入真实的 SageQueue
        ...
        while received_count < expected_messages:
            ...
            for queue_name, queue in queues.items():
                try:
                    ...
                    # 验证消息完整性
                    if hasattr(message, "payload") and isinstance(
...
class MultiprocessStressTester:
                        ...
                        self.shared_state["memory_samples"].append(
                            {
                                "timestamp": time.time(),
                                "memory_mb": memory_mb,
                                "active_processes": self.shared_state.get(
                                    "active_processes", 0
                                ),
                            }
                        )
        ...
        try:
            ...
            for i in range(message_count):
                ...
                try:
                    ...
                    # 定期更新共享统计
                    if i % 100 == 0:
                        self.shared_state["total_sent"] = (
                            self.shared_state.get("total_sent", 0) + 100
                        )
            ...
            self.shared_state["active_processes"] = (
                self.shared_state.get("active_processes", 1) - 1
            )
            return stats
    ...
    def stress_consumer_worker(
        ...
        try:
            ...
            while received_count < expected_messages and not self.shared_state.get(
                "stop_signal", False
            ):
                if time.time() - start_time > timeout:
                    break
                ...
                for queue_name, queue in queues.items():
                    try:
                        ...
                        if received_count % 100 == 0:
                            self.shared_state["total_received"] = (
                                self.shared_state.get("total_received", 0) + 100
                            )
...
class TestMultiprocessStress:
    ...
    def test_concurrent_read_write_stress(self):
        ...
        with ProcessPoolExecutor(max_workers=config.num_processes) as executor:
            ...
            expected_per_consumer = (
                num_producers * config.messages_per_process
            ) // num_consumers
        ...
        self._analyze_stress_results(
            producer_stats, consumer_stats, tester.shared_state
        )

    def test_lifecycle_management_stress(self):
        """测试多进程生命周期管控压力"""
        config = StressTestConfig(
            num_processes=6, lifecycle_cycles=50, test_duration=60
        )
        ...
        with ProcessPoolExecutor(max_workers=config.num_processes) as executor:
            ...
            for i in range(config.num_processes):
                future = executor.submit(
                    tester.lifecycle_stress_worker, i, config.lifecycle_cycles
                )
    ...
    def _analyze_stress_results(
        self, producer_stats: List[Dict], consumer_stats: List[Dict], shared_state: Dict
    ):
        """分析压力测试结果"""
        print("\n=== 多进程并发读写压力测试结果 ===")
        ...
        avg_producer_throughput = total_sent / max(
            stat.get("duration", 1) for stat in producer_stats
        )
        ...
        avg_consumer_throughput = total_received / max(
            stat.get("duration", 1) for stat in consumer_stats
        )
        ...
        if memory_samples:
            ...
            avg_memory = sum(sample["memory_mb"] for sample in memory_samples) / len(
                memory_samples
            )
            print(f"内存使用:")
        ...
        assert (
            producer_errors < total_sent * 0.1
        ), f"生产者错误率过高: {producer_errors}/{total_sent}"
        assert (
            consumer_errors < total_received * 0.1
        ), f"消费者错误率过高: {consumer_errors}/{total_received}"

    def _analyze_lifecycle_results(
        self, lifecycle_stats: List[Dict], shared_state: Dict
    ):
        """分析生命周期测试结果"""
        print("\n=== 多进程生命周期管控压力测试结果 ===")
        ...
        if memory_samples:
            memory_trend = [
                sample["memory_mb"] for sample in memory_samples[-10:]
            ]  # 最后10个样本
            memory_growth = (
                memory_trend[-1] - memory_trend[0] if len(memory_trend) > 1 else 0
            )
        ...
        assert (
            destruction_errors < total_cycles * 0.05
        ), f"队列销毁失败率过高: {destruction_errors}/{total_cycles}"

    def _analyze_memory_pressure_results(
        self, all_stats: List[Dict], shared_state: Dict, config: StressTestConfig
    ):
        """分析内存压力测试结果"""
        print("\n=== 内存压力下多进程操作测试结果 ===")
        ...
        for phase, stats in phases.items():
            total_ops = sum(
                stat.get("messages_sent", 0) + stat.get("messages_received", 0)
                for stat in stats
            )
        ...
        if memory_samples:
            ...
            # 检查内存阈值
            assert (
                peak_memory < config.memory_threshold_mb
            ), f"内存使用超过阈值: {peak_memory} > {config.memory_threshold_mb} MB"

    def _analyze_extreme_concurrency_results(
        self, extreme_stats: List[Dict], shared_state: Dict
    ):
        """分析极限并发测试结果"""
        print("\n=== 极限并发场景测试结果 ===")
        ...
        successful_processes = len(
            [s for s in extreme_stats if not s.get("fatal_error", False)]
        )
        success_rate = (
            successful_processes / total_processes if total_processes > 0 else 0
        )

        print(
            f"系统稳定性: {success_rate:.2%} ({successful_processes}/{total_processes})"
        )
