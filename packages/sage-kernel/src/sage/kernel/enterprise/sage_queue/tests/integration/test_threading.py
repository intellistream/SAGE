...
from ..utils import ConcurrencyTester, DataGenerator


def producer_thread(
    sage_queue, num_messages: int, message_prefix: str, results: list, errors: list
):
...
class TestMultithreadingBasic:
    ...
    def test_multiple_producers_single_consumer(self, large_queue, thread_helper):
        ...
        def safe_consumer(queue, num_msgs, consumer_id):
            ...
            thread_errors = []
            consumer_thread(
                queue, num_msgs, consumer_id, results, thread_errors, timeout=15.0
            )
        ...
        for i in range(num_producers):
            thread_helper[0](
                safe_producer, args=(large_queue, messages_per_producer, f"prod{i}")
            )
    ...
    def test_single_producer_multiple_consumers(self, large_queue, thread_helper):
        ...
        def safe_consumer(queue, max_msgs, consumer_id):
            ...
            thread_errors = []
            consumer_thread(
                queue, max_msgs, consumer_id, results, thread_errors, timeout=15.0
            )
        ...
        for i in range(num_consumers):
            ...
            thread_helper[0](
                safe_consumer, args=(large_queue, total_messages, f"cons{i}")
            )
        ...
        assert len(producer_results) == total_messages
        assert (
            len(consumer_results) == total_messages
        )  # Total consumed should equal produced

        # All produced messages should be consumed (no duplicates)
        assert len(set(consumer_results)) == len(
            consumer_results
...
class TestMultithreadingStress:
    ...
    def test_high_throughput_threading(self, large_queue):
        ...
        with ThreadPoolExecutor(max_workers=num_producers + num_consumers) as executor:
            ...
            consumer_futures = []
            for i in range(num_consumers):
                expected = total_messages // num_consumers + (
                    1 if i < (total_messages % num_consumers) else 0
                )
                future = executor.submit(
                    self._high_throughput_consumer, large_queue, expected, f"cons{i}"
                )
