...
def multiprocess_writer_worker(
    queue_name: str, worker_id: int, num_messages: int
) -> Dict[str, Any]:
...
def multiprocess_reader_worker(
    queue_name: str, worker_id: int, expected_messages: int, timeout: float = 30.0
) -> Dict[str, Any]:
...
def concurrent_rw_worker(
    queue_name: str, worker_id: int, num_operations: int, read_write_ratio: float = 0.5
) -> Dict[str, Any]:
    """并发读写混合操作 worker"""
    try:
        queue = SageQueue(queue_name)
        ...
        return {
            ...
            "duration": duration,
            "ops_per_sec": (
                (writes_completed + reads_completed) / duration if duration > 0 else 0
            ),
            "success": True,
        }
...
def queue_reference_worker(
    queue_ref_data: Dict[str, Any], worker_id: int, num_operations: int
) -> Dict[str, Any]:
...
def test_multiprocess_producer_consumer():
    ...
    try:
        ...
        print(
            f"配置: {num_producers} 生产者 × {messages_per_producer} = {total_messages} 消息"
        )
        ...
        with ProcessPoolExecutor(max_workers=num_producers + num_consumers) as executor:
            ...
            for i in range(num_producers):
                future = executor.submit(
                    multiprocess_writer_worker, queue_name, i, messages_per_producer
                )
                futures.append(future)

            # 启动消费者
            for i in range(num_consumers):
                expected = messages_per_consumer + (
                    total_messages % num_consumers if i == 0 else 0
                )
                future = executor.submit(
        ...
        print(
            f"  消息完整率: {total_consumed/total_produced*100:.1f}%"
            if total_produced > 0
            else "  消息完整率: N/A"
        )
...
def test_concurrent_read_write():
    ...
    try:
        ...
        print(
            f"启动 {num_workers} 个并发读写进程，每个执行 {operations_per_worker} 操作"
        )
        print(
            f"读写比例: {read_write_ratio*100:.0f}% 读, {(1-read_write_ratio)*100:.0f}% 写"
        )
        ...
        avg_ops_per_sec = (
            sum(w["ops_per_sec"] for w in successful_workers) / len(successful_workers)
        ...
        print(
            f"  最终统计: 写入={final_stats['total_bytes_written']}, 读取={final_stats['total_bytes_read']}"
        )
...
def test_queue_reference_passing():
    ...
    try:
        ...
        print(
            f"启动 {num_workers} 个进程通过引用访问队列，每个执行 {operations_per_worker} 操作"
        )
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for worker_id in range(num_workers):
                future = executor.submit(
                    queue_reference_worker, ref_data, worker_id, operations_per_worker
                )
                futures.append(future)
        ...
        for w in successful_workers:
            print(
                f"  进程{w['worker_id']}: {w['completed_ops']} ops, {w['ops_per_sec']:.0f} ops/s"
            )
