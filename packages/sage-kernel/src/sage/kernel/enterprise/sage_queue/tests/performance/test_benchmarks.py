...
from ..utils import (DataGenerator, PerformanceCollector,
                     generate_test_data_sizes, measure_time)
...
class TestThroughputBenchmarks:
    ...
    def test_single_thread_throughput(
        self, large_queue, message_size, performance_monitor
    ):
        ...
        assert (
            put_throughput >= min_throughput
        ), f"Put throughput too low: {put_throughput:.2f} msg/s"
        assert (
            get_throughput >= min_throughput
        ), f"Get throughput too low: {get_throughput:.2f} msg/s"
    ...
    def test_multithreaded_throughput(self, large_queue, performance_monitor):
        ...
        min_throughput = PERFORMANCE_BENCHMARKS["min_throughput_msg_per_sec"]
        max_latency = (
            PERFORMANCE_BENCHMARKS["max_latency_ms"] / 1000
        )  # Convert to seconds

        assert (
            aggregate_throughput >= min_throughput
        ), f"Aggregate throughput too low: {aggregate_throughput:.2f} msg/s"
        assert (
            avg_put_latency <= max_latency
        ), f"Put latency too high: {avg_put_latency:.6f}s"
        assert (
            avg_get_latency <= max_latency
        ), f"Get latency too high: {avg_get_latency:.6f}s"
...
class TestLatencyBenchmarks:
    ...
    def test_roundtrip_latency(self, medium_queue):
        ...
        ), f"Average latency too high: {avg_latency:.6f}s"
        assert (
            p95_latency <= max_latency * 2
        ), f"P95 latency too high: {p95_latency:.6f}s"
    ...
    def test_concurrent_latency(self, large_queue):
        ...
        assert (
            avg_latency <= max_concurrent_latency
        ), f"Concurrent average latency too high: {avg_latency:.6f}s"
...
class TestMemoryEfficiency:
    ...
    def test_memory_usage_scaling(self, performance_monitor):
        ...
        for size in queue_sizes:
            ...
            try:
                while (
                    items_added < size // 200
                ):  # Don't completely fill to avoid blocking
    ...
    def test_memory_leak_detection(self, performance_monitor):
        ...
        if len(memory_samples) >= 3:
            ...
            ), f"Potential memory leak: {memory_growth_mb:.2f}MB growth"
...
class TestStressBenchmarks:
    ...
    def test_sustained_high_load(self, large_queue, performance_monitor):
        ...
        stats = {"messages_produced": 0, "messages_consumed": 0, "errors": []}
        ...
        avg_throughput = (stats["messages_produced"] + stats["messages_consumed"]) / (
            2 * actual_duration
        )
        error_rate = len(stats["errors"]) / (
            stats["messages_produced"] + len(stats["errors"])
        )
        ...
        # Performance assertions
        min_sustained_throughput = (
            PERFORMANCE_BENCHMARKS["min_throughput_msg_per_sec"] * 0.8
        )  # Allow 20% reduction under stress
        assert (
            avg_throughput >= min_sustained_throughput
        ), f"Sustained throughput too low: {avg_throughput:.2f} msg/s"
        assert (
            error_rate <= 0.05
        ), f"Error rate too high: {error_rate:.3%}"  # Max 5% error rate
    ...
    def test_burst_load_handling(self, large_queue):
        ...
        assert (
            total_received >= total_sent * 0.95
        ), f"Too many messages lost: {total_received}/{total_sent}"
