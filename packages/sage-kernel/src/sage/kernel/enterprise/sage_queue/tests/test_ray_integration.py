...
class DataProducerActor:
    ...
    def produce_data(
        self, queue_ref: SageQueueRef, batch_size: int, batch_count: int
    ) -> Dict[str, Any]:
        """生产数据到队列"""
        print(
            f"Producer {self.actor_id}: 开始生产 {batch_count} 批数据，每批 {batch_size} 条"
        )
        ...
        try:
            for batch_idx in range(batch_count):
                ...
                if (batch_idx + 1) % 10 == 0 or batch_idx == batch_count - 1:
                    ...
                    print(
                        f"Producer {self.actor_id}: 进度 {progress:.1f}% ({batch_idx + 1}/{batch_count} 批次)"
                    )
            ...
            print(
                f"✓ Producer {self.actor_id}: 完成，发送 {total_messages} 条消息，耗时 {duration:.3f}s，速率 {rate:.0f} msg/s"
            )
            queue.close()
            return result
...
class DataConsumerActor:
    ...
    def consume_data(
        self, queue_ref: SageQueueRef, expected_messages: int, timeout: float = 30.0
    ) -> Dict[str, Any]:
        """从队列消费数据"""
        print(
            f"Consumer {self.actor_id}: 开始消费，预期 {expected_messages} 条消息，超时 {timeout}s"
        )
        ...
        try:
            while len(consumed_messages) < expected_messages:
                ...
                if elapsed > timeout:
                    print(
                        f"Consumer {self.actor_id}: 超时，只消费了 {len(consumed_messages)}/{expected_messages} 条消息"
                    )
                    break
                ...
                try:
                    ...
                    if len(consumed_messages) % 100 == 0:
                        ...
                        print(
                            f"Consumer {self.actor_id}: 进度 {progress:.1f}% ({len(consumed_messages)}/{expected_messages})"
                        )
            ...
            result = {
                ...
                "success_rate": (
                    len(consumed_messages) / expected_messages
            ...
            print(
                f"✓ Consumer {self.actor_id}: 完成，消费 {len(consumed_messages)} 条消息，耗时 {duration:.3f}s，速率 {rate:.0f} msg/s"
            )
...
class DataProcessorActor:
    ...
    def process_data(
        self,
        input_queue_ref: SageQueueRef,
        output_queue_ref: SageQueueRef,
        ...
        try:
            while processed_messages < expected_messages:
                ...
                if elapsed > timeout:
                    print(
                        f"Processor {self.actor_id}: 超时，只处理了 {processed_messages}/{expected_messages} 条消息"
                    )
                    break
                ...
                try:
                    ...
                    processed_message = {
                        ...
                        "processing_result": {
                            ...
                            "avg": sum(message["data"]["values"])
                            / len(message["data"]["values"]),
                    ...
                    if processed_messages % 50 == 0:
                        ...
                        print(
                            f"Processor {self.actor_id}: 进度 {progress:.1f}% ({processed_messages}/{expected_messages})"
                        )
            ...
            result = {
                ...
                "success_rate": (
                    processed_messages / expected_messages
                    if expected_messages > 0
                    else 0
                ),
            }
            ...
            print(
                f"✓ Processor {self.actor_id}: 完成，处理 {processed_messages} 条消息，耗时 {duration:.3f}s，速率 {rate:.0f} msg/s"
            )
...
def test_simple_producer_consumer():
    ...
    try:
        ...
        print(
            f"配置: {batch_count} 批次 × {batch_size} 消息 = {total_messages} 条总消息"
        )
...
def test_multiple_producers_consumers():
    ...
    try:
        ...
        print(
            f"配置: {producer_count} 个生产者 × {messages_per_producer} 消息 = {total_messages} 条总消息"
        )
        print(
            f"      {consumer_count} 个消费者，每个预期消费 ~{messages_per_consumer} 条消息"
        )

        # 创建Actor
        producers = [
            DataProducerActor.remote(f"producer_{i}") for i in range(producer_count)
        ]
        consumers = [
            DataConsumerActor.remote(f"consumer_{i}") for i in range(consumer_count)
        ]
        ...
        print(
            f"  消息丢失率: {(total_produced - total_consumed) / total_produced * 100:.2f}%"
        )
        ...
        for i, result in enumerate(consumer_results):
            print(
                f"  消费者{i}: {result['rate']:.0f} msg/s, 成功率: {result['success_rate']:.1%}"
            )
...
def test_pipeline_processing():
    ...
    try:
        ...
        print(
            f"配置: 生产 {total_messages} 条消息 -> {processor_count} 个处理器 -> 1 个最终消费者"
        )

        # 创建Actor
        producer = DataProducerActor.remote("pipeline_producer")
        processors = [
            DataProcessorActor.remote(f"processor_{i}") for i in range(processor_count)
        ]
        ...
        # 阶段1: 生产数据到输入队列
        print("阶段1: 启动生产者")
        producer_task = producer.produce_data.remote(
            input_queue_ref, batch_size, batch_count
        )

        # 阶段2: 处理器从输入队列读取，处理后写入输出队列
        print("阶段2: 启动处理器")
        processor_tasks = [
            processor.process_data.remote(
                input_queue_ref, output_queue_ref, messages_per_processor, 45.0
            )
        ...
        # 阶段3: 最终消费者从输出队列读取
        print("阶段3: 启动最终消费者")
        consumer_task = final_consumer.consume_data.remote(
            output_queue_ref, total_messages, 60.0
        )
        ...
        print(
            f"  端到端成功率: {consumer_result['consumed_messages'] / producer_result['total_messages']:.1%}"
        )

        for i, result in enumerate(processor_results):
            print(
                f"  处理器{i}: {result['rate']:.0f} msg/s, 成功率: {result['success_rate']:.1%}"
            )
