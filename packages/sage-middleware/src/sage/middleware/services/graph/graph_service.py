...
class MemoryGraphBackend:
    ...
    def create_relationship(self, rel: GraphRelationship) -> str:
        ...
        if len(self.relationships) >= self.max_relationships:
            raise RuntimeError(
                f"Maximum relationships limit reached: {self.max_relationships}"
            )
...
class GraphService(BaseService):
    ...
    def __init__(self, service_factory: "ServiceFactory", ctx: "ServiceContext" = None):
        ...
        if self.config.backend_type == "neo4j":
            if not all(
                ...
                raise ValueError(
                    "Neo4j URI, user and password required for Neo4j backend"
                )
            ...
            self.logger.info(
                f"Graph Service '{self.service_name}' initialized with Neo4j backend"
            )
        else:
            self.backend = MemoryGraphBackend(
                self.config.max_nodes, self.config.max_relationships
            )
            self.logger.info(
                f"Graph Service '{self.service_name}' initialized with memory backend"
            )
    ...
    def create_node(
        ...
        node = GraphNode(id=node_id or "", labels=labels, properties=properties)
    ...
    def create_relationship(
        ...
        """创建关系"""
        self.logger.debug(
            f"Creating relationship: {from_node} -[{rel_type}]-> {to_node}"
        )
    ...
    def get_node_context(self, node_id: str, depth: int = 1) -> Dict[str, Any]:
        ...
        if not context["node"]:
            return context
        rels = (
            self.get_node_relationships(node_id, direction="both")
    ...
    def find_nodes(
        ...
        """查找节点"""
        self.logger.debug(
            f"Finding nodes with labels: {labels}, properties: {properties}"
        )
    ...
    def get_node_relationships(
        ...
        """获取节点的关系"""
        self.logger.debug(
            f"Getting relationships for node: {node_id}, direction: {direction}"
        )
