...
class FaissVDBBackend:
    ...
    def __init__(
        ...
        # 导入FaissIndex
        try:
            from sage.middleware.services.vdb.search_engine.faiss_index import \
                FaissIndex
            ...
            raise ImportError(
                "FAISS dependencies not available. Install with: pip install faiss-cpu"
            )
        ...
        try:
            ...
            for doc in vectors:
                ...
                if len(doc.vector) != self.embedding_dimension:
                    self.logger.warning(
                        f"Vector dimension mismatch: expected {self.embedding_dimension}, got {len(doc.vector)}"
                    )
                    continue
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "faiss_index_ntotal": (
                    getattr(self.faiss_index.index, "ntotal", 0)
...
class VDBService(BaseService):
    ...
    def __init__(self, service_factory: "ServiceFactory", ctx: "ServiceContext" = None):
        ...
        self.logger.info(
            f"VDB Service '{self.service_name}' initialized with FAISS backend"
        )
        self.logger.info(
            f"Index type: {self.config.index_type}, Dimension: {self.config.embedding_dimension}"
        )
