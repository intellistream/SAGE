...
class MemoryService(BaseService):
    ...
    def __init__(
        ...
        if (
            ...
            and hasattr(service_factory, "config")
            and service_factory.config is not None
        ):
        ...
        # 日志配置预览
        self.logger.info(
            f"KV Service: {getattr(self.config, 'kv_service_name', 'kv_service')}"
        )
        self.logger.info(
            f"VDB Service: {getattr(self.config, 'vdb_service_name', 'vdb_service')}"
        )
        self.logger.info(
            f"Graph Service: {getattr(self.config, 'graph_service_name', 'graph_service')}"
        )
    ...
    def _get_graph_service(self):
        ...
        if not hasattr(self, "ctx") or self.ctx is None:
            raise RuntimeError("Service context not available")
        return self.ctx.service_manager.get_service_proxy(
            self.config.graph_service_name
        )
        ...
        try:
            ...
            if create_knowledge_graph and self.config.enable_knowledge_graph:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to create knowledge graph for memory {memory_id}: {e}"
                    )
    ...
    def search_memories(
        ...
        try:
            ...
            if similarity_threshold is not None:
                search_results = [
                    r for r in search_results if r["distance"] <= similarity_threshold
                ]
                ...
                if memory_data:
                    ...
                    if include_graph_context and self.config.enable_knowledge_graph:
                        try:
                            graph_service = self._get_graph_service()
                            # 获取该记忆的图上下文（相邻节点和关系）
                            graph_context = graph_service.get_node_context(
                                memory_id, depth=1
                            )
                            enhanced_memory["graph_context"] = graph_context
                        except Exception as e:
                            self.logger.warning(
                                f"Failed to get graph context for {memory_id}: {e}"
                            )
                    ...
                    self.logger.warning(
                        f"Memory {memory_id} found in VDB but not in KV"
                    )
    ...
    def get_memory(
        self, memory_id: str, include_graph_context: bool = False
        ...
        try:
            ...
            if self.config.enable_knowledge_graph:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to delete graph node for {memory_id}: {e}"
                    )
    ...
    def get_session_memories(
        ...
        try:
            ...
            self.logger.debug(
                f"GET session memories: {session_id}, found {len(memories)} memories"
            )
            return result
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                kv_service = self._get_kv_service()
                service_stats["kv"] = (
                    kv_service.stats() if hasattr(kv_service, "stats") else {}
                )
            except Exception as e:
            ...
            try:
                vdb_service = self._get_vdb_service()
                service_stats["vdb"] = (
                    vdb_service.stats() if hasattr(vdb_service, "stats") else {}
                )
            except Exception as e:
            ...
            if self.config.enable_knowledge_graph:
                try:
                    graph_service = self._get_graph_service()
                    service_stats["graph"] = (
                        graph_service.stats() if hasattr(graph_service, "stats") else {}
                    )
                except Exception as e:
