...
class REFORMCompressor:
    ...
    def __init__(
        ...
        logger.info(f"Compression params: max_tokens={max_tokens}, prefix={keep_prefix}, suffix={keep_suffix}")
        logger.info(f"KV cache optimization: {'enabled' if use_kv_cache else 'disabled'}")
        logger.info(f"Chunking: {'enabled' if enable_chunking else 'disabled'}, chunk_size={self.chunk_size}")
    ...
    def compress(
        ...
        """
        logger.debug(f"REFORMCompressor.compress called with context length: {len(context)}, question: '{question[:50]}...'")
    ...
    def _compress_chunked(
        ...
        """
        logger.info(f"Starting chunked compression: {len(context_tokens)} tokens, chunk_size={self.chunk_size}")
        ...
        for i, chunk_info in enumerate(chunks):
            logger.debug(f"Processing chunk {i+1}/{len(chunks)}: tokens {chunk_info['global_start']}-{chunk_info['global_end']}")
            try:
        ...
        logger.info(f"Chunked compression: {original_tokens} → {compressed_tokens} tokens ({compression_rate:.2%}), {num_spans} spans, {len(chunks)} chunks")
        logger.debug(f"Selected {len(selected_tokens)} tokens, recomputed to {actual_token_count} actual tokens")
    ...
    def _process_chunk(self, chunk_info: dict, question: str) -> list[tuple]:
        ...
        if self.use_kv_cache:
            ...
            question_len = question_range[1] - question_range[0]
    ...
    def _global_top_k_selection(
        ...
        suffix = [(s, p, t) for s, p, t in all_scores if p >= total_context_tokens - self.keep_suffix]
        middle = [(s, p, t) for s, p, t in all_scores 
        ...
        if remaining_budget <= 0:
            logger.warning(f"Prefix+suffix already exceed budget: {len(selected)} > {self.max_tokens}")
    ...
    def _merge_spans_from_scores(self, selected_tokens: list[tuple]) -> tuple[str, int]:
        ...
        span_texts = []
        for span in spans:
            span_text = self.model_extractor.tokenizer.decode(
