...
class NeuroMemVDB:
    ...
    def register_collection(self, collection_name: str, config=None):
        ...
        if collection_exists:
            ...
            if collection:
                ...
                self.logger.info(f"成功连接已存在的collection: {collection_name}")
        else:
            if config is not None:
                ...
                if collection:
                    self.online_register_collection[collection_name] = collection
                    self.logger.info(f"成功创建新collection: {collection_name}")
            else:
                ...
                if collection:
                    self.online_register_collection[collection_name] = collection
                    self.logger.info(f"成功创建默认collection: {collection_name}")
    ...
    def insert(self, raw_data: Any, metadata: Optional[Dict[str, Any]] = None):
        ...
        if not self.online_register_collection:
            self.logger.info("警告: 没有注册的collection，无法插入数据")
            return
    ...
    def retrieve(self, raw_data: Any, topk: int = 5, index_name: str = "global_index"):
        ...
        if not self.online_register_collection:
            self.logger.info("警告: 没有注册的collection，无法检索数据")
            return

        self.logger.info(f"检索查询: {raw_data}")
        self.logger.info("=" * 50)
        ...
        for collection_name, collection in self.online_register_collection.items():
            try:
                ...
                self.logger.info(f"Collection '{collection_name}' 检索结果:")
                if results:
                    for i, result in enumerate(results, 1):
                        self.logger.info(f"  {i}. 文本: {result['text']}")
                        if result["metadata"]:
                            self.logger.info(f"     元数据: {result['metadata']}")
                else:
                    self.logger.info("  无结果")
                self.logger.info("-" * 30)
            except Exception as e:
                self.logger.info(f"Collection '{collection_name}' 检索失败: {str(e)}")
                self.logger.info("-" * 30)
    ...
    def build_index(
        ...
        if not self.online_register_collection:
            self.logger.info("警告: 没有注册的collection，无法创建索引")
            return
    ...
    def store_to_disk(self):
        ...
        try:
            ...
            self.logger.info("成功将所有collection数据保存到磁盘")
        except Exception as e:
            self.logger.info(f"保存数据到磁盘失败: {str(e)}")
    ...
    def clean_collection(self, collection_name: str):
        ...
        try:
            ...
            if collection_name in self.online_register_collection:
                ...
                self.logger.info(f"已从在线注册列表中移除collection: {collection_name}")

            # 通过manager删除collection（包括磁盘数据）
            self.manager.delete_collection(collection_name)
            self.logger.info(f"成功清理collection: {collection_name}")
        except Exception as e:
            self.logger.info(f"清理collection '{collection_name}' 失败: {str(e)}")
...
if __name__ == "__main__":
    self.logger.info("默认数据目录:", NeuroMemVDB._get_default_data_dir())
    self.logger.info("\n" + "=" * 60)
    self.logger.info("开始测试 NeuroMemVDB")
    self.logger.info("=" * 60)
