"""MultiIndex Memory Service - 多索引混合存储

Layer: L4 (Middleware)

实现算法：
- Mem0: 多索引混合记忆（VDB + KV）+ CRUD 决策
- Mem0ᵍ: 图增强版本（VDB + KV + Graph）
- EmotionalRAG: 双向量索引（语义 + 情感）

设计原则：
- Service : Collection = 1 : 1（单一 HybridCollection）
- 一份数据 + 多种索引（VDB / KV / Graph）
- 支持融合检索（weighted / rrf / union）

特性：
1. 多索引支持：语义向量、情感向量、BM25 关键词、图索引
2. 融合策略：weighted（加权融合）、rrf（倒数排名融合）、union（合并）
3. 被动插入模式：自动检测相似项，触发 CRUD 决策
4. 灵活配置：可动态指定索引配置和融合参数
...
class MultiIndexMemoryService(BaseMemoryService):
    """多索引混合记忆服务
    ...
    支持的模式：
    1. 基础混合检索（Mem0）：VDB + KV（BM25）
    2. 图增强版本（Mem0ᵍ）：VDB + KV + Graph
    3. 双向量索引（EmotionalRAG）：语义向量 + 情感向量
    4. 自定义组合：任意 VDB/KV/Graph 索引组合

    融合策略：
    - weighted: 加权融合（需指定 fusion_weights）
    - rrf: 倒数排名融合（Reciprocal Rank Fusion，默认 k=60）
    - union: 合并所有结果（不排序）
    """
    ...
    def __init__(
        ...
        Args:
            indexes: 索引配置列表，每个配置包含:
                - name: 索引名称（如 "semantic", "keyword", "entity_graph"）
            ...
            fusion_strategy: 融合策略 ("weighted" | "rrf" | "union")
                ...
                - rrf: 倒数排名融合（推荐，无需权重）
                - union: 合并所有结果
            fusion_weights: 索引权重 {"index_name": weight}（仅 weighted 策略需要）
            rrf_k: RRF 参数（仅 rrf 策略使用，默认 60）
        ...
        self.logger.info(
            f"MultiIndexMemoryService initialized: strategy={fusion_strategy}, "
    ...
    def from_config(cls, service_name: str, config: Any) -> ServiceFactory:
        ...
        Args:
            service_name: 服务名称（如 "hybrid.multi_index"）
        ...
        # 读取配置
        indexes = config.get(f"services.{service_name}.indexes")
        fusion_strategy = config.get(f"services.{service_name}.fusion_strategy", "rrf")
        fusion_weights = config.get(f"services.{service_name}.fusion_weights")
        rrf_k = config.get(f"services.{service_name}.rrf_k", 60)
        collection_name = config.get(
            f"services.{service_name}.collection_name", f"hybrid_{service_name.replace('.', '_')}"
        )
        graph_enabled = config.get(f"services.{service_name}.graph_enabled", False)
        entity_extraction = config.get(f"services.{service_name}.entity_extraction", False)
        relation_extraction = config.get(f"services.{service_name}.relation_extraction", False)
    ...
    def insert(
        ...
        *,
        ...
        支持两种插入模式：
        - passive: 由服务自行决定存储方式（默认插入所有索引，检测相似项）
        ...
        Args:
            ...
            metadata: 元数据，可包含:
                - vectors: 多个向量 {"index_name": vector}（如 EmotionalRAG）
    ...
    def _update_pending_status(
        ...
        try:
            similar_results = self.retrieve_single(
                ...
                index_name=(
                    self.index_configs[0].get("name") if self.index_configs else "semantic"
                ),
                top_k=10,
            )
