...
try:
    from python.sage_db import SageDB, DatabaseConfig, IndexType, DistanceMetric
except ImportError:
...
def benchmark_multi_thread(db: SageDB, num_threads: int, queries_per_thread: int = 500, 
                          dimension: int = 768) -> Tuple[float, float]:
...
def benchmark_batch_search(db: SageDB, batch_size: int = 100, num_batches: int = 10, 
                           dimension: int = 768) -> float:
...
def main():
    ...
    for num_threads, data in results.items():
        ...
        print(f"{num_threads:<10} {data['total_qps']:<12.1f} {speedup:<12.2f}x {data['per_thread_qps']:<15.1f}")
