...
auto FileDataSource::is_initialized() const -> bool { return initialized_; }

auto FileDataSource::is_closed() const -> bool { return closed_; }
...
auto FileDataSource::get_reading_stats() const -> FileDataSourceStats {
  return FileDataSourceStats{.total_files_ = file_list_.size(),
...
auto FileDataSource::scan_files(const std::string& path,
                                bool recursive) -> std::vector<std::string> {
      ...
      if (recursive) {
        for (const auto& entry :
          ...
          if (entry.is_regular_file() &&
              is_supported_format(entry.path().string())) {
...
auto FileDataSource::is_supported_format(const std::string& file_path) const
    -> bool {
