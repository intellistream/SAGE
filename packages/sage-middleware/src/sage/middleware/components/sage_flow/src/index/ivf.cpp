...
IVF::IVF(std::shared_ptr<MemoryPool> memory_pool, int num_clusters,
         double rebuild_threshold, int nprobe)
...
auto IVF::AddVector(uint64_t id, const std::vector<float>& vector) -> bool {
  ...
  double growth_ratio = static_cast<double>(vectors_since_last_rebuild_) /
                        static_cast<double>(vectors_.size());
  if (growth_ratio > rebuild_threshold_ &&
      vectors_.size() >= static_cast<size_t>(num_clusters_)) {
    rebuild_clusters();
...
auto IVF::RemoveVector(uint64_t id) -> bool {
  ...
  if (cluster_it != vector_to_cluster_.end()) {
    ...
    cluster_list.erase(
        std::remove(cluster_list.begin(), cluster_list.end(), id),
        cluster_list.end());
...
auto IVF::Search(const std::vector<float>& query_vector,
...
auto IVF::SaveIndex(const std::string& file_path) const -> bool {
  ...
  // Save parameters
  file.write(reinterpret_cast<const char*>(&num_clusters_),
             sizeof(num_clusters_));
  file.write(reinterpret_cast<const char*>(&rebuild_threshold_),
             sizeof(rebuild_threshold_));
  file.write(reinterpret_cast<const char*>(&nprobe_), sizeof(nprobe_));
  file.write(reinterpret_cast<const char*>(&vectors_since_last_rebuild_),
             sizeof(vectors_since_last_rebuild_));
  ...
  file.write(reinterpret_cast<const char*>(&centroid_count),
             sizeof(centroid_count));
  ...
  for (const auto& [cluster_id, vector_ids] : inverted_lists_) {
    ...
    file.write(reinterpret_cast<const char*>(vector_ids.data()),
               static_cast<std::streamsize>(id_count * sizeof(uint64_t)));
  }
...
auto IVF::LoadIndex(const std::string& file_path) -> bool {
  ...
  file.read(reinterpret_cast<char*>(&rebuild_threshold_),
            sizeof(rebuild_threshold_));
  file.read(reinterpret_cast<char*>(&nprobe_), sizeof(nprobe_));
  file.read(reinterpret_cast<char*>(&vectors_since_last_rebuild_),
            sizeof(vectors_since_last_rebuild_));
  ...
  for (size_t i = 0; i < centroid_count; i++) {
    ...
    file.read(reinterpret_cast<char*>(centroids_[i].data()),
              static_cast<std::streamsize>(dim * sizeof(float)));
  }
  ...
  for (size_t i = 0; i < list_count; i++) {
    ...
    file.read(reinterpret_cast<char*>(vector_ids.data()),
              static_cast<std::streamsize>(id_count * sizeof(uint64_t)));
...
auto IVF::Size() const -> size_t { return vectors_.size(); }
...
auto IVF::GetType() const -> IndexType { return IndexType::kIvf; }

// Private helper methods
auto IVF::l2_distance(const std::vector<float>& a,
...
void IVF::kmeans_clustering() {
  ...
  for (int i = 0; i < actual_clusters; i++) {
    centroids_[static_cast<size_t>(i)] =
        vectors_.at(vector_ids[static_cast<size_t>(i)]);
  }
  ...
  while (changed && iteration < max_iterations) {
    ...
    for (int i = 0; i < actual_clusters; i++) {
      if (cluster_counts[static_cast<size_t>(i)] > 0) {
        for (size_t j = 0; j < new_centroids[static_cast<size_t>(i)].size();
             j++) {
          new_centroids[static_cast<size_t>(i)][j] /=
              static_cast<float>(cluster_counts[static_cast<size_t>(i)]);
        }
        centroids_[static_cast<size_t>(i)] =
            new_centroids[static_cast<size_t>(i)];
      }
    }
...
auto IVF::search_clusters(const std::vector<float>& query_vector,
                          int nprobe) const -> std::vector<int> {
  if (centroids_.empty()) {
    return {};
  }
  ...
  int actual_nprobe =
      std::min(nprobe, static_cast<int>(centroid_distances.size()));
