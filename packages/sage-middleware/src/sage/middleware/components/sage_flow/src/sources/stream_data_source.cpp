#include "sources/stream_data_source.hpp"
...
auto StreamDataSource::is_initialized() const -> bool { return initialized_; }

auto StreamDataSource::is_closed() const -> bool { return closed_; }

auto StreamDataSource::get_streaming_stats() const -> StreamDataSourceStats {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  return StreamDataSourceStats{
...
void StreamDataSource::start_streaming() {
  ...
  consumer_thread_ =
      std::make_unique<std::thread>(&StreamDataSource::consume_loop, this);
}
