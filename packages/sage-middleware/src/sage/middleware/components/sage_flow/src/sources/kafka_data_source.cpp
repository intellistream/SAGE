...
#include "sources/kafka_data_source.hpp"

#include <chrono>
#include <thread>

#include "message/multimodal_message.hpp"
#include "message/multimodal_message_factory.hpp"
#include "sources/streaming_stats.hpp"
...
auto KafkaDataSource::is_initialized() const -> bool { return initialized_; }

auto KafkaDataSource::is_closed() const -> bool { return closed_; }

auto KafkaDataSource::get_kafka_stats() const -> KafkaDataSourceStats {
  std::lock_guard<std::mutex> lock(buffer_mutex_);
  std::lock_guard<std::mutex> error_lock(error_mutex_);

  return KafkaDataSourceStats{.topic_ = topic_,
...
void KafkaDataSource::lazy_init() {
  ...
  try {
    ...
    consumer_thread_ =
        std::make_unique<std::thread>(&KafkaDataSource::consume_loop, this);

  } catch (const std::exception& e) {
    handle_consumer_error("Lazy initialization failed: " +
                          std::string(e.what()));
  }
}
    ...
    try {
      ...
      if (++msg_counter % 5 == 0) {  // Generate message every 5 iterations
        std::string simulated_message =
            "kafka_message_" + std::to_string(msg_counter) + "_topic_" +
            topic_ + "_group_" + consumer_group_;
      ...
      if (running_.load()) {
        handle_consumer_error("Error in consume loop: " +
                              std::string(e.what()));
...
auto KafkaDataSource::parse_kafka_config() -> bool {
  ...
  try {
    ...
    std::string timeout_str =
        config_.get_property("consumer_timeout_ms", "1000");
...
auto KafkaDataSource::deserialize_message(const std::string& raw_message)
    -> std::string {
