...
auto BruteForceIndex::AddVector(uint64_t id,
                                const std::vector<float>& vector) -> bool {
...
auto BruteForceIndex::Search(const std::vector<float>& query_vector,
  ...
  std::partial_sort(
      results.begin(),
      results.begin() +
          static_cast<std::ptrdiff_t>(std::min(k, results.size())),
...
auto BruteForceIndex::CalculateCosineSimilarity(
...
auto BruteForceIndex::CalculateEuclideanDistance(
    const std::vector<float>& vec1,
    const std::vector<float>& vec2) const -> float {
  if (vec1.size() != vec2.size()) {
