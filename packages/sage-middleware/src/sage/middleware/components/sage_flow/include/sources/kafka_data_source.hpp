...
#include "kafka_data_source_stats.hpp"
...
public:
   ...
   */
  auto get_kafka_stats() const -> KafkaDataSourceStats;
...
private:
  ...
  void
  consume_loop();  // Background thread main loop (like SAGE's _consume_loop)
  void cleanup_resources();  // Resource cleanup

  // Kafka-specific helpers
  auto parse_kafka_config() -> bool;
  auto create_kafka_consumer()
      -> bool;  // Platform-specific Kafka consumer creation
