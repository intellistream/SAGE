...
public:
  explicit IVF(std::shared_ptr<MemoryPool> memory_pool, int num_clusters = 100,
               double rebuild_threshold = 0.5, int nprobe = 10);
  ...
  auto Search(const std::vector<float>& query_vector,
              size_t k) const -> std::vector<SearchResult> override;
...
private:
  ...
  std::unordered_map<int, std::vector<uint64_t>>
      inverted_lists_;  // Cluster -> vector IDs
  std::unordered_map<uint64_t, std::vector<float>>
      vectors_;  // Store all vectors
  std::unordered_map<uint64_t, int>
      vector_to_cluster_;              // Vector ID -> cluster ID
  ...
  auto search_clusters(const std::vector<float>& query_vector,
