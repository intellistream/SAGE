...
class MemoryOrchestratorService(BaseServiceTask):
    ...
    def store_memory(
        ...
        try:
            ...
            vdb_success = vdb_service.add_vectors(vdb_data)
            if not vdb_success:
                # 回滚KV存储
    ...
    def search_memories(
        ...
        try:
            ...
            search_results = vdb_service.search_vectors(
                ...
                metadata_filter=where_conditions if where_conditions else None
            )
            
            # 过滤分数
            if score_threshold is not None:
                search_results = [r for r in search_results if r['distance'] <= score_threshold]
