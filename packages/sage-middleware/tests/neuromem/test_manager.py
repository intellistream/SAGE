...
def test_neuromem_manager():
    ...
    vdb_collection.insert(index_name="test_index", raw_data="将军，您的恩情我们一辈子也还不完！", metadata={"priority": "high"})
    vdb_collection.insert(index_name="test_index", raw_data="我从丹东来带走一片雪白~", metadata={"priority": "low", "tag": "poem"})
    vdb_collection.insert(index_name="test_index", raw_data="想吃广东菜", metadata={"priority": "low", "tag": "food"})

    results = vdb_collection.retrieve(index_name="test_index", raw_data="广东菜", with_metadata=True)
