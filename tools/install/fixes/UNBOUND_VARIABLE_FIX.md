...
2. `CI`, `GITHUB_ACTIONS` - CI/CD 环境检测变量
3. `VIRTUAL_ENV`, `CONDA_DEFAULT_ENV`, `CONDA_PREFIX` - 虚拟环境检测变量
4. 其他可能未设置的环境变量
...
2. **CI 测试**: 在 CI/CD 流程中运行 `test_environment_doctor.sh`
3. **文档更新**: 在 DEVELOPER.md 中添加脚本开发最佳实践
...
2. **安全访问**: 使用 `${VAR:-default}` 语法访问环境变量
3. **测试覆盖**: 为关键脚本编写测试用例
4. **错误处理**: 不盲目使用 `set -u`，而是有针对性地进行变量检查
5. **文档说明**: 在脚本中添加注释说明变量的用途和默认值
