...
class SAGEInstaller:
    ...
    def __init__(self, project_root: str = None, use_curses: bool = True):
        ...
        self.env_manager = EnvironmentManager(
            str(self.project_root), ui=self.ui)
        self.dependency_checker = DependencyChecker(
            str(self.project_root), ui=self.ui)
        self.submodule_manager = SubmoduleManager(
            str(self.project_root), ui=self.ui)
  ...
  python3 install.py --quiet             # 静默模式
    ...
    def interactive_setup(self):
        ...
        if not self.config["env_name"]:
            profile_suffix = get_profile(
                self.config["profile"]).environment_suffix
        ...
        try:
            ...
            if passed == total:
                ...
                if not self.config["quiet_mode"]:
                    report = self.dependency_checker.generate_check_report(
                        checks)
                    print("\n" + report)
    ...
    def setup_environment(self) -> bool:
        ...
        self.ui.show_info(
            f"   强制重装: {'是' if self.config['force_reinstall'] else '否'}")
    ...
    def install_packages(self) -> bool:
        ...
        package_installer = PackageInstaller(
            str(self.project_root), env_vars, ui=self.ui)
        ...
        if "use_requirements" in profile.additional_config:
            ...
            requirements_path = self.project_root / \
                "scripts" / "requirements" / requirements_file

            self.progress.start_step(
                "requirements_install", f"安装requirements: {requirements_file}")
            ...
            if requirements_path.exists():
                if package_installer.install_requirements_file(str(requirements_path)):
                    ...
                    self.progress.fail_step(
                        "requirements_install", "Requirements安装失败")
                    return False
            else:
                self.progress.fail_step(
                    "requirements_install", f"Requirements文件不存在: {requirements_path}")
                return False
        ...
        if pip_packages:
            ...
            results = package_installer.install_packages(
                pip_packages, use_conda=False)
            failed_packages = [pkg for pkg,
            ...
            if not failed_packages:
                ...
                self.progress.fail_step(
                    "pip_packages", f"{len(failed_packages)} 个包安装失败")
    ...
    def install_stage2_packages(self) -> bool:
        ...
        results = package_installer.install_packages(
            stage2_packages, use_conda=False)
        failed_packages = [pkg for pkg,
                           success in results.items() if not success]
    ...
    def show_completion_info(self):
        ...
        self.ui.show_info(
            f"🔸 测试安装: python -c 'import sage; print(\"SAGE运行正常!\")'")
        self.ui.show_info(
            f"🔸 验证vllm: python -c 'import vllm; print(\"vLLM已安装:\", vllm.__version__)'")
    ...
    def run(self, args: argparse.Namespace = None) -> bool:
        ...
        try:
            ...
            if args.list_profiles:
                ...
                self.ui.pause("\033[1m\033[34m按 Enter 键返回主菜单...\033[0m")
                return True
            ...
            # 执行安装步骤
            self.logger.info(
                f"开始SAGE安装 - 配置: {self.config['profile']}, 环境: {self.config['env_name']}")
