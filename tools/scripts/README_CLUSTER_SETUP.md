...
2. **Ray 版本自动同步** - 启动 worker 前自动检查并同步 Ray 版本
...
1. **首次使用**：先配置 SSH 免密登录
2. **版本同步**：保持所有节点的 Ray 版本一致
3. **日志查看**：定期检查 `/tmp/sage_worker_logs/worker.log`
4. **资源监控**：通过 Ray Dashboard 监控集群状态（http://localhost:8265）
5. **清理进程**：出现问题时使用 `sage cluster stop` 完全清理
