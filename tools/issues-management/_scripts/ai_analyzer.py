        ...
        for label, keys in keyword_map.items():
            for k in keys:
                if k in title:
                    recs.add(label)
        if recs:
            suggestions.append({'number': issue.get('number'), 'title': issue.get('title'), 'recommend': list(recs)})
    return suggestions
...
def assess_priority(issues: list):
    ...
    for issue in issues:
        ...
        if any(k in title for k in ('critical', 'urgent', 'blocker')):
            ...
            priority = 'medium'
        results.append({'number': issue.get('number'), 'title': issue.get('title'), 'priority': priority})
    return results


def main():
    parser = argparse.ArgumentParser(description="AI 智能分析 Issues")
    parser.add_argument("--mode", choices=["duplicates", "labels", "priority", "comprehensive"], required=True)
    ...
    if args.mode == 'duplicates':
        ...
        for a, b, reason in dups:
            print(f" - #{a.get('number')} {a.get('title')[:80]} <-> #{b.get('number')} {b.get('title')[:80]} ({reason})")
        # 写入报告
        out = config.output_path / f"duplicates_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        with open(out, 'w', encoding='utf-8') as f:
            f.write(f"# 重复Issues分析\n\n共 {len(dups)} 组\n\n")
            for a, b, reason in dups:
                f.write(f"- #{a.get('number')} {a.get('title')} <-> #{b.get('number')} {b.get('title')} ({reason})\n")
        print(f"报告已保存: {out}")

    elif args.mode == 'labels':
        suggestions = suggest_label_optimizations(issues)
        print(f"为 {len(suggestions)} 个Issues 推荐标签")
        out = config.output_path / f"label_suggestions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        with open(out, 'w', encoding='utf-8') as f:
            f.write(f"# 标签优化建议\n\n共 {len(suggestions)} 条建议\n\n")
            for s in suggestions:
                f.write(f"- #{s.get('number')}: {s.get('title')} -> {', '.join(s.get('recommend', []))}\n")
        print(f"报告已保存: {out}")

    elif args.mode == 'priority':
        res = assess_priority(issues)
        out = config.output_path / f"priority_assessment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        with open(out, 'w', encoding='utf-8') as f:
            f.write(f"# 优先级评估\n\n共 {len(res)} 条\n\n")
            for r in res:
