...
def assert_frozen(points: list[dict[str, float]]) -> None:
    for key, expected in EXPECTED.items():
        ...
        if len(actual) != len(expected) or any(abs(a - e) > 5e-5 for a, e in zip(actual, expected)):
            raise SystemExit(f"frozen-value mismatch for {key}: expected {expected}, got {actual}")


def coords(values: list[float], x0: float, y0: float, width: float, height: float, ymax: float) -> str:
    pieces = []
...
def panel(
    *,
    ...
    lines = [
        f"\\node[font=\\bfseries\\small] at ({x0 - 0.28:.3f},{y0 + height + 0.32:.3f}) {{{label}}};",
    ]
    for tick in yticks:
        ...
        lines.extend(
            [
                ...
                f"\\node[anchor=east, font=\\scriptsize] at ({x0 - 0.10:.3f},{y:.3f}) {{{tick_label}}};",
            ]
        )
    lines.extend(
        [
    ...
    for index, budget in enumerate((4, 8, 12)):
        ...
        lines.extend(
            [
                ...
                f"\\node[anchor=north, font=\\scriptsize] at ({x:.3f},{y0 - 0.10:.3f}) {{{budget}}};",
            ]
        )
    lines.extend(
        [
            f"\\node[font=\\scriptsize] at ({x0 + width / 2:.3f},{y0 - 0.48:.3f}) {{Candidate budget}};",
            f"\\node[rotate=90, font=\\scriptsize] at ({x0 - 0.92:.3f},{y0 + height / 2:.3f}) {{{ylabel}}};",
        ]
    )
        ...
        lines.extend(
            [
                ...
                *[
                ...
                f"\\node[anchor=west, font=\\scriptsize] at ({legend_x + 0.47:.3f},{legend_y - 0.30 * idx:.3f}) {{{name}}};",
            ]
        )
    return lines
...
def main() -> None:
    ...
    values = lambda key: [point[key] for point in points]
    lines = [
