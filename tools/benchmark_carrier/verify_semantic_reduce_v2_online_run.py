...
def _independent_request_failures(
    protocol: dict[str, Any], protocol_sha: str, request: dict[str, Any], grant: dict[str, Any]
) -> list[str]:
    failures: list[str] = []

    def expect(value: bool, name: str) -> None:
        if not value:
    ...
    expect(set(request) == top_keys, "schema")
    expect(request.get("schema_version") == "semantic-reduce-v2-reservation-request/1", "version")
    expect(request.get("request_id") == f"semantic-reduce-v2-{protocol_sha[:12]}", "id")
    ...
    try:
        ...
        issued = _timestamp(grant.get("issued_at_utc"))
        expect(requested <= issued < expires, "chronology")
        expect(
            expires - requested
            == timedelta(minutes=protocol["reservation_shape"]["request_ttl_minutes"]),
            "ttl",
        )
    ...
    shape, service = protocol["reservation_shape"], protocol["service"]
    expect(
    ...
    expect(
        request.get("protocol") == {
            "path": request.get("protocol", {}).get("path"),
            "sha256": protocol_sha,
            "status": protocol["status"],
        }
        and Path(str(request.get("protocol", {}).get("path", ""))).name
        == "v2_real_online_protocol.json", "protocol",
    )
    ...
    if isinstance(reviews, list):
        for item in reviews:
            ...
            expect(
                ...
                and declared_digest == hashlib.sha256(canonical).hexdigest(),
                "review-sha",
            )
            ...
            expect(envelope.get("protocol_sha256") == protocol_sha, "review-protocol")
            expect(envelope.get("execution_commit") == protocol["repository"]["execution_commit"], "review-commit")
            expect(envelope.get("reviewed_frozen_sources") is True, "review-sources")
            ...
            try:
                ...
                expect(requested is not None and reviewed <= requested, "review-chronology")
            except (TypeError, ValueError, OverflowError):
                failures.append("review-timestamp")
    expect(roles == {"systems-novelty", "experiment-statistics-provenance", "artifact-fail-closed"}, "review-roles")
    ...
    expect(request.get("queue_base") == {**protocol["queue_base_observation"], "mutation_performed": False}, "queue-base")
    ...
    expect(
        request.get("cleanup_release_chain") == [
...
def verify_run(
    ...
    *,
    ...
    request_contract_failures = _independent_request_failures(
        protocol, expected_protocol_sha256, reservation_request, grant
    )
    request_checks["full-envelope-contract"] = not request_contract_failures
    if failed := [name for name, passed in request_checks.items() if not passed]:
        _fail(
            "reservation request provenance mismatch: "
            + ",".join(failed)
            + "; contract="
