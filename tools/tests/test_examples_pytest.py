...
from example_strategies import ExampleTestStrategies, ExampleTestFilters, ExampleEnvironmentManager
...
class TestExamplesIntegration:
    ...
    def test_category_examples(self, example_suite, category):
        ...
        assert success_rate >= expected_rate, f"类别 {category} 成功率应该至少 {expected_rate*100}%"
    ...
    def test_all_quick_examples(self, example_suite):
        ...
        # 快速示例的整体成功率应该较高
        success_rate = stats["passed"] / stats["total"] if stats["total"] > 0 else 0
        assert success_rate >= 0.6, f"快速示例整体成功率应该至少60%，实际: {success_rate*100:.1f}%"
    ...
    def test_example_categorization(self, analyzer):
        ...
        for example in examples:
            ...
            assert example.category in path_parts, f"类别 {example.category} 应该在路径中: {example.file_path}"
    ...
    def test_examples_integration_with_issues_manager(self):
        ...
        if example_stats["failed"] > 0:
            # 这里可以集成到问题管理系统
            assert example_stats["failed"] < example_stats["total"], "不应该所有示例都失败"
