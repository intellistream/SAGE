...
# 调整T5 model 的编码器和解码器，实现 Fusion-in-Decoder Transformer
class FiDT5(transformers.T5ForConditionalGeneration):
    def __init__(self, config):
        """
        Initializes the FiDT5 reranker model with the given configuration.

        Args:
            config (dict): Configuration dictionary for the model.

        Notes:
            - Wraps the encoder to obtain a Fusion-in-Decoder model.
            - Sets the encoder's embedding tokens to be shared. But need to correct!
        """
        super().__init__(config) 
        self.wrap_encoder()   # Wrap the encoder to obtain a Fusion-in-Decoder model
        self.encoder.embed_tokens = self.shared   # need to 

    @classmethod
    def from_pretrained(cls, *args, **kwargs):
        model = super(FiDT5, cls).from_pretrained(*args, **kwargs)
        model.encoder.embed_tokens = model.shared  # 确保从预训练模型加载时也添加 embed_tokens 属性
        return model
...
class EncoderWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        # make outputs to [bsize*n_psgs, max_len->k, hidden(768)]
        # [bsize, npsg*output_k, hidden_state]
        # [bsize, npsg*maxlen, hidden_state 756] 
        # bsize, max_len, hidden_state  ] npsg
        # bsize, output_k, hidden_state
