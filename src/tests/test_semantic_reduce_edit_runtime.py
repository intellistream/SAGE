...
def test_true_split_executes_catalog_partition_and_conserves_evidence() -> None:
    ...
    source = set(
        next(c for c in h0.candidates if c.candidate_id == split.candidate_ids[0]).evidence_ids
    )
    produced = [
    ...
    assert {frozenset(part) for part in produced} == {frozenset(part) for part in split.parts}
...
def test_invalid_split_plan_rolls_back(parts, reason) -> None:
    ...
    forged_catalog = ProposalCatalog(
        proposals=tuple(
            forged if p.proposal_id == forged.proposal_id else p for p in catalog.proposals
        ),
...
def test_unknown_proposal_malformed_and_request_failure_have_distinct_outcomes() -> None:
    ...
    failed = runtime.execute(dataset.evidence, CallbackProposalSelector(fail, name="failing-model"))
...
def test_merge_conserves_exact_union() -> None:
    dataset = generate_semantic_merge_dataset(seed=7, scenario="ambiguous-disconnected-merge")
    runtime = _runtime()
    h0 = runtime.build_h0(dataset.evidence)
    catalog = runtime.build_catalog(h0, dataset.evidence)
    merge = _proposal(catalog, "MERGE")
    sources = {candidate.candidate_id: set(candidate.evidence_ids) for candidate in h0.candidates}
    result = runtime.commit_selection(
        evidence=dataset.evidence,
        h0=h0,
        catalog=catalog,
...
def test_catalog_and_trace_claim_only_executable_bounded_actions() -> None:
    ...
    assert (
        set(result.trace["bounded_actions"])
...
def test_each_oracle_free_split_generator_emits_a_real_partition() -> None:
    conflict_dataset = generate_semantic_merge_dataset(seed=7, scenario="ambiguous-overmerge")
    ...
    disconnected_items = [
        replace(
            conflict_dataset.evidence[0],
            evidence_id="topology-a",
            service="embedding",
            upstream_hint=None,
        ),
        replace(
            conflict_dataset.evidence[1],
            evidence_id="topology-b",
            service="kv-cache",
            upstream_hint=None,
        ),
    ]
