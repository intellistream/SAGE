...
def test_semantic_merge_suite_covers_representative_scenarios() -> None:
    for scenario in SCENARIOS:
        dataset = generate_semantic_merge_dataset(seed=7, shard_count=8, scenario=scenario)
...
def test_semantic_graph_reducer_beats_fragment_baselines_on_hard_scenarios() -> None:
    ...
    baseline_scores = []
    for scenario in scenarios:
        map_only = run_semantic_merge_workload(seed=7, reducer="map-only", scenario=scenario)
        service_local = run_semantic_merge_workload(
            seed=7, reducer="service-local", scenario=scenario
        )
        window = run_semantic_merge_workload(seed=7, reducer="window-aggregate", scenario=scenario)
        semantic = run_semantic_merge_workload(seed=7, reducer="semantic-graph", scenario=scenario)
...
def test_ambiguous_candidate_stress_scenarios_have_covered_hybrid_blind_spots() -> None:
    for scenario in ("ambiguous-disconnected-merge", "ambiguous-temporal-split"):
        semantic = run_semantic_merge_workload(seed=7, reducer="semantic-graph", scenario=scenario)
        hybrid = run_semantic_merge_workload(seed=7, reducer="hybrid-hint", scenario=scenario)
...
def test_semantic_merge_report_records_trace_cost_and_failure_taxonomy() -> None:
    ...
    assert payload["reducer_trace"]["output_hypothesis_count"] == len(payload["detected_incidents"])
    ...
    assert all("failure_type" in item for item in payload["false_positive_incidents"])
...
def test_openai_hybrid_merge_reducer_edits_graph_candidates_only() -> None:
    dataset = generate_semantic_merge_dataset(seed=7, shard_count=8, scenario="partial-evidence")
...
def test_openai_hybrid_validated_reducer_repairs_or_falls_back() -> None:
    ...
    assert any(item["action"] == "drop-suppressed" for item in reducer.last_call["edit_trace"])
    ...
    for candidate_index, candidate in enumerate(graph_candidates):
        ...
        for evidence_id in candidate.get("evidence_ids", []):
            ...
            if evidence.source_incident_id:
                by_source.setdefault(evidence.source_incident_id, []).append(evidence.evidence_id)
        if len(by_source) <= 1:
...
def test_openai_hybrid_validated_reducer_accepts_evidence_preserving_merge() -> None:
    ...
    for candidate_index, source_id in enumerate(source_by_candidate):
        ...
        merge_indices = [
            idx
            for idx, other_source in enumerate(source_by_candidate)
...
def test_openai_pairwise_action_validated_reducer_assembles_legal_edits() -> None:
    ...
    assert all(
        item["response_text"] in {"KEEP", "MERGE"} for item in reducer.last_call["request_trace"]
    )
...
def test_openai_pairwise_action_invalid_output_abstains_without_fallback() -> None:
    ...
    assert reducer.last_call["fallback_count"] == 0
    assert reducer.last_call["invalid_action_count"] == reducer.last_call["input_pair_count"]
