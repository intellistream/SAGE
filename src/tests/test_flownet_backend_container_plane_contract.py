...
def test_backend_container_plane_supports_capability_selection_and_submit_poll_cancel() -> None:
    ...
    try:
        ...
        assert gpu_backend.submitted_requests[-1]["backend_selection"][
            "selected_backend_state"
        ]["runtime_metrics"]["free_vram_bytes"] == 96_000_000_000
        assert gpu_backend.submitted_requests[-1]["backend_selection"][
            "candidate_backend_states"
        ][0]["backend_id"] == "fake-gpu-backend"
...
def test_runtime_host_submit_backend_job_auto_polls_selected_backend() -> None:
    ...
    try:
        ...
        blocked_backend = FakeBackendContainer("fake-runtime-backend-blocked", queue_capacity=0, epoch=11)
        runtime_host.register_backend_container(
            backend,
