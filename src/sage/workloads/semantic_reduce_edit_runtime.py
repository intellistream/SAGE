...
def _state_from_hypotheses(
    ...
    candidates = tuple(
        Candidate(candidate_id=f"C{index}", hypothesis=item)
...
class BoundedEditRuntime:
    ...
    def build_catalog(
        ...
        for left_index, left in enumerate(h0.candidates):
            ...
            for right in h0.candidates[left_index + 1 :]:
                ...
                if len(left_regions | right_regions) != 1:
                    continue
                right_hints = {
                    item.upstream_hint for item in right_items if item.upstream_hint
                }
        ...
        generators = (
            ...
            (
                "temporal-gap",
                lambda items: _temporal_gap_partition(
                    items, gap_minutes=self.temporal_gap_minutes
                ),
            ),
            (
                "topology-disconnected",
                lambda items: _topology_partition(
                    items, topology=self.service_topology
                ),
            ),
        )
        for candidate in h0.candidates:
            ...
            for proposal in proposals:
                ...
                if proposal.action == "SPLIT":
                    ...
                    for part in proposal.parts:
                        output.append(
                            self._hypothesis_for_ids(
                                set(part), evidence_by_id, action="split"
                            )
                        )
    ...
    def _preserved_result(
        ...
        *,
        ...
        trace = self._trace(
            ...
            validator_outcome=(
                "not-invoked"
                if outcome in {"preserved-abstain", "preserved-no-proposal", "preserved-request-failure"}
                else "rejected"
            ),
    ...
    def restore_checkpoint(checkpoint: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(h0, dict) or not isinstance(catalog, dict) or not isinstance(selected, list):
