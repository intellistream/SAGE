...
def score_state(
    ...
    matched_incidents, matched_hypotheses, _ = score_merge_hypotheses(
        ...
        *,
        ...
        proposals = sorted(
            ...
            key=lambda proposal: (
                ...
                and proposal.generator == "conflicting-upstream-hints"
                else 1
                if proposal.action == "MERGE"
                and dict(proposal.metadata).get("same_non_null_hint")
                else 2,
...
def _gold_proposal_coverage(
    ...
    source_by_evidence = {
        item.evidence_id: item.source_incident_id for item in evidence
    }
    ...
    gold_split_candidates = {
        candidate_id
        for candidate_id, sources in candidate_sources.items()
        if len(sources) >= 2
    }
...
def _conservation(result: EditRuntimeResult) -> bool:
    before = sorted(
        evidence_id
        for candidate in result.h0.candidates
        for evidence_id in candidate.evidence_ids
    )
    after = sorted(
        evidence_id
...
def evaluate_workload(
    ...
    *,
    ...
    constrained = ConstrainedAgglomerativeMergeReducer().reduce(
        workload.dataset.evidence
    )
    scores = {
        "h0": score_state(h0_result.hypotheses, workload.dataset.incidents),
        "oracle": oracle_score,
        "deterministic": score_state(
            deterministic_result.hypotheses, workload.dataset.incidents
        ),
