...
def _prefixed(dataset: MergeDataset, *, prefix: str, region: str, minute_offset: int) -> MergeDataset:
    incident_ids = {
        incident.incident_id: f"{prefix}-{incident.incident_id}"
        for incident in dataset.incidents
    }
    incidents = [
        replace(
...
def generate_heldout_workload(
    ...
    *,
    ...
    if family == "hint-missing":
        ...
        metadata.update({"missing_edge": ["scheduler", "decode"], "stale_edge": ["embedding", "kv-cache"]})
...
def field_separability(workload: HeldoutWorkload, *, score_threshold: float = 0.5) -> dict[str, Any]:
    ...
    true_retention = sum(item.score >= score_threshold for item in true_items) / max(1, len(true_items))
    noise_rejection = sum(item.score < score_threshold for item in noise_items) / max(1, len(noise_items))
    return {
