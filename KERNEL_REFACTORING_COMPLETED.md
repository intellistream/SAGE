...
**迁移建议**:
1. 短期：无需修改，兼容层保证向后兼容
2. 中期：逐步更新为 `sage.common` 导入
3. 长期：移除兼容层（在 v1.0 或下一个大版本）
...
2. **向后兼容**: 兼容层避免破坏现有代码
3. **充分测试**: 每个阶段都验证
4. **批量操作**: sed 命令高效处理大量文件
5. **文档优先**: 先分析再执行

### ⚠️ 遇到的挑战
1. **类名不一致**: BaseJoinFunction vs JoinFunction
2. **循环导入**: 初期文件仍引用 kernel
3. **隐式依赖**: wrap_lambda 未在 __init__.py 导出
4. **大量文件**: 60+ 文件需要更新导入
...
2. ✅ **清晰分层**: 函数接口位于 L1，kernel/libs 位于 L3
3. ✅ **向后兼容**: 现有代码无需立即修改
4. ✅ **依赖优化**: libs 核心功能不再依赖 kernel
