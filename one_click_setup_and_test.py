...
使用方法:
    ...
    python one_click_setup_and_test.py --discover-only
    python one_click_setup_and_test.py --test-only
"""
...
class TestFileCollector:
    ...
    def __init__(self, project_root: Path):
        ...
        self.non_test_patterns = [
            ...
            r'one_click_setup_and_test.*\.py$',  # 本脚本及其变体
        ]
    ...
    def collect_test_files(self) -> List[Path]:
        ...
        for py_file in self.project_root.rglob('*.py'):
            ...
            if any(part in str(py_file) for part in [
                ...
                '_deps', 'third_party', 'external'  # 排除第三方依赖
            ]):
                continue
...
class OneClickSAGETester:
    ...
    def print_step(self, step: str, title: str):
        """打印步骤"""
        print(f"\n📋 步骤 {step}: {title}")
        print("-" * 40)
    ...
    def check_pytest_xdist(self, venv_python: Path) -> bool:
        """检查pytest-xdist是否可用"""
        try:
    ...
    def run_tests(self, workers: int = 16, quick_test: bool = False, test_files: List[Path] = None, categorized_tests: Dict[str, List[Path]] = None):
        ...
        # 检查是否支持并行测试
        use_parallel = self.check_pytest_xdist(venv_python)
        if not use_parallel:
            print("⚠️  pytest-xdist 不可用，将使用串行测试")
            workers = 1
        ...
        if quick_test:
            ...
            if test_files and len(test_files) > 0:
                ...
                pytest_cmd = f"{venv_python} -m pytest {' '.join(test_paths)} -v --tb=short"
                if use_parallel and workers > 1:
                    pytest_cmd += f" -n {min(workers, len(sample_tests))}"
            ...
            if test_files and len(test_files) > 0:
                ...
                if categorized_tests:
                    ...
                    for category, files in categorized_tests.items():
                        ...
                        pytest_cmd = f"{venv_python} -m pytest {' '.join(test_paths)} -v --tb=short"
                        if use_parallel and workers > 1:
                            pytest_cmd += f" -n {min(workers, len(files))}"
...
- **仅运行测试**: `python one_click_setup_and_test.py --test-only`
...
示例用法:
  ...
  python one_click_setup_and_test.py --test-only       # 仅运行测试
        """
    )
    ...
    try:
        if args.discover_only:
            ...
            if test_files:
                print(f"\n🎉 成功发现 {len(test_files)} 个测试文件")
            else:
                print("\n⚠️  未发现测试文件")
            success = len(test_files) > 0
        elif args.test_only:
            # 仅运行测试
            success = tester.run_only_tests(
