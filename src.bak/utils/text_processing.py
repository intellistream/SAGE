...
def process_text_to_embedding(text, max_length=512, stride=256):
    """
    Process a long text into a single embedding_model using chunking and aggregation.
    :param text: Input text to process.
    :param max_length: Maximum token length for each chunk.
    :param stride: Overlap between consecutive chunks.
    :return: Aggregated embedding_model as a PyTorch tensor.
    """
    ...
    try:
        ...
        for chunk in tokens["input_ids"]:
            # Generate embedding_model for each chunk
        ...
        if embeddings:
            ...
            logger.info(f"Processed text into embedding_model of shape: {aggregated_embedding.shape}")
        ...
        logger.error(f"Error processing text to embedding_model: {str(e)}")
        raise RuntimeError(f"Text processing failed: {str(e)}")


def process_session_text_to_embedding(session_texts):
    """
    Process session texts into a single embedding_model using parallel processing.
    """
