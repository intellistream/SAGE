CONDA ?= conda
CONDA_ENV_NAME ?= esage-vllm-hust-dev
CONDA_RUN ?= $(CONDA) run --no-capture-output -n $(CONDA_ENV_NAME)
PYTHON ?= $(CONDA_RUN) python
PIP ?= $(PYTHON) -m pip
PYTEST ?= $(PYTHON) -m pytest
SAGE_DEV ?= $(CONDA_RUN) sage-dev
SHARED_WORKLOAD_SEEDS ?= 7,11,13
SHARED_WORKLOAD_RUN_ID ?= make-shared-workload-probe
SEMANTIC_MR_RUN_ID ?= make-semantic-mr-offline-smoke

.PHONY: help install lint format test test-quick test-all quality clean clean-cache build publish check version docs build-extensions clean-env shared-workloads-probe semantic-mr-offline-smoke semantic-mr-offline-paper
...
help:
	...
	@echo ""
	@echo "🧾 Semantic MapReduce artifact gates:"
	@echo "  make shared-workloads-probe    - 运行 pinned llm-serving-workloads provenance probe"
	@echo "  make semantic-mr-offline-smoke - 运行 no-NPU Semantic MR smoke suite"
	@echo "  make semantic-mr-offline-paper - 运行 no-NPU Semantic MR paper suite"
	@echo ""
	@echo "🐍 默认 conda 环境: $(CONDA_ENV_NAME)"
	@echo ""
...
install:
	@echo "🚀 运行快速安装..."
	SAGE_ENV_NAME=$(CONDA_ENV_NAME) ./quickstart.sh
...
install-dev:
	...
	@echo ""
	@echo "📊 验证版本一致性..."
	@$(CONDA_RUN) pip list | grep -E "^isage"
...
publish:
	@echo "📦 发布到 TestPyPI..."
	$(SAGE_DEV) pypi publish --dry-run

publish-prod:
	@echo "📦 发布到生产 PyPI..."
	@$(SAGE_DEV) pypi publish

# 版本管理
version:
	@$(SAGE_DEV) version list

version-bump:
	$(SAGE_DEV) version bump

shared-workloads-probe:
	$(CONDA_RUN) env PYTHONPATH=src python tools/benchmark_carrier/run_shared_llm_serving_workload_probe.py \
		--seeds "$(SHARED_WORKLOAD_SEEDS)" \
		--run-id "$(SHARED_WORKLOAD_RUN_ID)"

semantic-mr-offline-smoke:
	$(CONDA_RUN) env PYTHONPATH=src python tools/benchmark_carrier/run_semantic_mapreduce_offline_suite.py \
		--profile smoke \
		--run-id "$(SEMANTIC_MR_RUN_ID)"

semantic-mr-offline-paper:
	$(CONDA_RUN) env PYTHONPATH=src python tools/benchmark_carrier/run_semantic_mapreduce_offline_suite.py \
		--profile paper
