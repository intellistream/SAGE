...
class ParallelProcessor(BaseFunction):
    ...
    def __init__(self, processor_name="Processor"):
        ...
        print(f"🔧 {self.processor_name} instance {self.instance_id} created in thread {self.thread_id}")
...
class MultiStreamCoMapProcessor(BaseCoMapFunction):
    ...
    def __init__(self):
        super().__init__()
        self.instance_id = id(self)
        print(f"🔧 CoMapProcessor instance {self.instance_id} created in thread {threading.get_ident()}")
...
def validate_single_stream_parallelism():
    ...
    result1 = (source_stream
               .map(ParallelProcessor, "Mapper", parallelism=3)     # 3 parallel mappers
               .filter(ParallelFilter, parallelism=2)              # 2 parallel filters
               .sink(ValidationSink, parallelism=1))               # 1 sink
...
def validate_execution_graph_nodes():
    ...
    numbers = [1, 2, 3, 4, 5]
    result = (env.from_collection(NumberListSource, numbers)
              .set_parallelism(2)
              .map(ParallelProcessor, "NodeTest")               # Should create 2 nodes
              .set_parallelism(3)
              .filter(ParallelFilter)                           # Should create 3 nodes
              .sink(ValidationSink, parallelism=1))             # Should create 1 node
    ...
    try:
        ...
        print(f"    Each transformation with parallelism=N will create N parallel operator nodes.")
...
def main():
    ...
    print(f"📊 Total transformations tested: {len(env1.pipeline) + len(env2.pipeline) + len(env3.pipeline)}")
    ...
    print(f"   - ExecutionGraph will create corresponding parallel nodes during execution")
