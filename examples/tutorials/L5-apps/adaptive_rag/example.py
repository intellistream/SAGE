...
sys.path.insert(0, str(_project_root / "packages" / "sage-common" / "src"))
sys.path.insert(0, str(_project_root / "packages" / "sage-kernel" / "src"))

# 添加当前目录以支持相对导入
_current_dir = Path(__file__).parent
sys.path.insert(0, str(_current_dir.parent))  # L5-apps
sys.path.insert(0, str(_current_dir.parent.parent))  # tutorials
...
def example_pipeline_run():
    ...
    from adaptive_rag.pipeline import PipelineConfig
...
def example_custom_classifier():
    ...
    from adaptive_rag.classifier import (
        BaseClassifier,
...
def example_strategy_functions():
    ...
    from adaptive_rag.functions import (
...
from tutorials.L5_apps.adaptive_rag import AdaptiveRAGPipeline, build_sage_pipeline
