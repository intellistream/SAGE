...
class ArxivSearchTool:
    ...
    def call(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            items = self._search_arxiv(query=query, size=size, max_results=max_results, with_abstract=with_abs)
            return {"output": items, "meta": {"query": query, "size": size, "max_results": max_results}}
    ...
    # === 具体抓取 ===
    def _search_arxiv(self, query: str, size: int, max_results: int, with_abstract: bool) -> List[Dict[str, Any]]:
        ...
        while len(results) < max_results:
            ...
            for paper in papers:
                ...
                if with_abstract:
                    ...
                    abstract = (abstract_elem.text.strip() if abstract_elem else "").replace("△ Less", "").strip()
                ...
                link = (link_tag["href"] if isinstance(link_tag, Tag) and link_tag.has_attr("href") else "")
