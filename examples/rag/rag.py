...
class PrivateQABatch(BatchFunction):
    ...
    def execute(self):
        ...
        self.logger.info(
            f"正在处理第 {self.counter + 1}/{len(self.questions)} 个问题: {question}"
        )
        self.counter += 1
    ...
    def execute(self, data):
        ...
        try:
            ...
            if result["status"] == "success" and result.get("results"):
                ...
                if retrieved_texts:
                    self.logger.debug(
                        f"检索结果预览: {retrieved_texts[0][:50]}..."
                    )  # 显示第一条的前50个字符
                # 确保返回标准格式给后续组件
                return (query, retrieved_texts)
            else:
                error_msg = (
                    result.get("message", "未知错误") if result else "服务返回空结果"
                )
        ...
        if result["status"] == "success":
            ...
            for i, sentence in enumerate(knowledge_sentences):
                ...
                if insert_result["status"] == "success":
                    ...
                    self.logger.error(
                        f"插入第 {i+1} 条知识失败: {insert_result['message']}"
                    )

            self.logger.info(
                f"成功插入 {success_count}/{len(knowledge_sentences)} 条私密信息知识"
            )
...
def pipeline_run() -> None:
    ...
    config_path = os.path.join(
        os.path.dirname(__file__), "..", "config", "config_batch.yaml"
    )
