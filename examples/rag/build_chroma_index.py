...
def load_knowledge_to_chromadb():
    ...
    logging.info(f"=== 预加载多格式知识库到 ChromaDB ===")
    logging.info(f"存储路径: {persistence_path}")
    ...
    for file_path, collection_name in files_and_collections:
        ...
        logging.info(f"集合已创建，索引类型: {index_type}")
        ...
        logging.info(f"✓ 已添加 {len(chunk_docs)} 个文本块")
        logging.info(f"✓ 数据库文档数: {collection.count()}")

        # 测试检索
        test_query = "什么是ChromaDB"
        query_embedding = model.encode([test_query]).tolist()
        results = collection.query(query_embeddings=query_embedding, n_results=3)
        logging.info(f"检索: {test_query}")
