...
def pipeline_run():
    ...
    false_stream = true_stream.side_output(1)  # 获取第二个输出流
    true_stream.sink(FileSink, config["sink_true"])
    false_stream.sink(FileSink, config["sink_false"])
    connected_stream = true_stream.connect(false_stream)  # 连接两个流
    connected_stream.sink(TerminalSink, config["sink_terminal"])  # 输出到终端
    env.submit()
