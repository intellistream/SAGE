...
2.  Implement the `GPUResourceManager` class.
3.  **Dependencies**: Use `pynvml` for GPU monitoring.
...
5.  **Error Handling**: Ensure robust error handling for NVML calls.
...
2.  Implement the `EngineLifecycleManager` class.
3.  **Dependencies**: `subprocess`, `psutil` (for process management), `sage.common.config.ports.SagePorts`.
4.  **Key Methods**:
...
5.  **Port Management**: You might need a simple helper to find available ports if `port` is not provided, or rely on the caller.
6.  **Logging**: Log all spawn/stop events.
