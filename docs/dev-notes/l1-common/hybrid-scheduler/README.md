# sageLLM 混合调度器 - 项目总结

> **分支**: `feature/embedding_lmm_mixed_scheduler`  
> **开发周期**: 2025年11月  
> **状态**: ✅ 已完成

---

## 1. 项目概述

### 1.1 项目目标

将 `IntelligentLLMClient` 和 `IntelligentEmbeddingClient` 合并为统一的 `UnifiedInferenceClient`，通过增强 sageLLM Control Plane 实现 LLM 和 Embedding 请求在统一 GPU 资源池内的混合调度，提高资源利用率和系统吞吐量。

### 1.2 核心价值

| 价值点 | 描述 |
|--------|------|
| 🔄 **统一入口** | 单一客户端处理 LLM (chat/generate) 和 Embedding 请求 |
| 📊 **智能调度** | 基于请求类型、负载状态和优先级的自适应调度策略 |
| 🚀 **资源优化** | Embedding 批处理聚合，减少 GPU 碎片化 |
| 🔌 **OpenAI 兼容** | 统一 API Server 完全兼容 OpenAI API 规范 |
| ✅ **向后兼容** | 现有客户端代码无需任何修改 |

---

## 2. 架构设计

### 2.1 系统架构

```
┌─────────────────────────────────────────────────────────────────────────┐
│                           应用层 (Application Layer)                     │
├─────────────────────────────────────────────────────────────────────────┤
│                      UnifiedInferenceClient                              │
│                 chat() | generate() | embed()                            │
│        ┌─────────────────────┬─────────────────────┐                    │
│        │   Simple Mode       │  Control Plane Mode │                    │
│        │  (直连后端 API)      │  (通过调度器路由)    │                    │
│        └─────────────────────┴─────────────────────┘                    │
├─────────────────────────────────────────────────────────────────────────┤
│                       UnifiedAPIServer                                   │
│              (OpenAI-Compatible REST API Gateway)                        │
│   ┌────────────────┬────────────────┬────────────────┬──────────────┐   │
│   │ /v1/chat/      │ /v1/           │ /v1/           │ /v1/models   │   │
│   │ completions    │ completions    │ embeddings     │ /health      │   │
│   └────────────────┴────────────────┴────────────────┴──────────────┘   │
├─────────────────────────────────────────────────────────────────────────┤
│                    sageLLM Control Plane (增强版)                        │
│   ┌─────────────────────────────────────────────────────────────────┐   │
│   │                   RequestClassifier                              │   │
│   │    (请求分类: LLM_CHAT / LLM_GENERATE / EMBEDDING)               │   │
│   ├─────────────────────────────────────────────────────────────────┤   │
│   │                  HybridSchedulingPolicy                          │   │
│   │  • 请求类型分组          • Embedding 批处理聚合                    │   │
│   │  • 优先级调度            • 混合实例负载均衡                        │   │
│   └─────────────────────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────────────────────┤
│                        统一资源池 (GPU Pool)                             │
│   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐        │
│   │  vLLM Instance  │  │  vLLM Instance  │  │  TEI Instance   │        │
│   │  (LLM Only)     │  │  (LLM+Embed)    │  │  (Embed Only)   │        │
│   └─────────────────┘  └─────────────────┘  └─────────────────┘        │
└─────────────────────────────────────────────────────────────────────────┘
```

### 2.2 核心组件

| 组件 | 文件位置 | 职责 |
|------|----------|------|
| `RequestClassifier` | `sageLLM/control_plane/request_classifier.py` | 自动识别请求类型，筛选兼容实例 |
| `HybridSchedulingPolicy` | `sageLLM/control_plane/strategies/hybrid_policy.py` | 混合调度策略，批处理聚合 |
| `UnifiedInferenceClient` | `sage_llm/unified_client.py` | 统一客户端入口 |
| `UnifiedAPIServer` | `sage_llm/unified_api_server.py` | OpenAI 兼容 API 服务器 |
| CLI `inference` | `sage-cli/commands/apps/inference.py` | 服务管理命令 |

---

## 3. 开发阶段

### Phase 1: 基础设施准备 ✅

**目标**: 扩展类型系统，支持 Embedding 请求和混合实例

| 任务 | 交付物 | 状态 |
|------|--------|------|
| T1.1 扩展请求类型 | `RequestType` 枚举, `RequestMetadata` 扩展 | ✅ |
| T1.2 扩展实例类型 | `ExecutionInstanceType` 扩展, 能力判断方法 | ✅ |
| T1.3 Embedding 执行器 | `EmbeddingExecutor` (集成到 hybrid_policy) | ✅ |

**关键代码**:
```python
class RequestType(Enum):
    LLM_CHAT = "llm_chat"
    LLM_GENERATE = "llm_generate"
    EMBEDDING = "embedding"

class ExecutionInstanceType(Enum):
    # ... 现有类型 ...
    EMBEDDING = "embedding"       # 纯 Embedding 实例
    LLM_EMBEDDING = "llm_embedding" # 混合实例
```

### Phase 2: Control Plane 混合调度核心 ✅

**目标**: 实现请求分类和混合调度策略

| 任务 | 交付物 | 状态 |
|------|--------|------|
| T2.1 请求分类器 | `RequestClassifier` (~576 行) | ✅ |
| T2.2 混合调度策略 | `HybridSchedulingPolicy` (~885 行) | ✅ |
| T2.3 Manager 扩展 | 集成到调度流程 | ✅ |

**关键代码**:
```python
class HybridSchedulingPolicy(SchedulingPolicy):
    def schedule(self, requests, instances) -> list[SchedulingDecision]:
        # 1. 按 request_type 分组
        # 2. Embedding: 批量聚合 → 专用实例优先
        # 3. LLM: 使用配置的回退策略
        # 4. 混合实例负载均衡
```

### Phase 3: 统一客户端实现 ✅

**目标**: 创建统一客户端，合并 LLM 和 Embedding 接口

| 任务 | 交付物 | 状态 |
|------|--------|------|
| T3.1 统一客户端 | `UnifiedInferenceClient` (~1062 行) | ✅ |
| T3.2 向后兼容 | 现有客户端保持不变 | ✅ |

**关键代码**:
```python
class UnifiedInferenceClient:
    @classmethod
    def create_auto(cls) -> UnifiedInferenceClient:
        """自动检测: 环境变量 → 本地服务 → 云端 API"""
    
    def chat(self, messages, **kwargs) -> str: ...
    def generate(self, prompt, **kwargs) -> str: ...
    def embed(self, texts, **kwargs) -> list[list[float]]: ...
```

### Phase 4: 服务端统一 ✅

**目标**: 创建统一 API 服务器，集成 CLI

| 任务 | 交付物 | 状态 |
|------|--------|------|
| T4.1 统一 API Server | `UnifiedAPIServer` (~956 行) | ✅ |
| T4.2 CLI 集成 | `sage inference start/stop/status` (~650 行) | ✅ |

**API 端点**:
- `POST /v1/chat/completions` - 聊天补全
- `POST /v1/completions` - 文本补全
- `POST /v1/embeddings` - 向量嵌入
- `GET /v1/models` - 模型列表
- `GET /health` - 健康检查

### Phase 5: 测试 ✅

**目标**: 完善测试覆盖

| 任务 | 交付物 | 测试数 | 状态 |
|------|--------|--------|------|
| T5.1 单元测试 | 客户端、兼容性测试 | 55 | ✅ |
| T5.2 集成测试 | 调度、服务器、端到端测试 | 87 | ✅ |
| CLI 测试 | 命令行测试 | 23 | ✅ |
| **总计** | | **165** | ✅ |

---

## 4. 文件变更

### 4.1 新增文件 (12 个)

```
packages/sage-common/src/sage/common/components/sage_llm/
├── unified_client.py                    # 统一推理客户端
├── unified_api_server.py                # OpenAI 兼容 API 服务器
└── sageLLM/control_plane/
    ├── request_classifier.py            # 请求分类器
    └── strategies/
        └── hybrid_policy.py             # 混合调度策略

packages/sage-common/tests/
├── unit/components/sage_llm/
│   ├── test_unified_client.py           # 客户端单元测试
│   └── test_compat.py                   # 兼容性测试
└── integration/
    ├── conftest.py                      # 测试 fixtures
    ├── test_hybrid_scheduling_e2e.py    # 调度集成测试
    ├── test_unified_api_server.py       # 服务器集成测试
    └── test_unified_client_e2e.py       # 客户端集成测试

packages/sage-cli/
├── src/sage/cli/commands/apps/inference.py  # CLI 命令
└── tests/test_inference_cli.py              # CLI 测试
```

### 4.2 修改文件 (8 个)

| 文件 | 变更 |
|------|------|
| `sageLLM/control_plane/types.py` | 新增 RequestType, 扩展 RequestMetadata/ExecutionInstance |
| `sageLLM/control_plane/strategies/__init__.py` | 导出 HybridSchedulingPolicy |
| `sageLLM/control_plane/__init__.py` | 导出新增类型 |
| `sage_llm/__init__.py` | 导出 UnifiedInferenceClient, UnifiedAPIServer |
| `sage_llm/client.py` | 向后兼容层 |
| `sage_embedding/client.py` | 向后兼容层 |
| `sage-cli/commands/apps/__init__.py` | 注册 inference 命令 |
| `sage-cli/main.py` | 添加 inference 子命令 |

---

## 5. 使用指南

### 5.1 统一客户端

```python
from sage.common.components.sage_llm import UnifiedInferenceClient

# 自动检测模式
client = UnifiedInferenceClient.create_auto()

# 聊天
response = client.chat([{"role": "user", "content": "Hello"}])

# 生成
text = client.generate("Once upon a time")

# 嵌入
vectors = client.embed(["text1", "text2"])
```

### 5.2 Control Plane 模式

```python
client = UnifiedInferenceClient.create_with_control_plane(
    instances=[
        {"host": "localhost", "port": 8001, "model_name": "Qwen/...", "instance_type": "llm"},
        {"host": "localhost", "port": 8090, "model_name": "BAAI/bge-m3", "instance_type": "embedding"},
    ],
    scheduling_policy="hybrid",
)
```

### 5.3 CLI 命令

```bash
# 启动服务
sage inference start --port 8000 --background

# 查看状态
sage inference status

# 停止服务
sage inference stop
```

### 5.4 环境变量

| 变量名 | 用途 |
|--------|------|
| `SAGE_UNIFIED_BASE_URL` | 统一服务端点 |
| `SAGE_CHAT_BASE_URL` | LLM 服务端点 |
| `SAGE_EMBEDDING_BASE_URL` | Embedding 服务端点 |
| `SAGE_CHAT_MODEL` | 默认 LLM 模型 |
| `SAGE_EMBEDDING_MODEL` | 默认 Embedding 模型 |

---

## 6. 测试

### 6.1 运行测试

```bash
cd /home/yjy/SAGE

# 单元测试
conda run -n sage python -m pytest packages/sage-common/tests/unit/components/sage_llm/ -v

# 集成测试
conda run -n sage python -m pytest packages/sage-common/tests/integration/ -v

# CLI 测试
conda run -n sage python -m pytest packages/sage-cli/tests/test_inference_cli.py -v

# 全量测试
conda run -n sage python -m pytest \
    packages/sage-common/tests/unit/components/sage_llm/ \
    packages/sage-common/tests/integration/ \
    packages/sage-cli/tests/test_inference_cli.py \
    -v
```

### 6.2 测试覆盖

| 类别 | 测试数 |
|------|--------|
| 单元测试 - 客户端 | 38 |
| 单元测试 - 兼容性 | 17 |
| 集成测试 - 调度 | 22 |
| 集成测试 - 服务器 | 33 |
| 集成测试 - 客户端 | 32 |
| CLI 测试 | 23 |
| **总计** | **165** |

---

## 7. 配置参考

### 7.1 调度策略配置

```python
HybridSchedulingConfig(
    embedding_batch_size=32,           # Embedding 批处理大小
    embedding_priority="normal",        # high/normal/low/adaptive
    llm_fallback_policy="adaptive",    # fifo/priority/slo_aware/adaptive
    hybrid_instance_ratio=0.7,         # 混合实例 LLM:Embedding 比例
    prefer_specialized_instances=True, # 优先使用专用实例
)
```

---

## 8. 客户端选择指南

### 8.1 三种客户端并存

本项目**新增** `UnifiedInferenceClient`，但**保留**原有的 `IntelligentLLMClient` 和 `IntelligentEmbeddingClient`：

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         用户可选的客户端                                  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌───────────────────────┐    ┌─────────────────────────────────────┐  │
│  │ UnifiedInferenceClient │    │ IntelligentLLMClient (保留)         │  │
│  │ (新增 - 推荐)          │    │ IntelligentEmbeddingClient (保留)   │  │
│  │                       │    │                                     │  │
│  │ • chat()              │    │ • 独立使用场景                       │  │
│  │ • generate()          │    │ • 现有代码无需任何修改               │  │
│  │ • embed()             │    │ • 简单场景更简洁                     │  │
│  └───────────┬───────────┘    └──────────────┬──────────────────────┘  │
│              │                               │                          │
│              └───────────────┬───────────────┘                          │
│                              ▼                                          │
│  ┌─────────────────────────────────────────────────────────────────────┤
│  │              sageLLM Control Plane (共享调度)                        │
│  └─────────────────────────────────────────────────────────────────────┤
│                              │                                          │
│                              ▼                                          │
│  ┌─────────────────────────────────────────────────────────────────────┤
│  │                    统一资源池 (GPU Instances)                        │
│  └─────────────────────────────────────────────────────────────────────┘
└─────────────────────────────────────────────────────────────────────────┘
```

### 8.2 如何选择？

| 场景 | 推荐客户端 | 理由 |
|------|-----------|------|
| 新项目，同时需要 LLM 和 Embedding | `UnifiedInferenceClient` | 统一入口，享受混合调度 |
| 现有项目，只使用 LLM | `IntelligentLLMClient` | 无需修改，向后兼容 |
| 现有项目，只使用 Embedding | `IntelligentEmbeddingClient` | 无需修改，向后兼容 |
| 简单脚本，只需要单一功能 | 独立客户端 | 代码更简洁 |
| 需要高级调度（Control Plane） | `UnifiedInferenceClient` | 支持 Control Plane 模式 |

### 8.3 代码示例

```python
# 方式 1: 新代码 - 使用 UnifiedInferenceClient (推荐)
from sage.common.components.sage_llm import UnifiedInferenceClient
client = UnifiedInferenceClient.create_auto()
response = client.chat([{"role": "user", "content": "Hello"}])
vectors = client.embed(["text1", "text2"])

# 方式 2: 现有代码 - 继续使用独立客户端 (仍然有效)
from sage.common.components.sage_llm import IntelligentLLMClient
from sage.common.components.sage_embedding import IntelligentEmbeddingClient
llm = IntelligentLLMClient.create_auto()
embedder = IntelligentEmbeddingClient.create_auto()
```

> **重要**: 三种客户端**都连接到同一个 sageLLM 资源池**，享受混合调度的好处。选择哪种客户端取决于你的使用场景和代码风格偏好。

### 8.4 为什么保留原有客户端？

| 原因 | 说明 |
|------|------|
| **向后兼容** | 现有使用 `IntelligentLLMClient` / `IntelligentEmbeddingClient` 的代码**无需任何修改** |
| **简单场景** | 只需要 LLM 或只需要 Embedding 的场景，用原有客户端更简洁 |
| **渐进迁移** | 团队可以逐步迁移到 `UnifiedInferenceClient`，而不是一次性修改所有代码 |

---

## 9. 相关文档

- [PULL_REQUEST.md](./PULL_REQUEST.md) - PR 详细说明
- [SAGE 架构文档](../../../docs-public/docs_src/dev-notes/package-architecture.md)
- [copilot-instructions.md](../../../../.github/copilot-instructions.md) - LLM & Embedding 使用指南

---

*文档最后更新: 2025年11月27日*
