...
\item It reports a 10-seed offline evaluation, trace-guided evidence-policy ablations, substrate probes, and a three-workload-seed single-accelerator online LLM-reducer comparison, separating derived matrices from real-online mechanism evidence while showing how coverage gates, constrained validation, and cost accounting make reducer behavior inspectable under one schema and scorer.
\end{enumerate}
...
\begin{table*}[t]
\caption{One-token pairwise action probe on one NPU endpoint, repeated over three workload seeds and three full-coverage hard cases (nine case/seed runs per reducer). Free-form pairwise JSON fails the schema contract in all nine runs; the action reducer restricts model output to an enum and lets the system assemble validated edits. Evidence: real-online. This controlled mechanism result does not establish production-trace or cross-model robustness.}
...
\midrule
Semantic graph & 0.4427 & 0.4722 & 0.4318 & 0.5185 & 0.33 & 0.0 & 0.0 & 0 & 0 & 0 \\
Hybrid hint & 0.6596 & 0.9167 & 0.7204 & 0.7083 & 0.24 & 0.0 & 0.0 & 0 & 0 & 0 \\
Validated pairwise & 0.6596 & 0.9167 & 0.7204 & 0.7083 & 7498.64 & 508.7 & 0.0 & 0 & 9 & 9 \\
Validated action & 0.9630 & 0.9167 & 0.9301 & 0.9815 & 324.19 & 536.1 & 2.2 & 0 & 0 & 0 \\
\bottomrule
\end{tabular*}
\end{table*}

A follow-up pairwise edit probe narrows the interface further. Instead of asking the model to rewrite complete candidates, the reducer proposes short candidate pairs. The first version still asks for a small JSON decision list; across the three-seed hardcase matrix, every validated free-form call fails the JSON/schema contract and falls back to \texttt{hybrid-hint}. Table~\ref{tab:pairwise-action-online} shows the next design step: the model is restricted to one token, \texttt{KEEP}, \texttt{MERGE}, \texttt{SPLIT}, or \texttt{ABSTAIN}, for each proposed pair. The runtime then assembles the edit payload and applies the same schema, root/affected-service, evidence-reference, and fallback validators. Over nine case/seed runs, the validated action path records zero invalid actions, invalid schemas, or fallbacks and raises mean F1 from 0.7204 to 0.9301. Disconnected merge reaches F1 1.0000 on all three seeds with three accepted merges per run. Temporal fragmentation reaches F1 0.8000/1.0000/1.0000; the seed-7 run retains two unmatched fragments, which the failure trace exposes. Ambiguous overmerge preserves F1 0.8571 on all seeds while accepting no edits, making the token cost of abstention visible. These are three controlled workload seeds on one model and endpoint, not repeated sampling or production telemetry. The evidence supports a sharper systems claim: reliable LLM-backed semantic reduction needs a constrained edit interface where the model supplies bounded semantic judgments and the system owns JSON construction, validation, and traceability.
...
Semantic MapReduce separates the semantic control plane from data and serving planes. Data engines retain scans, joins, windows, and distributed execution; serving engines retain inference. The orchestration layer owns evidence eligibility, reducer policy, hypothesis provenance, validation, and trace replay. The evaluated mechanism needs no new kernels or scheduler semantics. This boundary leaves four systems challenges: broader data-plane attachment while preserving the evidence schema; production-derived workload validation; cross-model and repeated-sampling studies of bounded actions; and co-design only when a validator truly needs serving-level constrained decoding. Such extensions should preserve the evaluation order used here: establish coverage, measure reducer quality, then report live latency and token cost.
