...
\item It reports a 10-seed evaluation, an operator ablation, and real-endpoint readiness probes, showing where incident-level reduction helps, where map-stage evidence policy still determines recall, and why LLM-backed reducers require an explicit output contract before quality claims are meaningful.
\end{enumerate}
...
The implementation has three components. The generator creates synthetic telemetry and hidden incidents. The map stage implements \textsc{Shard}, \textsc{MapEvidence}, and \textsc{Normalize} by partitioning events, computing local summaries, and producing anomaly candidates. The reducer implements \textsc{GroupEvidence} and \textsc{SemanticReduce} over shard summaries. The default reducer is deterministic: it clusters candidates using incident type, service, region, and temporal overlap. The map-only and window-aggregation reducers are diagnostic baselines that approximate alert-only and windowed-analytics workflows without incident-level semantic reduction. The \texttt{llm-stub} reducer is an interface placeholder that exercises the same resolver and report path while delegating to the deterministic reducer.
...
Endpoint health is not sufficient for a semantic reducer. Earlier probes showed that a live endpoint can still fail the reducer contract by emitting malformed JSON or by crashing on the first generation request. The final single-NPU run fixes those serving-readiness issues and completes the LLM-backed reducer path, as shown in Table~\ref{tab:llm-reducer-online}. The 2k run recovers only one of four injected incidents because the evidence set is sparse; the 20k run recovers all four with denser evidence. This is a useful sanity result because it demonstrates an operational \textsc{SemanticReduce} integration under the common evidence/scoring contract. It is not evidence that LLM reduction dominates the deterministic reducer across workloads.
