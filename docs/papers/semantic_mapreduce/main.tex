...
Sections~2--5 motivate and define the contract and its runtime integration; Sections~6--7 describe the workload and evaluation; Sections~8--10 discuss limitations, trust, and related work.
...
\begin{table}[t]
\caption{Reducer invocation as a runtime-owned state transition. The model can propose only the action column; it cannot construct, validate, or publish report state.}
\label{tab:runtime-ownership}
\centering
\small
\begin{tabular}{@{}p{0.18\columnwidth}p{0.42\columnwidth}p{0.29\columnwidth}@{}}
\toprule
Phase & Runtime-owned object or check & Model authority \\
\midrule
Admit & typed evidence IDs, candidate digest, $H_0$ & none \\
Propose & pair ID and legal action set & one enum action \\
Assemble & evidence-linked edit $\Delta$ & none \\
Validate & schema, evidence coverage, root/service consistency & none \\
Publish & committed $H$ or preserved $H_0$ & none \\
Recover & checkpointed state/trace and replay digest & none \\
\bottomrule
\end{tabular}
\end{table}
