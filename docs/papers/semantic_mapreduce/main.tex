...
\title{Semantic MapReduce: An Auditable Operator Contract for LLM-Backed Reduction}
...
The semantic-merge workloads in Table~\ref{tab:workload-contract-coverage} separate positive edits from no-op and adversarial obligations. This matters because an edit protocol can appear strong if every case rewards merging. Sanity and cascade families test ordinary incident formation; concurrent and false-correlation families require separation; partial evidence distinguishes a map-coverage limit from a reducer error; and the three ambiguity families force merge, split, or abstain decisions after coverage is complete. All nine families use the same evidence schema and scorer in the ten-seed derived matrix; the hardcase subset is also evaluated real-online.

\begin{table*}[t]
\caption{Workload coverage is organized by reducer-contract obligation. All nine families have ten-seed derived matrices and three-seed runtime fault injection; the final three-row subset also has real-online action evidence.}
...
\midrule
Single service & local, complete & preserve a trivial incident & 10-seed derived \\
Cascade & cross-service, complete & fuse downstream symptoms & 10-seed derived \\
Shared bottleneck & shared cause, complete & recover one common cause & 10-seed derived \\
Concurrent & overlapping independent incidents & keep incident units separate & 10-seed derived \\
False correlation & correlated but unrelated symptoms & reject a spurious merge & 10-seed derived \\
Partial evidence & root observation absent & expose coverage/inference boundary & 10-seed derived \\
Ambiguous overmerge & candidate too coarse & split or safely abstain & 10-seed derived + 3-seed real-online \\
Disconnected merge & one incident fragmented by grouping & accept evidence-preserving merges & 10-seed derived + 3-seed real-online \\
Temporal split & one incident spans candidate windows & merge without losing temporal support & 10-seed derived + 3-seed real-online \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\caption{One-token pairwise action probe on one NPU endpoint across nine families and three seeds (27 runs per reducer). The action reducer restricts model output to an enum and lets the system assemble validated edits. Evidence: real-online; one model/endpoint.}
