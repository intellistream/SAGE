...
\draw[black!20, line width=0.3pt] (3.42,0.08) -- (3.42,1.82);
\node[note, anchor=west] at (3.54,1.74) {\textbf{k ev/s}};
\node[callout, anchor=west] at (3.54,1.50) {101.6k/s};
\node[callout, anchor=west] at (3.54,0.92) {104.2k/s};
\node[callout, anchor=west] at (3.54,0.34) {104.0k/s};

\draw[black!20, line width=0.3pt] (4.48,0.08) -- (4.48,1.82);
\node[note, anchor=west] at (4.60,1.74) {\textbf{Output unit}};
\node[callout, anchor=west] at (4.60,1.50) {shard symptoms};
\node[callout, anchor=west] at (4.60,0.92) {window fragments};
\node[callout, anchor=west] at (4.60,0.34) {evidence-linked\\incidents};

\end{tikzpicture}
\caption{Motivating result. Semantic reduction improves incident-level quality without a visible local throughput penalty in this workload because it reduces compact evidence, not raw events. The gray rows stop at symptoms or window fragments; semantic reduction outputs evidence-linked incident hypotheses.}
\Description{A compact chart showing map-only alerts at mean F1 0.3075 and throughput 101.6 thousand events per second, window aggregation at mean F1 0.7129 and throughput 104.2 thousand events per second, and semantic reduction at mean F1 0.9579 and throughput 104.0 thousand events per second. A side column explains that the first two baselines output shard-local symptoms and window fragments, while semantic reduction outputs evidence-linked incidents.}
