...
This distinction appears even in a controlled telemetry workload. In an NPU-backed LLM serving scenario, Figure~\ref{fig:intro-motivation} shows that map-only alerts reach 0.3075 mean F1 and window aggregation reaches 0.7129, while an incident-level reducer reaches 0.9579 across 20 tail-aware runs. The reason is not faster scanning; it is that the reducer turns duplicate shard symptoms and window-level detections into evidence-linked incident hypotheses. A separate adapter probe shows that LangGraph, LlamaIndex, and Ray-local wrappers can drive the same contract, but their quality is identical only because they reuse the same reducer and scorer. Existing substrates can carry the computation; the missing abstraction is first-class semantic reduction with auditable evidence.
...
\begin{tikzpicture}[
  ...
  note/.style={align=left, font=\sffamily\tiny},
  callout/.style={align=left, font=\sffamily\tiny, text=black!75}
]
\def\xzero{1.34}
\def\xscale{2.15}
...
\draw[bar, fill=black!18] (\xzero,1.36) rectangle (\xzero+0.3075*\xscale,1.64);
\draw[bar, fill=black!38] (\xzero,0.78) rectangle (\xzero+0.7129*\xscale,1.06);
\draw[bar, fill=blue!45!black] (\xzero,0.20) rectangle (\xzero+0.9579*\xscale,0.48);

\node[anchor=west, font=\sffamily\tiny] at (\xzero+0.3075*\xscale+0.05,1.50) {0.308};
\node[anchor=west, font=\sffamily\tiny] at (\xzero+0.7129*\xscale+0.05,0.92) {0.713};
\node[anchor=east, font=\sffamily\tiny, text=white] at (\xzero+0.9579*\xscale-0.05,0.34) {0.958};

\draw[black!20, line width=0.3pt] (3.72,0.08) -- (3.72,1.82);
\node[note, anchor=west] at (3.86,1.74) {\textbf{Output unit}};
\node[callout, anchor=west] at (3.86,1.50) {shard symptoms};
\node[callout, anchor=west] at (3.86,0.92) {window fragments};
\node[callout, anchor=west] at (3.86,0.34) {evidence-linked\\incidents};

\end{tikzpicture}
\caption{Motivating result. The advantage is not a generic wrapper effect: semantic reduction changes the output unit from local/window symptoms into evidence-linked incident hypotheses.}
\Description{A bar chart showing map-only alerts at mean F1 0.3075, window aggregation at 0.7129, and semantic reduction at 0.9579. A side legend explains that the first two baselines output shard-local symptoms and window fragments, while semantic reduction outputs evidence-linked incidents.}
