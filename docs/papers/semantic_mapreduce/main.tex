...
The workload instantiates the operator model directly. \textsf{Shard} partitions generated telemetry by shard count, service, region, and time. \textsf{MapEvidence} computes shard-level statistics, anomaly candidates, and local summaries. \textsf{Normalize} converts candidates into scored evidence objects with provenance fields. \textsf{GroupEvidence} clusters candidates by incident type, service, region, and temporal overlap. \textsf{SemanticReduce} emits incident hypotheses with map-only, window, deterministic, hybrid, stubbed, or OpenAI-compatible reducers. \textsf{Edit} and \textsf{Validate} are exercised by candidate-level and one-token action reducers: the model may propose bounded keep/merge/split/abstain decisions, while the system assembles evidence-linked edits, checks schema/root/affected-service invariants, and falls back when required. \textsf{ReportTrace} scores hypotheses, records matched/missed incidents, classifies reducer failures, and emits per-run reports.
...
\begin{table}[t]
\caption{Semantic-merge suite across seven workload families and ten seeds. The suite includes a single-service sanity case, cascade and shared-bottleneck incidents, concurrent incidents, false correlations, partial-evidence cases, and ambiguous over-merge cases.}
...
\begin{table}[t]
\caption{Coverage gate over ten seeds. A reducer-quality comparison is meaningful only when injected incidents enter the evidence objects. Baseline-aware evidence reaches full coverage for 10K and larger settings in this workload; the 2K tail-aware setting does not.}
...
\begin{table*}[t]
\caption{One-token pairwise action probe on the same single-NPU endpoint. The three hard cases have full evidence coverage. Free-form pairwise reducers still fail the schema contract, while the action reducer restricts model output to an enum and lets the system assemble validated edits. This is a mechanism probe, not a multi-seed robustness claim.}
...
\toprule
Reducer & Precision & Recall & F1 & Support recall & Reduce ms & Tokens & Accepted edits & Invalid act. & Fallback \\
\midrule
Semantic graph & 0.3810 & 0.3333 & 0.3463 & 0.4167 & 0.59 & 0.0 & 0.0 & 0.0 & 0.0 \\
Hybrid hint & 0.6349 & 0.9167 & 0.6948 & 0.6528 & 0.26 & 0.0 & 0.0 & 0.0 & 0.0 \\
LLM pairwise & 0.3810 & 0.3333 & 0.3463 & 0.4167 & 911.48 & 506.7 & 0.0 & 0.0 & 1.0 \\
Validated LLM pairwise & 0.6349 & 0.9167 & 0.6948 & 0.6528 & 841.94 & 506.7 & 0.0 & 0.0 & 1.0 \\
LLM pairwise action & 0.8889 & 0.9167 & 0.8857 & 0.9444 & 729.29 & 534.0 & 2.3 & 0.0 & 0.0 \\
Validated LLM pairwise action & 0.8889 & 0.9167 & 0.8857 & 0.9444 & 723.74 & 534.0 & 2.3 & 0.0 & 0.0 \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\textbf{Agentic data analysis.} Agentic systems can write queries, call tools, inspect results, and refine answers. They are useful for exploration, but high-volume operational analysis also needs replayable control flow. This prototype chooses explicit map stages, reducer stages, and evidence objects to improve evaluation and auditability.
