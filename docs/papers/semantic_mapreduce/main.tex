...
\begin{table}[t]
\caption{Adjacent-framework adapter baselines over the same 20 tail-aware runs. Each adapter receives the same evidence objects and calls the same incident reducer and scorer. Equal quality is therefore expected; the result shows that adjacent frameworks can host the workflow but do not provide semantic reduction unless the evidence contract and reducer are supplied. Ray-local also reported NPU-detection and \texttt{/tmp/ray} space warnings on this host, so its local overhead should not be read as tuned Ray performance.}
...
\midrule
SAGE-local & 0.9579 & 58,136 & 1,303.22 \\
