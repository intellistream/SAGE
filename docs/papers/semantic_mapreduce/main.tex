...
\node[anchor=west, font=\sffamily\tiny] at (\xzero+0.3075*\xscale+0.05,1.50) {0.308: duplicate local symptoms};
\node[anchor=west, font=\sffamily\tiny] at (\xzero+0.7129*\xscale+0.05,0.92) {0.713: window duplicates};
\node[anchor=west, font=\sffamily\tiny] at (\xzero+0.9579*\xscale+0.05,0.34) {0.958};
...
\Description{A bar chart showing map-only alerts at mean F1 0.3075, window aggregation at 0.7129, and incident reduction at 0.9579. Labels say the first two baselines suffer duplicate local symptoms and window duplicates, while the incident reducer deduplicates, clusters, and traces evidence. A note says LangGraph, LlamaIndex, and Ray-local adapters match only with the same evidence contract and reducer.}
